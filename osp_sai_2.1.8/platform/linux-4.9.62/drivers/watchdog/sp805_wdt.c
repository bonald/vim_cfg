...
static int wdt_restart(struct watchdog_device *wdd, unsigned long action, void *cmd)
{	
	struct sp805_wdt *wdt = watchdog_get_drvdata(wdd);
...
static irqreturn_t sp805_wdt_irq(int irq, void *dev_id)
{
...
static const struct watchdog_info wdt_info = {
	.options = WDIOF_MAGICCLOSE | WDIOF_SETTIMEOUT | WDIOF_KEEPALIVEPING | WDIOF_PRETIMEOUT,
...
sp805_wdt_probe(struct amba_device *adev, const struct amba_id *id)
{
	...
	ret = devm_request_irq(&adev->dev, adev->irq[0], sp805_wdt_irq,
				  0, "sp805", wdt);
	if (ret < 0) {
