...
static int port_err_status(int status,int port,void *dev_id)
{
	...
	if(status & DDR_ERR_OVER_TOPBOUND)
	{
		...
		temp =(addr_l
			|(((unsigned long)((addr_h >> 12) &0x3 )) << 32)
			);
		
		printk(KERN_EMERG "ERROR:port%d is out of top-bound range!\n The error address is 0x%p\n",id,(void*)temp);
	}

	if(status & DDR_ERR_WCMDQ_OVER)
	{
		printk(KERN_ERR "ERROR:port%d write command queue is overflow!\n",id);
	}

	if(status & DDR_ERR_WCMDQ_UNDER)
	{
		printk(KERN_ERR "ERROR:port%d write command queue is underflow!\n",id);
	}
	...
	{
		printk(KERN_ERR "ERROR:port%d write response queue is overflow!\n",id);
	}

	if(status & DDR_ERR_WESPQ_UNDER)
	{
		printk(KERN_ERR "ERROR:port%d write response queue is underflow!\n",id);
	}
	...
	{
		printk(KERN_ERR "ERROR:port%d read command queue is overflow!\n",id);
	}

	if(status & DDR_ERR_RCMDQ_UNDER)
	{
		printk(KERN_ERR "ERROR:port%d read command queue is underflow!\n",id);
	}	
	...
	{
		printk(KERN_ERR "ERROR:port%d read response queue is overflow!\n",id);
	}	

	if(status & DDR_ERR_RESPQ_UNDER)
	{
		printk(KERN_ERR "ERROR:port%d read response queue is underflow!\n",id);
	}	
	return 1;
}
...
static int ctc5236_mc_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(&pdev->dev, mci->irq, ctc_mc_err_handler, 0, dev_name(&pdev->dev), mci);
	if(ret < 0){
