...
#include <linux/platform_data/gpio-ctcapb.h>
...
static void ctcapb_toggle_trigger(struct ctcapb_gpio_port *port, unsigned int offs)
{
     ...
     struct ctcapb_port_property *pp)
{
	...
	if (err) {
		dev_info(port->gpio->dev, "irq_alloc_domain_generic_chips failed\n");
	...
	for (i = 0; i < 2; i++) {
		...
		ct->regs.ack = (&port->regs->GpioEoiCtl - &port->regs->GpioDataCtl)*4;
		ct->regs.mask = (&port->regs->GpioIntrMask - &port->regs->GpioDataCtl)*4;
			       ...
			       unsigned int offs)
{
	...
	if(port->idx == 0){
               ...
               port->regs = (struct GpioSoc_port_regs *)&gpio->regs->GpioDataCtl;
	}else{
               dat = &gpio->regs->GpioHsReadData;
               set = &gpio->regs->GpioHsDataCtl;
               dirout = &gpio->regs->GpioHsOutCtl;
               port->regs = (struct GpioSoc_port_regs *)&gpio->regs->GpioHsDataCtl;
	}
	...
	port->gc.set_debounce = ctcapb_gpio_set_debounce;
...
static struct ctcapb_platform_data *
ctcapb_gpio_get_pdata(struct device *dev)
{
...
static int ctcapb_gpio_probe(struct platform_device *pdev)
{
	...
	gpio->regs = (struct    GpioSoc_regs *)devm_ioremap_resource(&pdev->dev, res);
	if (IS_ERR(gpio->regs))
