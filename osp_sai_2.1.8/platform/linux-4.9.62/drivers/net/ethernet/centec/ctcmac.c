...
extern const struct ethtool_ops ctcmac_ethtool_ops ;
...
static int ctcmac_of_init(struct platform_device *ofdev, struct net_device **pdev)
{
	...
	*pdev = alloc_etherdev_mq(sizeof(*priv), num_tx_qs);
    ...
    if (!is_valid_ether_addr(dev->dev_addr)) //zwh modify for random gmac_addr
    {
	...
	/* Get speed capalility, CTC5236 support 10M/100M/1000M speed */
	priv->supported = SUPPORTED_10baseT_Full | SUPPORTED_Autoneg;
...
#ifdef CONFIG_CTC_PLATFORM /* for bug 53382 modified by tongzb 2019-10-15 */
    /* modified by liwh for bug 54288, 2019-11-07 */
	/*if (phydev->link)
    {
        pr_warn(":kernel:INTERFACE:interface mgmt-if state change to up\n");
    }
    else
    {
        pr_warn(":kernel:INTERFACE:interface mgmt-if state change to down\n");
    }*/
	/* liwh end */
#endif
...
#if 0
		if(!(priv->ndev->flags & IFF_PROMISC) && ctcmac_filter(priv, skb)){
			dev_kfree_skb(skb);
			skb = NULL;
			continue;
		}
