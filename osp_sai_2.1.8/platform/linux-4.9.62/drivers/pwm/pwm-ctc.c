...
static inline void ctc_pwm_writel(struct ctc_pwm_chip *chip, unsigned int num, unsigned long offset, unsigned long val)
{
	regmap_write(chip->regmap_base, offsetof(struct SysCtl_regs, SysPwmCtl) + offset + num * 0x8, val);
}

static inline u32 ctc_pwm_readl(struct ctc_pwm_chip *chip, unsigned int num, unsigned long offset)
{
	u32 val;
	regmap_read(chip->regmap_base, offsetof(struct SysCtl_regs, SysPwmCtl) + offset + num * 0x8, &val);
	return val;
}

static inline u32 ctc_tach_readl(struct ctc_pwm_chip *chip, unsigned int num, unsigned long offset)
{
	u32 val;
	regmap_read(chip->regmap_base, offsetof(struct SysCtl_regs, SysTachLog) + offset + num * 0x8, &val);
	return val;
}

static int ctc_pwm_config(struct pwm_chip *chip, struct pwm_device *pwm, int duty_ns, int period_ns)
{
...
static void ctc_pwm_get_state(struct pwm_chip *chip, struct pwm_device *pwm, struct pwm_state *state)
{
	struct ctc_pwm_chip *pc = to_ctc_pwm_chip(chip);
	u32 cur_value;
	u32 cur_value_2;
...
static int ctc_pwm_capture(struct pwm_chip *chip, struct pwm_device *pwm, struct pwm_capture *result, unsigned long timeout)
{
	struct ctc_pwm_chip *pc = to_ctc_pwm_chip(chip);
	u32 period_tach;
	u32 duty_tach;
