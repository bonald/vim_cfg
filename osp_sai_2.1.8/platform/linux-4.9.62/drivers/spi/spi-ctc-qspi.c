...
static noinline int ctc_write_tx_buf(struct ctc_qspi *ctc_qspi, u8 offset, u32 value)
{
...
static noinline int fill_tx_entry(struct ctc_qspi *ctc_qspi, u8 *buf, int i, u8 off)
{
...
static void ctc_fill_tx_buf(struct ctc_qspi *ctc_qspi, u8* offset, u8* buf, u32 len)
{
...
static void ctc_stepx_conf(struct ctc_qspi *ctc_qspi, u8 lanes, u32 bytes, u32 output_en)
{
	u32 cycle =0;
	u32 stepx_conf = 0;
	...
	if (ctc_qspi->step == 0){
		ctc_reg_write_mask(ctc_qspi, PIO_STEP0_CONF(ctc_qspi->qspi_mode), stepx_conf, 0xffffffff);
		ctc_qspi->step ++;
	}else if (ctc_qspi->step == 1){
		ctc_reg_write_mask(ctc_qspi, PIO_STEP1_CONF(ctc_qspi->qspi_mode), stepx_conf, 0xffffffff);	
		ctc_qspi->step ++;
	}else if (ctc_qspi->step == 2){
		ctc_reg_write_mask(ctc_qspi, PIO_STEP2_CONF(ctc_qspi->qspi_mode), stepx_conf, 0xffffffff);	
		ctc_qspi->step ++;
	}else if (ctc_qspi->step == 3){
		ctc_reg_write_mask(ctc_qspi, PIO_STEP3_CONF(ctc_qspi->qspi_mode), stepx_conf, 0xffffffff);	
		ctc_qspi->step ++;
	}
...
static void ctc_extra_rx_buf(struct ctc_qspi *ctc_qspi, u8 offset, u8* buf, u8 len)
{
	int i = 0;
    	while(i < len)
    	{
...
static int ctc_transfer_for_PP_mode(struct ctc_qspi *ctc_qspi, struct spi_transfer *p_xfers[], u8 xfers_num, u32 msg_len)
{
	u8 i;
	u32 pp_conf;
	u32 timeout = 0;
	...
	for(i =0; i<mem_ops.addr.nbytes; i++){
		mem_ops.addr.val |= ((u8 *)p_xfers[1]->tx_buf)[i] << (8 * (mem_ops.addr.nbytes - i - 1));
	}
	...
	offset =0;
	ctc_qspi->tx_entry = 0;
	ctc_fill_pp_buf(ctc_qspi, &offset, (u8 *)mem_ops.data.buf.out, mem_ops.data.nbytes);

	/* PP CMD */
	ctc_reg_write_mask(ctc_qspi, PP_CMD_CODE,  mem_ops.cmd.opcode, 0xffffffff);
	pp_conf = ctc_pp_conf(mem_ops.cmd.buswidth, 1);
	ctc_reg_write_mask(ctc_qspi, PP_CMD_CONF,  pp_conf, 0xffffffff);

	/* PP ADDR */
	ctc_reg_write_mask(ctc_qspi, PP_ADDR_CODE, mem_ops.addr.val, 0xffffffff);
...
static int ctc_transfer_for_PIO(struct ctc_qspi *ctc_qspi, struct spi_transfer *p_xfers[], u8 xfers_num, u32 msg_len)
{
	u8   i;
	u8   offset;
	u32 timeout = 0;
	...
	for(i = 0; i < xfers_num; i++){
		if(p_xfers[i]->tx_buf){
			ctc_fill_tx_buf(ctc_qspi, &offset, (u8*)p_xfers[i]->tx_buf, p_xfers[i]->len);
			ctc_stepx_conf(ctc_qspi, p_xfers[i]->tx_nbits, p_xfers[i]->len, 1);
		}else{
			ctc_fill_tx_buf(ctc_qspi, &offset, (u8*)p_xfers[i]->rx_buf, p_xfers[i]->len);
			ctc_stepx_conf(ctc_qspi, p_xfers[i]->rx_nbits, p_xfers[i]->len, 0);
		}
	}
	...
	if(p_xfers[xfers_num-1]->rx_buf){
		ctc_extra_rx_buf(ctc_qspi, p_xfers[xfers_num-1]->len-1, 
			p_xfers[xfers_num-1]->rx_buf, p_xfers[xfers_num-1]->len);
	}
...
static int ctc_qspi_start_transfer_one(struct spi_master *master, struct spi_message *msg)
{
	...
	if(xfers[xfer_num-1]->tx_buf && xfer_num>=3){
		msg->actual_length = ctc_transfer_for_PP_mode(ctc_qspi, xfers, xfer_num, msg_len);
	}else{
		msg->actual_length = ctc_transfer_for_PIO(ctc_qspi, xfers, xfer_num, msg_len);
	}
