 ...
 */
static u32 __ctc_calc_ss_cnt(void)
{
	return CTC_SS_SCL_Timing / 2 / 20;
}

static u32 __ctc_calc_fs_cnt(void)
{
	return CTC_FS_SCL_Timing / 2 / 20  + 1;
}
...
int i2c_ctc_init(struct ctc_i2c_dev *dev)
{
	...
	if ((dev->master_cfg & CTC_IC_CON_SPEED_MASK) == CTC_IC_CON_SPEED_FAST){
		hcnt = __ctc_calc_fs_cnt() - 14 - 4;
		lcnt = __ctc_calc_fs_cnt() - 1 -2;

		ctc_writel(dev, hcnt, CTC_IC_FS_SCL_HCNT);
		ctc_writel(dev, lcnt, CTC_IC_FS_SCL_LCNT);
		dev_info(dev->dev, "Fast-mode HCNT:LCNT = %d:%d\n", hcnt, lcnt);
	
	}else {
		hcnt = __ctc_calc_ss_cnt() - 14 - 4;
		lcnt = __ctc_calc_ss_cnt() - 1 - 2;
		
		ctc_writel(dev, hcnt, CTC_IC_SS_SCL_HCNT);
		ctc_writel(dev, lcnt, CTC_IC_SS_SCL_LCNT);
		dev_info(dev->dev, "Standard-mode HCNT:LCNT = %d:%d\n", hcnt, lcnt);
	}
...
static int i2c_ctc_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
...
static void i2c_ctc_xfer_msg(struct ctc_i2c_dev *dev)
{
	...
	for (; dev->msg_write_idx < dev->msgs_num; dev->msg_write_idx++) {
		...
		if (!(dev->status & STATUS_WRITE_IN_PROGRESS)) {
			...
			if ((dev->master_cfg & CTC_IC_CON_RESTART_EN) && (dev->msg_write_idx > 0))
				need_restart = true;
		}
...
tx_aborted:
	if ((stat & (CTC_IC_INTR_TX_ABRT | CTC_IC_INTR_STOP_DET)) || dev->msg_err)
...
int i2c_ctc_probe(struct ctc_i2c_dev *dev)
{
	...
	snprintf(adap->name, sizeof(adap->name), "Centec TsingMa SoC's I2C adapter");
...
static int ctc_i2c_plat_probe(struct platform_device *pdev)
{
	...
	dev->master_cfg |= CTC_IC_CON_MASTER | CTC_IC_CON_SLAVE_DISABLE | CTC_IC_CON_RESTART_EN;
