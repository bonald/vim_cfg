...
static struct pinmux_cfg ls1043a_pinmux_cfg = {
	...
	.pmuxcr_set_bit = 0x1000,//i2c3_scl
#else
	.pmuxcr_set_bit = 0x10,
#endif
};
...
static int i2c_imx_recovery_for_layerscape(struct imx_i2c_struct *i2c_imx)
{
    ...
    /* gen i2c reset single */
    ret = gpio_request(i2c_imx->reset_gpio, i2c_imx->adapter.name);
    if (ret) {
        dev_err(&i2c_imx->adapter.dev,
            "can't get gpio: %d\n", ret);
        return ret;
    ...
    gpio_direction_output(i2c_imx->reset_gpio,0);
    udelay(10);
    gpio_direction_output(i2c_imx->reset_gpio,1);
    udelay(10);
    gpio_free(i2c_imx->reset_gpio);
#else
	u32 pmuxcr = 0;
	int ret;
	unsigned int i, temp;
...
#ifdef 	CONFIG_CTC_PLATFORM
static int i2c_ctc_recovery_bus(struct i2c_adapter *adapter)
{
    struct imx_i2c_struct *i2c_imx = i2c_get_adapdata(adapter);
    if(i2c_imx->layerscape_bus_recover == 1)
    {
        dev_info(&i2c_imx->adapter.dev, "Do i2c bus recovery.....\n");
						...
						struct i2c_msg *msgs, int num)
{
	...
	if (result) {
		/* timeout */
		if (((result == -ETIMEDOUT)||(result == -EAGAIN)) && (i2c_imx->layerscape_bus_recover == 1))
		{
            printk(KERN_WARNING "i2c is busy try to recovery\n");
			i2c_imx_recovery_for_layerscape(i2c_imx);
		}
		else
			goto out;
...
out:
    ...
    if(result != 0)
    {
        ...
        {
            for (i = 0; i < num; i++) {
                printk(KERN_WARNING "polling access 0x%x at 0x%x = %d\n",msgs[i].addr&0x7f,msgs[i].buf[0],-result);
            }
        }
		if (((result == -EAGAIN)||(result == -ETIMEDOUT) ) && (i2c_imx->layerscape_bus_recover == 1))
		{
            printk(KERN_WARNING "i2c is busy try to recovery\n");
			i2c_imx_recovery_for_layerscape(i2c_imx);
		}
		...
		struct platform_device *pdev)
{
    ...
    dev_info(&pdev->dev, "fsl-scl-gpio is %d\n",i2c_imx->gpio);
    ...
    u32 pmuxcr = 0;
    int ret=0;
	/* configure IIC3_SCL/GPIO pin as a GPIO input */
	if (i2c_imx->need_set_pmuxcr == 1) {
		pmuxcr = ioread32be(i2c_imx->pmuxcr_addr);
		if (i2c_imx->pmuxcr_endian == BIG_ENDIAN)
			iowrite32be(i2c_imx->pmuxcr_set|pmuxcr,
    ...
    i2c_imx->reset_gpio = of_get_named_gpio(np, "fsl-scl-reset", 0);
	if (!gpio_is_valid(i2c_imx->reset_gpio)) {
		dev_info(&pdev->dev, "fsl-scl-reset not found\n");
		return 0;
	}
    dev_info(&pdev->dev, "fsl-scl-reset is %d\n",i2c_imx->reset_gpio);
	ret = gpio_request(i2c_imx->reset_gpio, i2c_imx->adapter.name);
	if (ret) {
		dev_err(&i2c_imx->adapter.dev,
			"can't get gpio: %d\n", ret);
