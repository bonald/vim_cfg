...
#define SDHCI_REFCLK_100M  100000000

static bool support_vsel = false;
...
static u32 sdhci_ctc5236_readl(struct sdhci_host *host, int reg)
{
	...
	if (SDHCI_CAPABILITIES_1 == reg)		
		ret &= ~(SDHCI_SUPPORT_SDR50 | SDHCI_SUPPORT_SDR104 | SDHCI_SUPPORT_DDR50);	
...
void ctc_sdhci_set_clock(struct sdhci_host *host, unsigned int clock)
{
	...
	if (clock >= SDHCI_REFCLK_100M){
		/* Use IP default 180 shift phase */
		regmap_read(regmap_base, offsetof(struct SysCtl_regs, SysMshCfg), &val);
		val &= (~SYS_MSH_CFG_W0_MSH_INTF_C_CLK_TX_PHASE_SEL);
		val &= (~SYS_MSH_CFG_W0_MSH_INTF_C_CLK_TX_SEL);
	...
	if (clock == SDHCI_REFCLK_150M){	
		...
		regmap_read(regmap_base, offsetof(struct SysCtl_regs, SysClkPeriCfg), &val);
		val = val & (~SYS_CLK_PERI_CFG_W0_CFG_DIV_MSH_REF_CNT_MASK);
		val |= ((0x8 & SYS_CLK_PERI_CFG_W0_CFG_DIV_MSH_REF_CNT_MASK)) << 0;
		regmap_write(regmap_base, offsetof(struct SysCtl_regs, SysClkPeriCfg), val);
	}
	
	sdhci_set_clock(host, clock);
}
static int sdhci_ctc5236_prepare_tuning(struct sdhci_host *host, int CENTER_PH_CODE)
{
	int CLK_CTRL_R_value = 0x0;
...
static void sdhci_ctc5236_voltage_switch(struct sdhci_host *host)
{	
	u32 val = 0;	
	...
	if (support_vsel){
		...
		dev_info(mmc_dev(host->mmc), "change 1.8v I/O\n");
	}
}
...
static void ctc5236_phase_choose(struct platform_device *pdev)
{
	int val;

	if (of_property_read_bool(pdev->dev.of_node, "mmc-ddr-1_8v") || support_vsel) {
		/* Use our 90 shift phase */
		regmap_read(regmap_base, offsetof(struct SysCtl_regs, SysMshCfg), &val);
		val &= (~SYS_MSH_CFG_W0_MSH_INTF_C_CLK_TX_PHASE_SEL); 
		val |= SYS_MSH_CFG_W0_MSH_INTF_C_CLK_TX_SEL;
		regmap_write(regmap_base, offsetof(struct SysCtl_regs, SysMshCfg), val);
...
static int sdhci_ctc5236_probe(struct platform_device *pdev)
{
	...
	regmap_base = syscon_regmap_lookup_by_phandle(pdev->dev.of_node,"ctc,sysctrl");
	if (IS_ERR(regmap_base))
		return PTR_ERR(regmap_base);

	mmc_of_parse_voltage(pdev->dev.of_node, &host->ocr_mask);

	if (of_find_property(pdev->dev.of_node, "sd-support-uhs", NULL)){
		/* UHS-I needs switch 1.8v signal voltage */
		support_vsel = true;
	}
	
	ctc5236_phase_choose(pdev);
