...
static struct ctc_pin_bank *ctc_bank_num_to_bank(
					      ...
					      struct ctc_pinctrl *info)
{
     ...
     info->ctrl->pin_banks = devm_kzalloc(dev, sizeof(struct ctc_pin_bank)*CTC_PIN_BANK_NUM, GFP_KERNEL);
...
static inline const struct ctc_pin_group *ctc_pinctrl_name_to_group(
					const struct ctc_pinctrl *info,
					const char *name)
{
	int i;
...
{    
     ...
     if(!bank->pin_base){
          regmap_update_bits(info->regmap_base, 
               offsetof(struct SysCtl_regs, SysGpioMultiCtl), 3<<pin*2, mux<<pin*2);
     }else{
          regmap_update_bits(info->regmap_base, 
               offsetof(struct SysCtl_regs, SysGpioHsMultiCtl), 3<<pin*2, mux<<pin*2);
     }
     return 0;
}
					...
					struct ctc_pinctrl *info)
{
     ...
     pindesc = devm_kzalloc(&pdev->dev, sizeof(*pindesc) * info->ctrl->nr_pins, GFP_KERNEL);
     if (!pindesc) {
