...
static int uml_net_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    case SIOCDEVPRIVATE + 0xF:
        ...
        if (!(dev->flags & IFF_UP))
        {
            ifr->ifr_name[0] = 0; /* indicate the device is down */
            err = (*lp->change_config)(&lp->user, ifr);
            spin_unlock_irqrestore(&lp->lock, flags);

            goto out;
        }

        /*Modified by xgu for new linux kernel, change free_irq() to um_free_irq(), 2018-4-19*/
        um_free_irq(dev->irq, dev);
        err = (*lp->change_config)(&lp->user, ifr);
        if (err < 0)
        {
            um_request_irq(dev->irq, lp->fd, IRQ_READ, uml_net_interrupt,  IRQF_SHARED, dev->name, dev);
            spin_unlock_irqrestore(&lp->lock, flags);
            goto out;
        }
...
#ifdef CONFIG_CTC_PLATFORM
		  .change_config    = transport->user->change_config,
#endif
