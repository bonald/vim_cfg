...
static int umcast_read(int fd, struct sk_buff *skb, struct uml_net_private *lp)
{
    struct umcast_data *priv = (struct umcast_data *)lp->user;
    struct sockaddr_in from;
    struct net_device *dev;
    int ret;

    dev = skb->dev;
	ret = net_recvfrom2(fd, skb_mac_header(skb),
			    skb->dev->mtu + ETH_HEADER_OTHER, &from);

    if (ret <= 0)
        return ret;
    
    if(strcmp(dev->name, "eth99"))
    {
        if ((from.sin_addr.s_addr == priv->local_addr.sin_addr.s_addr)
            && (from.sin_port == priv->local_addr.sin_port))
        {
            return 0;
        }
    }
