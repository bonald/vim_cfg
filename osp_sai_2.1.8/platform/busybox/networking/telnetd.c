...
/* for bug 46991 added by liuyang 2018-5-9 */
#include <linux/ip.h>

/* Added by kcao 2017-05-02 */
static int bCtcSh = 0;
...
remove_iacs(struct tsession *ts, int *pnum_totty)
{
	...
	while (ptr < end) {
        ...
        /* Added by kcao 2017-05-02 */
        if ((ptr[1] == WILL || ptr[1] == WONT)  && ptr[2] == TELOPT_CTCSH)
        {
            bCtcSh = 1;
        }
...
make_new_session(
	...
	enum { sock = 0 };
#else
    char remote_host[0x40];
    char remote_port[0x40];
    char dest_host[0x40];
    char dest_port[0x40];
#endif
    ...
    /* Added by Alexander Liu */
    {
        ...
        slen = sizeof(addr);
        getpeername(sock, (struct sockaddr *)&addr, &slen);
        if (addr4->sin_family == AF_INET)
        {
            inet_ntop(AF_INET, &addr4->sin_addr, remote_host, sizeof(remote_host));
            snprintf(remote_port, sizeof(remote_port), "%d", ntohs(addr4->sin_port));
        }
        else if (IN6_IS_ADDR_V4COMPAT(&addr6->sin6_addr) || IN6_IS_ADDR_V4MAPPED(&addr6->sin6_addr))
        {
            ...
            snprintf(remote_port, sizeof(remote_port), "%d", ntohs(addr6->sin6_port));
        }
        else
        {
        ...
        val = 2;
        setsockopt(sock, SOL_TCP, TCP_KEEPINTVL, &val, sizeof(int));

        val = 5;
        setsockopt(sock, SOL_TCP, TCP_KEEPCNT, &val, sizeof(int));

        val = 1;
        setsockopt(sock, SOL_TCP, TCP_KEEPIDLE, &val, sizeof(int));
        /* for bug 46991 added by liuyang 2018-5-9 */		
        val = IPTOS_PREC_INTERNETCONTROL;
        setsockopt(sock, IPPROTO_IP, IP_TOS, &val, sizeof(int));
    }
	{
		struct sockaddr_storage addr;
        struct sockaddr_in *addr4 = (struct sockaddr_in *)&addr;
        ...
        if (addr4->sin_family == AF_INET)
        {
            ...
            snprintf(dest_port, sizeof(dest_port), "%d", ntohs(addr4->sin_port));
        }
        else if (IN6_IS_ADDR_V4COMPAT(&addr6->sin6_addr) || IN6_IS_ADDR_V4MAPPED(&addr6->sin6_addr))
        {
            inet_ntop(AF_INET, &addr6->sin6_addr.s6_addr32[3], dest_host, sizeof(dest_host));
            snprintf(dest_port, sizeof(dest_port), "%d", ntohs(addr6->sin6_port));
        }
        else
        {
            inet_ntop(AF_INET6, &addr6->sin6_addr, dest_host, sizeof(dest_host));
            snprintf(dest_port, sizeof(dest_port), "%d", ntohs(addr6->sin6_port));
        }
	}
	ts->sockfd_read = sock;
    ...
    {
        char myenv[256];
        snprintf(myenv, 256, "%s %s %s %s", remote_host, remote_port, dest_host, dest_port);
        xsetenv("TELNET_CONNECTION", myenv);
    }
#endif
    ...
    if (bCtcSh)
    {
        BB_EXECVP("/bin/sh", (char **)login_argv);
    }
    else
    {
...
#define IS_INETD (opt & OPT_INETD)
	...
	if (IS_INETD) {
		...
		if (!(opt & OPT_WAIT)) {
            ...
            /* for bug 46991 added by liuyang 2018-5-9 */
            setsockopt(master_fd, IPPROTO_IP, IP_TOS, &tos, sizeof(tos));
			xlisten(master_fd, 1);
		}
