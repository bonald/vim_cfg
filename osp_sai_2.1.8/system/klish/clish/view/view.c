...
static void clish_view_fini(clish_view_t * this)
{
	...
	while ((iter = lub_list__get_head(this->ptcmds))) {
		lub_list_del(this->ptcmds, iter);
		lub_list_node_free(iter);
	}
	lub_list_free(this->ptcmds);
    this->ptcmds = NULL;
	...
	const char *name, const char *help)
{
    ...
    if (cmd && 'G' == *name) {
        clish_shell_t *pshell = get_global_shell();
        clish_ptype_t *ptype = clish_shell_find_ptype(pshell, name);

        if (ptype) {    
            lub_list_add(this->ptcmds, cmd);
		} else {
		    fprintf(stderr, "Warning: can't find ptype %s at view %s\r\n",
                name, clish_view__get_name(this));
		}
    }
	return cmd;
}
	...
	const char *line, bool_t inherit)
{
	...
	for (i = 0; i < lub_argv__get_count(argv); i++) {
	    ...
	    if (i > 0 && !strcasecmp(clish_command__get_name(cmd), "GKEYLINE")) {
	        if (result != cmd) {
                /* GKEYLINE can only match from the first token */
                break;
	        }
	    }
		result = cmd;
	...
	const char *name, bool_t inherit)
{
    ...
    if (NULL == result && this->ptcmds) {
        lub_list_node_t *iter = NULL;
		for (iter = lub_list__get_head(this->ptcmds);
			    iter; iter = lub_list_node__get_next(iter)) {
			clish_command_t *ctmp = (clish_command_t *)lub_list_node__get_data(iter);
            clish_ptype_t *ptype = NULL;
            char *pret = NULL;
            clish_shell_t *pshell = get_global_shell();
            
            ptype = clish_shell_find_ptype(pshell, clish_command__get_name(ctmp));
            if (ptype && (pret = clish_ptype_validate(ptype, name))) {
                lub_string_free(pret);
                result = ctmp;
            }
		}      
    }
