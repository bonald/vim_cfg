...
static void clish_action_fini(clish_action_t *this)
{
	...
	if (NULL != this->postproc) {
        lub_string_free(this->postproc);    
	}
}
...
/*--------------------------------------------------------- */
void clish_action__set_postproc(clish_action_t *this, const char *postproc)
{
	if (this->postproc)
		lub_string_free(this->postproc);
	this->postproc = lub_string_dup(postproc);
}

/*--------------------------------------------------------- */
const char *clish_action__get_postproc(const clish_action_t *this)
{
