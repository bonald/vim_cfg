...
#define M_TYPE_XML_FILENAME "type.xml"
#ifdef _GLB_UML_SYSTEM_
const char *default_path = "/centec_switch/etc/cmd";
#else
const char *default_path = CTC_ETC_BASE_DIR"/etc/cmd";
#endif
...
void clish_shell_load_scheme(clish_shell_t *this, const char *xml_path)
{
	...
	char *dirname;
	char *saveptr = NULL;
    char szTypeFile[PATH_MAX];

    /* load ptype xml at first */
    snprintf(szTypeFile, PATH_MAX, "%s/%s", default_path, M_TYPE_XML_FILENAME);
#ifdef DEBUG
    fprintf(stderr, "Parse XML-file: %s\n", szTypeFile);
#endif
    (void)clish_shell_xml_read(this, szTypeFile);
	...
	for (dirname = strtok_r(buffer, ";", &saveptr);
		...
		for (entry = readdir(dir); entry; entry = readdir(dir)) {
			...
			if ((NULL != extension) &&
                ...
                if (strcasecmp(filename, szTypeFile)) {
#ifdef DEBUG
				    fprintf(stderr, "Parse XML-file: %s\n", filename);
#endif
				    /* load this file */
...
process_command(clish_shell_t * shell, clish_xmlnode_t * element, void *parent)
{
    ...
    {
        if(0 == strncmp("show", name, 4))
        {
           strcpy(tmp_access, "2");
           clish_command__set_access(cmd, tmp_access);
        }
        else
        {
           strcpy(tmp_access, "3");
	...
	/* lock field */
	if (lock && lub_string_nocasecmp(lock, "true") == 0)
...
process_action(clish_shell_t * shell, clish_xmlnode_t * element, void *parent)
{
	...
	char *postproc = clish_xmlnode_fetch_attr(element, "postproc");
	...
	if (postproc)
		clish_action__set_postproc(action, postproc);
	clish_xml_release(builtin);
	clish_xml_release(shebang);
	clish_xml_release(postproc);
}
