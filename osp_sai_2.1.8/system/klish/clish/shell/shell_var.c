...
/*--------------------------------------------------------- */
char *find_viewid_var(const char *name, clish_context_t *context)
{
    char *pszVar = NULL;
	...
	if (depth < 0)
		return NULL;
    do {
	    pszVar = find_var(name, &context->shell->pwdv[depth]->viewid, context);
        depth--;
    } while (NULL == pszVar && depth >= 1);
 ...
 */
char *expand_nextsegment(const char **string, const char *escape_chars,
...
char *clish_shell_expand_var(const char *name, clish_context_t *context)
{
    ...
    if (cmd) {
        pszCmdName = clish_command__get_name(cmd);
        if (!strcasecmp("GKEYLINE", pszCmdName)) {
            trl = clish_shell__get_tinyrl(this);
            tmp = tinyrl__get_last_buffer(trl);
            return lub_string_dup(tmp);
        }
    
        /* process do ... */
        if (!strncasecmp(pszCmdName, "do ", 3)) {
            pszCmdName += 3; 
        }
        if ('G' == *pszCmdName && !strcasecmp(name, pszCmdName)) {
            char *p;
            trl = clish_shell__get_tinyrl(this);
            tmp = tinyrl__get_last_buffer(trl);
            
            if (!strncasecmp(tmp, "do ", 3)) {
                /* skip do */
                tmp = tmp + 3;
            }
            ...
            p = strchr(tmp, ' ');
            if (NULL != p) {
                return lub_string_dupn(tmp, (p - tmp));
            }
            return lub_string_dup(tmp);
        }
    }
