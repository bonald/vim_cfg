	...
	const clish_command_t **ret_cmd, clish_pargv_t **pargv)
{
	...
	*pargv = clish_pargv_new();
    ...
    /* Added by Alexander for RSA key line command, 2014-11-11 16:30 */
	if (!strcasecmp(clish_command__get_name(cmd), "GKEYLINE")) {
	    argv = lub_argv_new(NULL, 0);
	    lub_argv_add(argv, line);
	} else {
	...
	unsigned *idx, clish_pargv_t *last, unsigned need_index)
{
	...
	int bLineMode = 0; /* Added by Alexander */
	...
	while (index < paramc) {
		...
		clish_ptype_t *mypt = NULL;
		char szLineParam[512];

		if (param) {
		    mypt = clish_param__get_ptype(param);
            if (mypt && !strcasecmp(clish_ptype__get_name(mypt), "KLINE")) {
                bLineMode = 1;
            }
		}
		...
		if (*idx < argc) {
		    if (!bLineMode) {
			    ...
			    int mylen = 0;
                arg = (const char *)szLineParam;
                for (i = *idx; i < argc; i++) {
                    snprintf(szLineParam + mylen, 512 - mylen, "%s ",
                             lub_argv__get_arg(argv, i));
                    mylen += strlen(lub_argv__get_arg(argv, i)) + 1;
                }
                szLineParam[strlen(szLineParam) - 1] = '\0';
			}
        }
