...
static bool_t remove_duplicate(tinyrl_history_t * this, const char *line)
{
	...
	for (i = this->length-1; i < this->length; i++) {
		tinyrl_history_entry_t *entry = this->entries[i];
		if (0 == strcmp(line, tinyrl_history_entry__get_line(entry))) {
    /* delete by kcao for bug 32811, duplicate entry not free */
#if 0
...
static void add_n_grow(tinyrl_history_t * this, const char *line)
{
	if (this->size == this->length) {
		/* increment the history memory by 8 entries each time we grow */
		unsigned new_size = this->size + 8;
		size_t nbytes;
    ...
    /* modified by kcao for bug 32811, duplicate entry will return */
    if (BOOL_TRUE == remove_duplicate(this, line)) {
        return;
    }
	append_entry(this, line);
}
...
{
    /* Added by kcao 2014-10-28 for bug 30431, filter empty line */
    if (0 == strlen(line) || !strcasecmp(line, ""))
    {
        return;
    }
...
/* Added by Alexander */
void tinyrl_history_reset_iter(const tinyrl_history_t * this,
					       void * iter)
{
    tinyrl_history_iterator_t *piter = (tinyrl_history_iterator_t *)iter;
    piter->history = this;
	piter->offset = this->length;
	return;
}
