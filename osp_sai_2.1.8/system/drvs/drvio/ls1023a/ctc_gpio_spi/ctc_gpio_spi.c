...
struct spi_rdwr_ioctl_data {
    uintptr_t msgs;   /* pointers to i2c_msgs */
};
...
u16 gpio_dir_offset;  
static volatile void __iomem *gpio_spi_virtual_addr;
...
static inline void setbits32(volatile void __iomem *addr, int bits)
{
    int val;
...
static inline void clrbits32(volatile void __iomem *addr, int bits)
{
    int val;
...
int ctc_gpio_spi_open(struct inode *inode, struct file *filp)
{
        ...
        ||((GLB_SERIES_E550==ctc_bd_info.board_series) && (GLB_BOARD_E550_48M4Y2C==ctc_bd_info.board_type)))
    {
        ...
        gpio_spi_cs_bit = 3;
        gpio_spi_clk_bit = 2;
        gpio_spi_data_out_bit = 4;
        gpio_spi_data_in_bit = 5;
        gpio_spi_valid = 1;
    }
    else if((GLB_SERIES_E530==ctc_bd_info.board_series)&& (GLB_BOARD_E530_24X2C_D==ctc_bd_info.board_type))
    {
        gpio_spi_virtual_addr = ctc_bd_info.gpio_g1_logic_addr;
        gpio_spi_cs_bit = 3;//28;
        gpio_spi_clk_bit = 2;//29;
        gpio_spi_data_out_bit = 4;//27;
        gpio_spi_data_in_bit = 5;//26;
        gpio_spi_valid = 1;
    }    
    else
    {
