...
{
    if((GLB_SERIES_E530==ctc_bd_info.board_series)&& (GLB_BOARD_E530_24X2C_D==ctc_bd_info.board_type))
    {
        gpio_spi_cs_bit = 496; //todo
        gpio_spi_clk_bit = 497; //todo
        gpio_spi_data_out_bit = 498; //todo
        gpio_spi_data_in_bit = 499; //todo
        gpio_spi_valid = 1;
    }    
    else
    {
    ...
    gpio_direction_output(gpio_spi_cs_bit, 0);
    gpio_direction_output(gpio_spi_clk_bit, 0);
    gpio_direction_output(gpio_spi_data_out_bit, 0);
    gpio_direction_input(gpio_spi_data_in_bit);   
