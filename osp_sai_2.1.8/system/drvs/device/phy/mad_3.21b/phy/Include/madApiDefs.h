...
#define IS_MARVELL_OUI(_reg2,_reg3)        \
	(((_reg2) == 0x0141 || (_reg2) == 0x002b) && (((_reg3)& 0xFC00) == 0x0C00 || ((_reg3)& 0xFa00) == 0x0a00))
...
#define MAD_AN_MOD_AUTO_SPEED_DUPLEX_FULL   (MAD_AUTO_AD_1000FDX         |   \
                                         MAD_AUTO_AD_100FDX     |    \
                                         MAD_AUTO_AD_10FDX  )
#define MAD_AN_MOD_AUTO_SPEED_DUPLEX_HALF   ( MAD_AUTO_AD_1000HDX     |    \
                                         MAD_AUTO_AD_100HDX | \
                                         MAD_AUTO_AD_10HDX)
...
#define MAD_USGMII_AUTO_AD_Link                MAD_BIT_15
#define MAD_USGMII_AUTO_AD_Duplex			   MAD_BIT_11
#define MAD_USGMII_AUTO_AD_High_Speed          MAD_BIT_10  /* 1000X Full Duplex */
#define MAD_USGMII_AUTO_AD_Low_Speed           MAD_BIT_9  /* 1000X Half Duplex */
#define MAD_USGMII_AUTO_AD_MASK_REG4  (MAD_USGMII_AUTO_AD_Link |  \
	MAD_USGMII_AUTO_AD_Duplex |    \
	MAD_USGMII_AUTO_AD_High_Speed | \
	MAD_USGMII_AUTO_AD_Low_Speed)
...
typedef unsigned short MAD_PG_BURST;

/* Enable CRC Checker */
typedef enum
{
	MAD_PG_CHECKER_DISABLE = 0,    /* Disable CRC checker */
	MAD_PG_CHECKER_ENABLE        /* Enable CRC checker */
}MAD_PG_CHECKER;

/* Trigger */
typedef enum
{
	MAD_PG_TRIGGER_TRANSMIITTING = 0,    /* Transmitting data */
	MAD_PG_TRIGGRT_DONE        /* Transmit done */
} MAD_PG_TRIGGER;

/* Self_clear */
typedef enum
{
	MAD_PG_CLEAR_SELFCLEAR = 0,    /* Transmitting data */
	MAD_PG_CLEAR_STAY        /* Transmit done */
} MAD_PG_CLEAR;
...
/* Enable type for 88E1780 */
typedef enum
{
	MAD_PG_DISABLE_1780 = 0,        /* Normal operation */
	MAD_PG_EN_COPPER_1780 = 2,        /* Enable Packet Check to copper */
	MAD_PG_EN_USGMII_1780 = 6        /* Enable Packet Check to USGMII */
} MAD_1780_PG_EN_TYPE;

/* Select Cisco mode for 88E1780 */
typedef enum
{
	MAD_USGMII_USGMII_SPEC = 0,        /* Tx_Config format as per USGMII specification Table 4 */
	MAD_USGMII_SGMII_SPEC = 1,        /* In USGMII mode Speed shifted to 11:10, instead of 11:9.(Valid in USGMII mode only) */
} MAD_USGMII_CISCO_MODE;

/* Packet Type value for Ingress PCH headers */
typedef enum
{
	MAD_USGMII_WITH_PCH = 0,			/* Ethernet Packet with PCH */
	MAD_USGMII_WITHOUT_PCH = 1,			/* Ethernet Packet without PCH */
	MAD_USGMII_IDLE_PACKET = 2,			/* Idle Packet, contains status data for a port, no packet data */
} MAD_USGMII_PKT_TYPE;

/* Extention Field Type value for Ingress PCH headers */
typedef enum
{
	MAD_USGMII_IGNORE_FIELD = 0,        /* Ignore Extension Field */
	MAD_USGMII_EXT_FIELD = 1,			/* Extension Field contains 8-bit Reserved + 32-bit Timestamp */
} MAD_USGMII_EXT_FIELD_TYPE;
...
/* Structure for 1780 packet generator */
typedef struct
{
	MAD_PG_BURST    burst;
	MAD_PG_TRIGGER  trigger;
	MAD_PG_CLEAR    clear;
	MAD_PG_PAYLOAD  payload;
	MAD_PG_LENGTH   length;
	MAD_PG_TX       tx;
	MAD_1780_PG_EN_TYPE  en_type;
} MAD_PG_1780;
...
typedef struct
{
	MAD_PG_BURST    burst;
	MAD_PG_TRIGGER  trigger;
	MAD_PG_CHECKER  check;
	MAD_PG_PAYLOAD  payload;
	MAD_PG_LENGTH   length;
	MAD_PG_TX       tx;
} MAD_COPPER_PG_1780;

/* Structure for 1780 packet count */
typedef struct
{
	MAD_U8  packet_count;
	MAD_U8  crc_count;
...
/* Definition for Temprature Sensor Enable */
typedef enum
{
	MAD_TEMP_SENSOR_SAMPLE = 0,
	MAD_TEMP_SENSOR_RATE,
	MAD_TEMP_SENSOR_6265,
	MAD_TEMP_SENSOR_DISABLE
} MAD_TEMP_TYPE;

/* Definition for Temprature Sensor Number of Sample to Average */
typedef enum
{
	MAD_TEMP_AVERAGE_9 = 0,
	MAD_TEMP_AVERAGE_11,
	MAD_TEMP_AVERAGE_13,
	MAD_TEMP_AVERAGE_15
}MAD_TEMP_SAMPLE_AVERAGE;

/* Definition for Temprature Sensor Sampling Rate */
typedef enum
{
	MAD_TEMP_INSTANT_1 = 5,
	MAD_TEMP_INSTANT_5,
	MAD_TEMP_INSTANT_10
}MAD_TEMP_SAMPLE_INSTANT;
...
typedef struct
{
	MAD_TEMP_TYPE tempType;   /*Page6,Reg27,bit15:14 */
	MAD_BOOL      tempSense;  /*This bit is valid only when tempType == 2'b10 */
} MAD_TEMP_ENABLE;

typedef struct
{
	MAD_TEMP_SAMPLE_AVERAGE      avgSample;   /*Page6,Reg27,bit12:11 */
	MAD_TEMP_SAMPLE_INSTANT      insSample;    /*Page6,Reg27,bit10:8 */
} MAD_TEMP_SAMPLE;

typedef struct
{
	MAD_BOOL      invertinp;     /*Page30, Reg26, bit13 */
	MAD_BOOL      invertoutp;    /*Page30, Reg26, bit12 */
} MAD_INVERT_POLARITY;
...
#define MAD_PHY_MODE_USGMII            0x4000    /* For 1780 */
#define MAD_PHY_MODE_USGMII_MEDIA      0x8000    /* For 1780 */
...
#define MAD_PHY_USGMII_PRBS_GEN         0x100000
...
#define MAD_PHY_EX_STATUS_TYPE5    5    /* 88E1780*/
...
#define MAD_PHY_PKTGEN_TYPE5    5    /* Uses Register 16 (1780)*/
...
#define MAD_RESET_MODE_PORT_USGMII            0x0007
#define MAD_RESET_MODE_ALL_USGMII			  0x0008
#define MAD_RESET_MODE_PORT_COMMON            0x0009
...
#define MAD_MEDIA_QSGMII_2_100FX            0x0003  /*154x*/
...
#define MAD_SYNC_CLK_MODE_PORT_USGMII            0x0004
...
typedef struct
{
	MAD_U16   AData;   /* The SNR data of Channel A */
	MAD_U16   BData;   /* The SNR data of Channel B */
	MAD_U16   CData;   /* The SNR data of Channel C */
	MAD_U16   DData;   /* The SNR data of Channel D */
} MAD_SNR_CHANNEL_DATA;
 ...
 *   ptpRegsSem      - Semaphore for PTP register access, ptpreadplus etc.
 *   ptpTodSem       - Semaphore for PTP Tod indirect access
 *   ptpModeSem      - Semaphore for PTP Mode indirect access
