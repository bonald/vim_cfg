...
)
{
	...
	else if ((MAD_DEV_HWMODE(dev, hwPort, MAD_PHY_MODE_USGMII)) &&
		(!MAD_DEV_IS_IN_SWITCH(dev)))
	{
		/* Get the required bit in Copper Status Register 1 */
		if ((retVal = madHwGetPagedPhyRegField(
...
*       en      - MAD_TRUE to enable QSGMII or MAD_FALSE to disable QSGMII
*
* OUTPUTS:
*       None
*
...
)
{
	...
	{
		if ((retVal = E1680_SysSetQSGMIIPwDown(dev, hwPort, en)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
...
)
{
	...
	if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
	{
		if ((retVal = E1680_SysGetQSGMIIPwDown(dev, hwPort, en)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
...
*									  MAD_RESET_MODE_PORT_USGMII: 0_30.15
*									  MAD_RESET_MODE_ALL_USGMII: 26_30.15
*									  MAD_RESET_MODE_PORT_COMMON: 20_18.15)
*
* RETURNS:
*       MAD_OK   - on success
*       MAD_[ERROR_STATUS] - on error
...
)
{
	...
	{
		if ((retVal = E1780_SysModSoftReset(dev, port, mode)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
	...
	if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
	{
		if ((retVal = E1680_SysModSoftReset(dev, port, mode)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
	...
	if (dev->deviceId == MAD_88E1548 || dev->deviceId == MAD_88E1545)
	{
		if ((retVal = E1548P_SysModSoftReset(dev, port, mode)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
...
* INPUTS:
*       portNum - Port number to be configured.
*       en      - MAD_TRUE to enable MAC or MAD_FALSE to disable MAC
*
* OUTPUTS:
*       None
*
...
)
{
	...
	if(MAD_DEV_CAPABILITY(dev, MAD_PHY_FULL_PAGE_ADDR))
	{
		if (dev->deviceId == MAD_88E1780)
		{
			if ((retVal = E1780_SysSetMacPowerDown(dev, hwPort, data)) != MAD_OK)
			{
				MAD_DBG_ERROR(("Writing to phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
			}
		...
		if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
		{
			if ((retVal = E1680_SysSetMacPowerDown(dev, hwPort, data)) != MAD_OK)
			{
				MAD_DBG_ERROR(("Writing to phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
			}
...
)
{
	...
	if(MAD_DEV_CAPABILITY(dev, MAD_PHY_FULL_PAGE_ADDR))
	{
		if (dev->deviceId == MAD_88E1780)
		{
			if ((retVal = E1780_SysGetMacPowerDown(dev, hwPort, en)) != MAD_OK)
			{
				MAD_DBG_ERROR(("Reading to phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
			}
		...
		if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
		{
			if ((retVal = E1680_SysGetMacPowerDown(dev, hwPort, en)) != MAD_OK)
			{
				MAD_DBG_ERROR(("Reading to phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
			}
...
)
{
	...
	if(MAD_DEV_HWMODE(dev, hwPort, MAD_PHY_MODE_COPPER))
	{
		...
		else
		{
			if (dev->deviceId == MAD_88E1780)
			{
				/* Set the Energe Detect mode */
				if ((retVal = E1780_SysSetEnergyDetect(dev, hwPort, mode)) != MAD_OK)
				{
					MAD_DBG_ERROR(("Writing phy reg failed.\n"));
					return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
				}
...
)
{
	...
	if(MAD_DEV_HWMODE(dev, hwPort, MAD_PHY_MODE_COPPER))
	{
		...
		else
		{
			if (dev->deviceId == MAD_88E1780)
			{
				/* Get the Energe Detect mode */
				if ((retVal = E1780_SysGetEnergyDetect(dev, hwPort, mode)) != MAD_OK)
				{
					MAD_DBG_ERROR(("Reading phy reg failed.\n"));
					return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
				}
...
)
{
	...
	if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
	{
		if ((retVal = E1680_SysSetLEDCtrl(dev, hwPort, ledData)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Paged Phy Register write failed.\n"));
			return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
		}
	...
	reg = MAD_REG_LED_TIMER_CTRL;
		data = ((ledData->ledForceInt<<15)&0x8000) | ((ledData->ledPulseDuration<<12)&0x7000)
			| ((ledData->ledIntPolarity << 11) & 0x800) | ((ledData->ledBlinkRate << 8) & 0x700)
			| ((ledData->ledSppedOffPulse_perio << 2) & 0xc) | ((ledData->ledSppedOnPulse_perio) & 0x3);
	if((retVal=madHwWritePagedPhyReg(dev,(MAD_U8)hwPort,(MAD_U16)page,
		reg,(MAD_U16)data)) != MAD_OK)
	{
		MAD_DBG_ERROR(("Paged Phy Register write failed.\n"));
...
)
{
	...
	if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
	{
		if ((retVal = E1680_SysGetLEDCtrl(dev,hwPort,ledData)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Paged Phy Register read failed.\n"));
			return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
		}
	...
	ledData->ledIntPolarity = (data >> 11) & 0x1;
...
)
{
	...
	{
		/*      MAD_DBG_ERROR(("Call Masec EEE LPI functions.\n")); */
		return msec_SetEEELPIEEEBuffThresh(dev, port, eee_buf_off_timeout, eee_buf_off_thresh);
	}
...
)
{
	...
	{
		/*      MAD_DBG_ERROR(("Call Masec EEE LPI functions.\n")); */
		return msec_GetEEELPIEEEBuffThresh(dev, port, eee_buf_off_timeout, eee_buf_off_thresh);
	}
	...
	if(MAD_DEV_CAPABILITY(dev, MAD_PHY_FULL_PAGE_ADDR))
	{
		...
		!= MAD_OK)
		{
			...
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
              ...
              /* Soft Reset */
              if((retVal = mdSysModSoftReset(dev, hwPort, MAD_RESET_MODE_PORT_COMMON)) != MAD_OK)
              {
                  MAD_DBG_ERROR(("Soft Reset failed.\n"));
                  return (retVal | MAD_API_FAIL_SW_RESET);
              }
