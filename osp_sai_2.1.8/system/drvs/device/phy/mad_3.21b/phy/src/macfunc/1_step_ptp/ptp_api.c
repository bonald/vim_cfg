...
*
*******************************************************************************/
MAD_STATUS  ptp_lut_set_udp (int port) {
...
*
*******************************************************************************/
MAD_STATUS  ptp_lut_clear_udp (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS ptp_1step_V2_L2_init(int port, PTP_1STEP_CFG *ptp_cfg, int numIngLut, int numEgLut)
{
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS ptp_1step_V2_IP4_UDP_init(int port, PTP_1STEP_CFG *ptp_cfg, int numIngLut, int numEgLut)
{
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS ptp_1step_V2_IP6_UDP_init(int port, PTP_1STEP_CFG *ptp_cfg, int numIngLut, int numEgLut)
{
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS ptp_1step_config(int port, PTP_1STEP_CFG *ptp_cfg, int numIngLut, int numEgLut)
{
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS new_ptp_init (void* dev, int port) {
  MAD_STATUS   retVal = MAD_OK;
  /*unsigned int ver;*/
/*  int i; */
  PTP_DBG_DEMO((">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> init\n"));
...
/*  retVal = set_ptp_latadj(port, 0x1878);  For 100M speed*/
  ...
  else if (((MAD_DEV *)dev)->deviceId == MAD_88E1680M)
  {
	  retVal = ptp_macsec_workaround_2();
	  if (retVal != MAD_OK) return retVal;
  }
...
*
*******************************************************************************/
MAD_STATUS new_ntp_init (void* dev, int port, int udp_src_port_2, int udp_dst_port_2) {
  MAD_STATUS   retVal = MAD_OK;
  /*unsigned int ver;*/
  PTP_DBG_DEMO((">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> init\n"));
  retVal = ptp_dev_init(dev);
...
* RETURNS:
*       MAD_OK          - on success
*       MAD_FAIL        - on no ts or error
*
* COMMENTS:
*
*******************************************************************************/
MAD_STATUS ptp_get_ts_tsq 
(
  IN  int portnum, 
  IN  PTP_Blk path, 
...
*/
      ...
      if(retVal!=MAD_OK) return MAD_FAIL;
      retVal = ptp_enable_lut_action(portnum, idx, action, path,MAD_FALSE); 
      if(retVal!=MAD_OK) return MAD_FAIL;
      if(queue == PTP_1STEP_QUEUE_HIGH)
      ...
      if(retVal!=MAD_OK) return MAD_FAIL;
      retVal = ptp_enable_lut_action(portnum, idx, action, path,MAD_TRUE); 
      if(retVal!=MAD_OK) return MAD_FAIL;
    }
#endif
    return MAD_FAIL;
...
MAD_STATUS ptp_print_tai_tod_cfg_gen 
(
   IN int port
)
{
...
/* Set TAI TOD cfg gen */
/* clk_mode: 1: clock generation, 2: clock reception, 3: clock reception adv */
MAD_STATUS ptp_set_tai_clk_mode
(
   IN int port, 
   IN int clk_mode
) 
...
*
*******************************************************************************/
MAD_STATUS ptp_set_tai_synce_clk_sel
(
   IN int port, 
   IN int refPort
) 
...
MAD_STATUS ptp_disable_tai_clk_gen 
(
  IN int port
)
{
...
*******************************************************************************/
/* Set TAI to generate external clock signal */
MAD_STATUS ptp_set_tai_clk_gen 
(
  IN int port
)
{
...
*******************************************************************************/
/* Set TAI to update TOD based on external clock signal */
MAD_STATUS ptp_set_tai_clk_rec 
(
  IN int port
)
{
...
*******************************************************************************/
/* Set TAI to update TOD based and auto adjust frequency based on ext clock signal */
MAD_STATUS ptp_set_tai_clk_rec_adv 
(
  IN int port
)
{
...
*******************************************************************************/
/* Set TOD func trig to copy LOAD into NEXT RT */
MAD_STATUS ptp_tai_trig_load_timer 
(
  IN int port
)
{
...
/* minimal_drift: min drift in ns that triggers drift adj */
/* drift_thr: min drift in ns that causes interrupt to be invoked to CPU */
MAD_STATUS ptp_set_drift_thr_cfg
(
  IN int port,
...
/*  set addr [expr $base_addr(TAI_CFG)+ [lsearch $ptp_tai_regs_list drift_thr_cfg] ] */
  ...
  retVal = ptp_writecheck_reg(port, addr, ((minimal_drift<<24)&0xff000000)|(drift_thr&0xffffff));
  return retVal;
}
...
*******************************************************************************/
/* set pclk period */
MAD_STATUS ptp_set_pclk_period 
(
  IN int port,
  IN int period
)
...
*
*******************************************************************************/
MAD_STATUS ptp_get_tai_pulse_delay 
(
  IN int port,
...
*
* OUTPUTS:
*      kpTodCfg	- keep original TOD configuration
*
* RETURNS:
*       MAD_OK          - on success
*       MAD_FAIL        - on error
...
*
*******************************************************************************/
MAD_STATUS ptp_tai_get_timer_operation 
(
  IN int port,
...
*
*******************************************************************************/
MAD_STATUS ptp_tai_set_timer_operation 
(
  IN int port,
...
*
*******************************************************************************/
MAD_STATUS ptp_tai_set_pulse_width 
(
  IN int port,
  IN int width_ns
)
...
*
*******************************************************************************/
MAD_STATUS ptp_tai_get_pulse_width 
(
  IN int port,
  OUT int *width_ns
)
...
*
*******************************************************************************/
MAD_STATUS ptp_tai_set_trig_gen_en 
(
  IN int port,
  IN int bitval
)
...
*******************************************************************************/
/* get pulse generation setup (master) */
MAD_STATUS ptp_get_pulse_gen (int port, int*width_ns, unsigned int *trigsec, unsigned int *trignsec)
{
  MAD_STATUS   retVal = MAD_OK;
...
*******************************************************************************/
/* pulse generation setup (master) */
MAD_STATUS ptp_go_pulse_gen (int port, int width_ns, int trigsec, int trignsec)
{
  MAD_STATUS   retVal = MAD_OK;
...
*******************************************************************************/
/* pulse reception setup (slave) */
MAD_STATUS ptp_go_pulse_rec (int port, int delay, PtpTimeStruct *nextTime, PtpTimeStruct * ldTime) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int kpTodCfg;
  /* disable tai */
  retVal = ptp_disable_tai(port, &kpTodCfg);
...
*
*******************************************************************************/
MAD_STATUS get_ptp_intr_msk ( int portnum, PTP_Blk path, unsigned int *mask)
{
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS set_ptp_intr_msk ( int portnum, PTP_Blk path, unsigned int mask)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short reg_addr;
  PTP_TYPE base_addr_id;
...
*
*******************************************************************************/
MAD_STATUS disable_ptp_watchdog_timer ( int portnum, PTP_Blk path)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short reg_addr;
  PTP_TYPE base_addr_id;
...
*
*******************************************************************************/
MAD_STATUS get_ptp_interrupts (int portnum, unsigned int *igrInt, unsigned int *egrInt)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short reg_addr;
  PTP_TYPE base_addr_id;
...
*
*******************************************************************************/
MAD_STATUS clear_ptp_interrupts ( int portnum)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short reg_addr;
  PTP_TYPE base_addr_id;
...
*
*******************************************************************************/
MAD_STATUS set_tai_intr_msk (int port, unsigned int mask)
{
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS get_tai_intr_msk (int port, unsigned int *mask)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short reg_addr = TAI_INTR_MASK;
...
*
*******************************************************************************/
MAD_STATUS ptp_global_intr_en ( int portnum, int en)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short reg_addr;
  PTP_TYPE base_addr_id;
  ...
  if(en)
    data |= 2;
  retVal = ptp_write_reg(portnum, reg_addr, data);
  if(retVal!=MAD_OK) return retVal;
...
*
*******************************************************************************/
MAD_STATUS set_top_intr_msk (int port, unsigned int mask)
{
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS get_top_intr_msk (int port, unsigned int *mask)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short reg_addr = MSEC_PTP_STAT_GLOBAL; /*0x210a */
  unsigned int data;
...
*
*******************************************************************************/
MAD_STATUS msec_get_intr_src (int port, unsigned *intSrc)
{
  MAD_STATUS   retVal = MAD_OK;
...
*
*******************************************************************************/
MAD_STATUS prgm_ptp_lut_ent_domain_num(int port, int idx, int domain_num, PTP_1STEP_LUT_ACT actions, PTP_Blk path)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
  ...
  lut_key = lut_key_adjust( K_FlagPTPv2, 1);
  lut_vld = lut_vld_adjust(K_FlagPTPv2);
  lut_key_top = (unsigned long)lut_key_adjust(K_DomainNumber, domain_num);
  lut_vld_top = (unsigned long)lut_vld_adjust(K_DomainNumber);
  lut_act = prgm_ptp_lut_acts(actions);
...
*
*******************************************************************************/
MAD_STATUS prgm_ptp_lut_ent_transspec(int port, int idx, int transspec, PTP_1STEP_LUT_ACT actions, PTP_Blk path)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_ipv4(int port, int idx, PTP_1STEP_LUT_ACT actions, PTP_Blk path) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_ntp(int port, int idx, PTP_1STEP_LUT_ACT actions, PTP_Blk path) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_ntp_list(int port, int idx, PTP_1STEP_LUT_ACT *actions, PTP_Blk path) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_ptpv2(int port, int idx, PTP_1STEP_LUT_ACT actions, PTP_Blk path) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_ptpv1(int port, int idx, PTP_1STEP_LUT_ACT actions, PTP_Blk path) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
  unsigned long lut_key_top=0;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_1731(int port, int idx, PTP_1STEP_LUT_ACT actions, PTP_Blk path) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_1731_opcode(int port, int idx, int opcode, PTP_1STEP_LUT_ACT actions, PTP_Blk path) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_stag(int port, int idx, PTP_1STEP_LUT_ACT actions, PTP_Blk path) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_ent_always0(int port, int idx, PTP_1STEP_LUT_ACT actions, PTP_Blk path) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
