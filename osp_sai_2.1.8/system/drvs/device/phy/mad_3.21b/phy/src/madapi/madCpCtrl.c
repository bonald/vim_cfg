...
)
{
  ...
  else
  {
    ...
    if(MAD_DEV_CAPABILITY(dev, MAD_PHY_FULL_PAGE_ADDR))
    {
	  ...
	  else if ((dev->phyInfo.hwMode[port] & MAD_PHY_MODE_USGMII) &&
		  (!MAD_DEV_IS_IN_SWITCH(dev)))
	  {
		  if ((retVal = madHwSetPagedPhyRegField(
			  dev, hwPort, MAD_PAGE30, 0, 12, 1, macData)) != MAD_OK)
		  {
			  MAD_DBG_ERROR(("Writing to paged phy reg failed.\n"));
			  return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		  }
		  /* Soft Reset */
		  if ((retVal = mdSysModSoftReset(dev, hwPort, MAD_RESET_MODE_PORT_USGMII)) != MAD_OK)
		  {
			  MAD_DBG_ERROR(("Soft Reset failed.\n"));
			  return (retVal | MAD_API_FAIL_SW_RESET);
		  }
...
)
{
  if(autoNegoEn==MAD_TRUE)  /* Auto nego enabled */
  {
    ...
    else if (speedMode==MAD_SPEED_AUTO) /* it is auto speed */
    {
         switch (duplexMode)
        {
...
* COMMENTS:
*       Page 0, Reg 0, Bit 15 : Soft reset
*       Page 0, Reg 26, Bit 8 : Copper Control 3 (DTE Detect)
*       or
...
*
* COMMENTS:
*       Page 0, Reg 26, Bit 11:10 : Copper Control 3 (Gigabit link down delay) (88E1680L, 88E1680)
*
*******************************************************************************/
...
*
* COMMENTS:
*       Page 0, Reg 26, Bit 9 : Copper Control 3 (Gigabit link down delay enable) (88E1680L, 88E1680).
*
*******************************************************************************/
