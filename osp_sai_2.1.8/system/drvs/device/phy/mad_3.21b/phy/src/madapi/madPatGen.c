...
)
{
  ...
  {
	  if ((retVal = E1780_PatCtrlSetPRBS(dev, hwPort, genCfg)) != MAD_OK)
	  {
  ...
  {
	  if ((retVal = E1680_PatCtrlSetPRBS(dev, hwPort, genCfg)) != MAD_OK)
	  {
...
)
{
  ...
  if (dev->deviceId == MAD_88E1780)
  {
	  if ((retVal = E1780_PatCtrlGetPRBS(dev, hwPort, genCfg)) != MAD_OK)
	  {
		  MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n", MAD_PAGE30, MAD_REG_USGMII_PRBS_CTRL));
		  return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
	  }
  ...
  if((retVal = madHwReadPagedPhyReg(dev,hwPort,MAD_PAGE4,MAD_REG_QSGMII_PRBS_ERR_COUNT_LSB,&data))
      != MAD_OK)
  {
    MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",MAD_PAGE4,MAD_REG_QSGMII_PRBS_ERR_COUNT_LSB));
    return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
  }
  ...
  if((retVal = madHwReadPagedPhyReg(dev,hwPort,MAD_PAGE4,MAD_REG_QSGMII_PRBS_ERR_COUNT_MSB,&data))
      != MAD_OK)
  {
    MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",MAD_PAGE4,MAD_REG_QSGMII_PRBS_ERR_COUNT_MSB));
    return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
  }
...
)
{
  ...
  if (dev->deviceId == MAD_88E1780)
  {
	  if ((retVal = E1780_ResetPRBSCounters(dev, hwPort)) != MAD_OK)
	  {
		  MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n", MAD_PAGE30, MAD_REG_USGMII_PRBS_CTRL));
		  return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
	  }
...
)
{
  ...
  if (dev->subDevId == MAD_SUB_88E1510)
  {
	  MAD_DBG_ERROR(("The phy not support SGMII\n"));
...
)
{
  ...
  if((retVal = madHwReadPagedPhyReg(dev,hwPort,MAD_PAGE1,MAD_REG_FIBER_PRBS_ERR_COUNT_LSB,&data))
      != MAD_OK)
  {
    MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",MAD_PAGE1,MAD_REG_FIBER_PRBS_ERR_COUNT_LSB));
    return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
  }
  genCfg->errCounter = data;
  if((retVal = madHwReadPagedPhyReg(dev,hwPort,MAD_PAGE1,MAD_REG_FIBER_PRBS_ERR_COUNT_MSB,&data))
      != MAD_OK)
  {
    MAD_DBG_ERROR(("Writing to paged phy reg (%i,%i) failed.\n",MAD_PAGE1,MAD_REG_FIBER_PRBS_ERR_COUNT_MSB));
    return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
  }
  genCfg->errCounter &= 0x0000ffff;
  genCfg->errCounter |= (data<<16);
