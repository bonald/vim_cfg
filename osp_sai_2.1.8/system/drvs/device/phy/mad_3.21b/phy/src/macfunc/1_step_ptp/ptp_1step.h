...
/* functions */
MAD_STATUS ptp_dev_init(void * dev);
PTP_TYPE get_ptp_base_addr_id ( int portnum, PTP_Blk path );

MAD_STATUS clear_bits (int port, unsigned short addr, unsigned int data);
...
MAD_STATUS ptp_write_reg (int port, unsigned short addr, unsigned int data);

MAD_STATUS ptp_writecheck_reg(int port, unsigned short addr, unsigned int data);

void ptp_1step_print_reg (int port, unsigned short addr);
/* Set bits by ORing new value with old value */
MAD_STATUS set_bits (int port, unsigned short addr, unsigned int data);
...
MAD_STATUS prgm_lut_entry (int port, unsigned long long lut_key, unsigned long long lut_valid, unsigned long lut_key_upper, unsigned long lut_valid_upper, unsigned long long lut_action,  short lut_index, int path );
/* Read lut entry */
MAD_STATUS read_lut_entry (int port, unsigned long long *lut_key, unsigned long long *lut_valid, unsigned long *lut_key_upper, unsigned long *lut_valid_upper, unsigned long long *lut_action, short lut_index, int path );
...
MAD_STATUS ptp_lut_update (int port, unsigned long long delete_lut_key , 
...
MAD_STATUS set_ntp_tod_mode (int port, unsigned int ntp_mode);

unsigned long long  prgm_ptp_lut_acts (PTP_1STEP_LUT_ACT action);
MAD_STATUS prgm_ptp_lut_ent_msgType(int port, int idx, PTPv2_message_type msgId, PTP_1STEP_LUT_ACT actions, PTP_Blk path);
/* */
