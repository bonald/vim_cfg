...
)
{
    ...
    if (en)
    {
        ...
        /* Enable TX_TCLK */
		/* For 88E1680L use page6, reg26, bit15 */
		/* Reg 27_4.8 must be set to '0' when TX_TCLK is set to '1'*/
		if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
		{
			if ((retVal = E1680_Enable_TX_TCLK(dev, hwPort)) != MAD_OK)
			{
				MAD_DBG_ERROR(("Writing from paged phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
			}
		...
		{
			if ((retVal = madHwSetPagedPhyRegField(dev, port, 28, 0, 8, 1, 0x0)) != MAD_OK)
			{
				MAD_DBG_ERROR(("Writing from paged phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
			}
		...
		/* For 151x 1780, use page6, reg26, bit15 */
		else if (dev->deviceId == MAD_88E151x)
		{
			if ((retVal = madHwSetPagedPhyRegField(dev, hwPort, 6, 26, 15, 1, 0x1))
				!= MAD_OK)
			{
				MAD_DBG_ERROR(("Writing from paged phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
    ...
    {
        /* Disable TX_TCLK */
		/* For 151x, use page6, reg26, bit15 */
		if (dev->deviceId == MAD_88E151x || dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M || dev->deviceId == MAD_88E1780)
		{
			if ((retVal = madHwSetPagedPhyRegField(dev, hwPort, 6, 26, 15, 1, 0x0))
				!= MAD_OK)
			{
				MAD_DBG_ERROR(("Writing from paged phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
