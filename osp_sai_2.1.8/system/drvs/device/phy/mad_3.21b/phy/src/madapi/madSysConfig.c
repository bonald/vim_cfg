...
)
{
	...
	/*Initialize the PTP register access(ptpreadplus) semaphore */
	if ((dev->ptpRegsSem = madSemCreate(dev, MAD_SEM_FULL)) == 0)
	{
		MAD_DBG_ERROR(("semCreate Failed.\n"));
		return MAD_API_FAIL_SEM_CREATE;
	}
	/*Initialize the PTP Tod indirect access semaphore */
	if ((dev->ptpTodSem = madSemCreate(dev, MAD_SEM_FULL)) == 0)
	{
		MAD_DBG_ERROR(("semCreate Failed.\n"));
		return MAD_API_FAIL_SEM_CREATE;
	}
	/*Initialize the PTP Mode indirect access semaphore */
	if ((dev->ptpModeSem = madSemCreate(dev, MAD_SEM_FULL)) == 0)
	{
		MAD_DBG_ERROR(("semCreate Failed.\n"));
		return MAD_API_FAIL_SEM_CREATE;
	}
		...
		case MAD_88E151x:
			{
...
#if 1
				retVal = madSmiIfWriteRegister(dev,dev->baseRegAddr+dev->port,MAD_REG_PAGE_ADDR,18);
				if (retVal != MAD_OK)
				{
				...
				retVal = madSmiIfReadRegister(dev,dev->baseRegAddr+dev->port,30,&data);
				if (retVal != MAD_OK)
				{
...
)
{
	...
	/* Delete the PTP register access(ptpreadplus) semaphore.    */
	if (madSemDelete(dev, dev->ptpRegsSem) != MAD_OK)
	{
		MAD_DBG_ERROR(("SemDelete Failed.\n"));
		return MAD_API_FAIL_SEM_DELETE;
	}
	/* Delete the PTP Tod indirect access semaphore.    */
	if (madSemDelete(dev, dev->ptpTodSem) != MAD_OK)
	{
		MAD_DBG_ERROR(("SemDelete Failed.\n"));
		return MAD_API_FAIL_SEM_DELETE;
	}
	/* Delete the PTP Mode indirect access semaphore.    */
	if (madSemDelete(dev, dev->ptpModeSem) != MAD_OK)
	{
		MAD_DBG_ERROR(("SemDelete Failed.\n"));
		return MAD_API_FAIL_SEM_DELETE;
	}
