...
)
{
    ...
    for(port=0; port<dev->numOfPorts; port++)
    {
	  ...
	  /* QSGMII TX amplititude change */
        ...
        if((retVal=madHwWritePagedPhyReg(dev,port,0xFD,7,0x200d)) != MAD_OK)
        {
	  ...
	  /* EEE Initialization */
        ...
        if((retVal=madHwWritePagedPhyReg(dev,port,0xFF,17,0xB030)) != MAD_OK)
        {
            MAD_DBG_ERROR(("SMI Paged Write failed (port%i, Page FF reg17)\n",port));
            return retVal;
        }
        /* REG22 = 0xFF; REG16 = 0x215C */
        if((retVal=madHwWritePagedPhyReg(dev,port,0xFF,16,0x215C)) != MAD_OK)
        {
        ...
        if((retVal=madHwWritePagedPhyReg(dev,port,0xFC,25,0x888C)) != MAD_OK)
        {
...
static MAD_STATUS runOnce88E1780
(
IN MAD_DEV    *dev
)
{
	...
	MAD_DBG_INFO(("runOnce88E1780 called\n"));

	for (port = 0; port<dev->numOfPorts; port++)
	{
		/* REG22 = 0xF8; REG4.11:8 = '1111' */
		if ((retVal = madHwSetPagedPhyRegField(dev, port, 0xF8, 4, 8, 4, 0xF)) != MAD_OK)
		{
			MAD_DBG_ERROR(("SMI Paged Write failed (port%i, Page F8 reg4)\n", port));
			return retVal;
		}

		/* REG22 = 0xF8; REG23 = 0x14 */
		if ((retVal = madHwWritePagedPhyReg(dev, port, 0xF8, 23, 0x14)) != MAD_OK)
		{
			MAD_DBG_ERROR(("SMI Paged Write failed (port%i, Page F8 reg23)\n", port));
			return retVal;
		}
	}
