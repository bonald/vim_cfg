...
/*  cyUsbInitDef */
  ...
  if (((MAD_DEV *)dev)->deviceId == MAD_88E1548)
  {
    ...
    for(i=0; i<4; i++)
    {
      if((retVal = madHwWritePagedPhyReg(dev,i,3,16,0x5777))!= MAD_OK)
      {
        PTP_DBG_INFO(("Writing to paged phy reg (%i,%i_%i) failed.\n",i,16,3));
            return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
      }
/*      if((retVal = madHwWritePagedPhyReg(dev,i,18,27,0x200a))!= MAD_OK) */
      if((retVal = madHwWritePagedPhyReg(dev,i,18,27,0x201a))!= MAD_OK)
      {
        PTP_DBG_INFO(("Writing to paged phy reg (%i,%i_%i) failed.\n",i,27,18));
            return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
      }
  ...
  {
	  int i;
	  for (i = 0; i<((MAD_DEV *)dev)->numOfPorts; i++)
	  {
		  /*      if((retVal = madHwWritePagedPhyReg(dev,i,18,27,0x200a))!= MAD_OK) */
		  if ((retVal = madHwWritePagedPhyReg(dev, i, 18, 27, 0x2000)) != MAD_OK)
		  {
			  PTP_DBG_INFO(("Writing to paged phy reg (%i,%i_%i) failed.\n", i, 27, 18));
			  return (retVal | MAD_API_FAIL_WRITE_PAGED_REG);
		  }
