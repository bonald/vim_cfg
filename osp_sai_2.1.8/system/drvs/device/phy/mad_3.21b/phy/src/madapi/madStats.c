...
)
{
	...
	{
		if ((retVal = E1780_SetCRCCheck(dev, hwPort, mode)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
	...
	{
		if ((retVal = E1680_SetCRCCheck(dev, hwPort, mode)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
	...
	{
		if((retVal = madHwSetPagedPhyRegField(
			dev, hwPort, 18, MAD_REG_CHECK_CONTROL_1,
						0,3,mode)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
...
)
{
    ...
    /* Get the CRC Checker path */
	if (dev->deviceId == MAD_88E1780)
	{
		if ((retVal = E1780_GetCRCCheck(dev, hwPort, &data)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading to paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
	...
	{
		if ((retVal = E1680_GetCRCCheck(dev, hwPort, &data)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading to paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
...
/*
	...
	while for 151x,1680L the register is page 18 reg 17 
*/
static
...
)
{
	...
	if (dev->deviceId == MAD_88E1780)
	{
		if ((retVal = E1780_GetCRCCounter(dev, hwPort, offset, &data)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
	...
	{
		if ((retVal = E1680_GetCRCCounter(dev, hwPort, offset, &data)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Writing to paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
		}
	...
	{
		if ((retVal = madHwGetPagedPhyRegField(
			dev, hwPort, 18, MAD_REG_CRC_COUNTERS,
...
*
* COMMENTS:
*       Page 18, Reg 17, Bit 7:0 : CRC error counter
*
*******************************************************************************/
...
*
* COMMENTS:
*       Page 18, Reg 17, Bit 15:8 : Frame counter
*
*******************************************************************************/
...
*
* COMMENTS:
*       Page 2, 4, 30, Reg 20, Bit 15 : RX_ER Capture
*       Page 18, Reg 26, Bit 15       : RX_ER Capture
*
*******************************************************************************/
...
)
{
	...
	if (dev->deviceId == MAD_88E1780)
	{
		if ((retVal = E1780_GetRxErCapture(dev, hwPort, path, capture)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
	...
	{
		if ((retVal = E1680_GetRxErCapture(dev, hwPort, path, capture)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
...
*
* COMMENTS:
*       Page 2, 4, 30  Reg 20, Bit 13:12 
*       Page 18  Reg 26, Bit 13:12
*
*******************************************************************************/
...
)
{
	...
	if (dev->deviceId == MAD_88E1780)
	{
		if ((retVal = E1780_GetRxErByteNum(dev, hwPort, path, byteNum)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
	...
	if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
	{
		if ((retVal = E1680_GetRxErByteNun(dev, hwPort, path, byteNum)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
...
*
* COMMENTS:
*       Page 2, 4, 30 Reg 20, Bit 9 
*       Page 18 Reg 26, Bit 9
*
*******************************************************************************/
...
)
{
	...
	if (dev->deviceId == MAD_88E1780)
	{
		if ((retVal = E1780_GetRxEr(dev, hwPort, path, rxEr)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
	...
	if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
	{
		if ((retVal = E1680_GetRxEr(dev, hwPort, path, rxEr)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
...
*
* COMMENTS:
*       Page 2, 4, 30,  Reg 20, Bit 8 
*       Page 18,  Reg 26, Bit 8
*
*******************************************************************************/
...
)
{
	...
	if (dev->deviceId == MAD_88E1780)
	{
		if ((retVal = E1780_GetRxDv(dev, hwPort, path, rxDv)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
	...
	if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
	{
		if ((retVal = E1680_GetRxDv(dev, hwPort, path, rxDv)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
...
*
* COMMENTS:
*       Page 2, 4, 30, Reg 20, Bit 7:0 
*       Page 18, Reg 26, Bit 7:0
*
*******************************************************************************/
...
)
{
	...
	if (dev->deviceId == MAD_88E1780)
	{
		if ((retVal = E1780_GetRxData(dev, hwPort, path, rxData)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
	...
	if (dev->deviceId == MAD_88E1680 || dev->deviceId == MAD_88E1680M)
	{
		if ((retVal = E1680_GetRxData(dev, hwPort, path, rxData)) != MAD_OK)
		{
			MAD_DBG_ERROR(("Reading from paged phy reg failed.\n"));
			return (retVal | MAD_API_FAIL_GET_PAGED_REG_FIELD);
		}
...
)
{
	...
	{
		/*Get the Link Disconnect Counter */
		if (dev->deviceId == MAD_88E1780)
		{
			if ((retVal = E1780_GetLinkDisconnectCounter(dev, hwPort, count)) != MAD_OK)
			{
		...
		{
			if ((retVal = E1680_GetLinkDisconnectCounter(dev, hwPort, count)) != MAD_OK)
			{
				MAD_DBG_ERROR(("Reading phy reg failed.\n"));
				return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
			}
		...
		{	
			if((retVal = madHwGetPagedPhyRegField(dev,hwPort,MAD_PAGE18,MAD_REG_LATE_COLIS_WIN_ADJ,0,8,count))
			   != MAD_OK)
			{
			  MAD_DBG_ERROR(("Reading phy reg failed.\n"));
			  return (retVal | MAD_API_FAIL_SET_PAGED_REG_FIELD);
