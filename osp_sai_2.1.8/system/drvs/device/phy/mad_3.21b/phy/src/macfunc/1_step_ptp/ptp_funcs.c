...
MAD_STATUS ptp_1step_dump_mac_statistics (int port, unsigned int *macValList) {
...
MAD_STATUS ptp_1step_dump_sysmac_statistics (int port, unsigned int *sysmacValList){
  MAD_STATUS   retVal = MAD_OK;
	int macName = 0;
...
MAD_STATUS ptp_1step_set_smac_loopback (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS enable_ptp (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
*/
  ...
  if(retVal!=MAD_OK) return retVal;
...
MAD_STATUS disable_ptp (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
/* Enable RX CRC check */
MAD_STATUS ptp_1step_enable_rx_crc_check (int port) {
  MAD_STATUS   retVal = MAD_OK;
  retVal = set_bits (port, 0x41*PTP_ADDR_ADJ+PTP_PORT_OFF(port), 1);
  return retVal;
}
/* Disable RX CRC check */
MAD_STATUS ptp_1step_disable_rx_crc_check (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_wiremac_ipg (int port, int ipg) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_cutthrough_mode (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short regaddr;
  unsigned int data;
...
MAD_STATUS ptp_set_store_forward_mode (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short regaddr;
  unsigned int data;
  regaddr = 0x70*PTP_ADDR_ADJ+PTP_PORT_OFF(port);
...
/* workaround on macsec for 88E1548P*/
MAD_STATUS ptp_macsec_workaround_1 (void) {
  MAD_STATUS   retVal = MAD_OK;
  int port;
  for(port=0; port<4; port++)
  {
    retVal = ptp_writecheck_reg(port, 0xA*PTP_ADDR_ADJ+PTP_PORT_OFF(port), 0x3A);
	if(retVal!=MAD_OK) break;
  }
  return retVal;
}
/* workaround on macsec for 88E1680M*/
MAD_STATUS ptp_macsec_workaround_2(void) {
	MAD_STATUS   retVal = MAD_OK;
	int port;
...
/* common PTP init  */
MAD_STATUS ptp_common_init (int port) {
  MAD_STATUS   retVal = MAD_OK;
  /* Setup STT_CTL */
...
/* PTP parser init */
MAD_STATUS ptp_parser_init (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS ptp_set_udata ( int port, unsigned int * filedata, unsigned int baseaddr );

MAD_STATUS ptp_parser_set_udata (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS ptp_parser_set_udata_ntp (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int eg_addr = (VN_EG0_PTP+PTP_PORT_OFF(port)+PTP_PMEU_UDATA);
  unsigned int ig_addr = (VN_IG0_PTP+PTP_PORT_OFF(port)+PTP_PMEU_UDATA);
  /* Load egress parser udata */
...
MAD_STATUS ptp_check_udata ( int port, unsigned int * filedata, unsigned int baseaddr );


MAD_STATUS ptp_parser_check_udata (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int eg_addr = VN_EG0_PTP+PTP_PORT_OFF(port)+PTP_PMEU_UDATA;
  unsigned int  ig_addr = VN_IG0_PTP+PTP_PORT_OFF(port)+PTP_PMEU_UDATA;
...
MAD_STATUS ptp_parser_check_udata_ntp (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int eg_addr = VN_EG0_PTP+PTP_PORT_OFF(port)+PTP_PMEU_UDATA;
  unsigned int  ig_addr = VN_IG0_PTP+PTP_PORT_OFF(port)+PTP_PMEU_UDATA;
  /* Check egress parser udata */
...
MAD_STATUS ptp_test_clk_in_out_init (int port, int isMaster) {
...
/* PTP update init */
MAD_STATUS ptp_update_init (int port) {
  MAD_STATUS   retVal = MAD_OK;
  /*Both ingress and egress paths */
...
MAD_STATUS ptp_update_set_udata (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS ptp_update_check_udata (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int  eg_addr = VN_EG0_PTP+PTP_PORT_OFF(port)+PTP_UMEU_UDATA;
  unsigned int  ig_addr = VN_IG0_PTP+PTP_PORT_OFF(port)+PTP_UMEU_UDATA;
...
MAD_STATUS ptp_update_set_udata_ntp (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int  eg_addr = VN_EG0_PTP+PTP_PORT_OFF(port)+PTP_UMEU_UDATA;
  unsigned int  ig_addr = VN_IG0_PTP+PTP_PORT_OFF(port)+PTP_UMEU_UDATA;
...
MAD_STATUS ptp_update_check_udata_ntp (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int  eg_addr = VN_EG0_PTP+PTP_PORT_OFF(port)+PTP_UMEU_UDATA;
  unsigned int  ig_addr = VN_IG0_PTP+PTP_PORT_OFF(port)+PTP_UMEU_UDATA;
...
MAD_STATUS ptp_1step_setup_simple_udata (int port) {
  MAD_STATUS   retVal = MAD_OK;
/*   Egress parser udata */
...
/*MAD_STATUS read_lut_entry (long long *lut_match, long long *lut_valid, long long *lut_action, short lut_index, PTP_TYPE path ); */
...
*
*******************************************************************************/
MAD_STATUS prgm_lut_entry (int port, unsigned long long lut_key, unsigned long long lut_valid, unsigned long lut_key_upper, unsigned long lut_valid_upper, unsigned long long lut_action, 
...
/* Read lut entry */
MAD_STATUS read_lut_entry (int port, unsigned long long *lut_key, unsigned long long *lut_valid, unsigned long *lut_key_top, unsigned long *lut_valid_top, unsigned long long *lut_action, short lut_index, int path ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short match_base_addr =(path+PTP_LUT_KEYMASK+8*PTP_ADDR_ADJ*lut_index);
  unsigned short action_base_addr;
  unsigned int data;
  ...
  dataLong |= (unsigned long long)data;
  *lut_key = dataLong;
...
MAD_STATUS check_lut_entry ( int port, int lut_index, int path ) {
  MAD_STATUS   retVal = MAD_OK;
#ifdef DEBUG_PTP
...
MAD_STATUS ptp_find_lut_action(int port, PTP_1STEP_LUT_ACT action, PTP_Blk path, int *entryNum) 
{
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS ptp_enable_lut_action(int port, int idx, PTP_1STEP_LUT_ACT action, PTP_Blk path,MAD_BOOL en ) 
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
/* print PTP stats */
MAD_STATUS get_ptp_stats1 (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
/* dump memory */
MAD_STATUS ptp_1step_dump_mem (int port, unsigned int addr, int range) {
  MAD_STATUS   retVal = MAD_OK;
#ifdef DEBUG_PTP
  int i;
...
/* Clear memory */
MAD_STATUS ptp_1step_write_zeroes (int port, unsigned int addr, int range)  {
  MAD_STATUS   retVal = MAD_OK;
  int i;
...
/* memory test */
MAD_STATUS ptp_1step_write_fs_check (int port, unsigned int addr, int range, unsigned int mask) {
  MAD_STATUS   retVal = MAD_OK;
  int err = 0;
  int i;
...
/* memory test */
MAD_STATUS ptp_1step_write_unique_check (int port, unsigned short addr, int range, unsigned int mask) {
  MAD_STATUS   retVal = MAD_OK;
  int err = 0;
  int i;
...
/* udata mem dump */
MAD_STATUS ptp_1step_mem_dump_update_udata (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int eg_addr =(VN_EG0_PTP+PTP_PORT_OFF(port)+PTP_UMEU_UDATA);
...
/* udata mem test */
MAD_STATUS ptp_1step_mem_test_update_udata (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
/* udata mem test */
MAD_STATUS ptp_1step_mem_test_parser_udata (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS get_ptp_regs ( int portnum, PTP_Blk path ) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS get_ptp_stats ( int portnum, PTP_Blk path ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr;
...
MAD_STATUS ptp_1step_display_contents (int port, unsigned short addr, int size)  {
  MAD_STATUS   retVal = MAD_OK;
#ifdef DEBUG_PTP
  int index;
...
MAD_STATUS get_ptp_pam ( int portnum, PTP_Blk path ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr;
...
MAD_STATUS get_ptp_lut ( int portnum, PTP_Blk path ) {
  MAD_STATUS   retVal = MAD_OK;
#ifdef DEBUG_PTP
  unsigned short my_base_addr;
...
unsigned long long lut_act_adjust ( PTP_1STEP_LUT_ACT name, unsigned long long value ) {
  unsigned long long dataLong = 0LL;
  dataLong = value&0xffffffffLL;
  if (name == PTP_Act_No_Action)
	  return 0;
  else 
	 return (dataLong << lut_act_pos[name] );
}

MAD_STATUS ptp_lut_clear (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
/* demo LUT */
MAD_STATUS ptp_lut_init (int port)
{
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS ptp_lut_init_ntp (int port)
{
  MAD_STATUS   retVal = MAD_OK;
  PTP_TYPE base_addr_id;
...
MAD_STATUS ptp_lut_init_ntp_2 (int port, int udp_src_port, int udp_dst_port)
{
  MAD_STATUS   retVal = MAD_OK;
  PTP_TYPE base_addr_id;
  unsigned short reg_addr;
...
MAD_STATUS get_ptp_tai_tod (int port, unsigned int *_ns, unsigned int *_sec_lo, unsigned int *_sec_hi ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr = ptp_1step_base_addr[TAI_CFG];
...
MAD_STATUS _set_ptp_tai_tod (int port, unsigned int ns, unsigned int sec_lo, unsigned int sec_hi ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr = ptp_1step_base_addr[TAI_CFG];
  unsigned long long sec;
...
MAD_STATUS get_ptp_tai_frc (int port, unsigned int *_ns, unsigned int *_sec_lo, unsigned int *_sec_hi ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr = ptp_1step_base_addr[TAI_CFG];
...
MAD_STATUS set_ptp_tai_frc (int port, unsigned int ns, unsigned int sec_lo, unsigned int sec_hi ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr = ptp_1step_base_addr[TAI_CFG];
  unsigned long long sec;
...
MAD_STATUS get_ptp_tai_drift_adjust (int port, long *adj)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr = ptp_1step_base_addr[TAI_CFG];
...
MAD_STATUS set_ptp_tai_drift_adjust (int port, long adj)
{
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr = ptp_1step_base_addr[TAI_CFG];
  /* drift adjustment, default is 0 */
...
MAD_STATUS get_ptp_tai_step (int port, long *nanosec, long *fracnanosec) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr = ptp_1step_base_addr[TAI_CFG];
...
MAD_STATUS set_ptp_tai_step (int port, long nanosec, long fracnanosec) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short my_base_addr = ptp_1step_base_addr[TAI_CFG];
  /* nanosecond, default is 8 */
  unsigned short reg_addr = my_base_addr + REG_TOD_STEP_NS;
...
static MAD_STATUS getPTPTsQueue(PtpTsQueueStruct *tsQueue, unsigned short *buf)
{
   MAD_STATUS   retVal = MAD_OK;
   tsQueue->seqId = *(buf + 5);
    ...
    tsQueue->ts.extTs.nanoseconds |= (unsigned int)*(buf + 1);
    tsQueue->ts.extTs.topseconds = *(buf + 2);
    tsQueue->ts.extTs.seconds = *(buf + 3);
    tsQueue->ts.extTs.seconds <<= 16;
	tsQueue->ts.extTs.seconds |= (unsigned int)*(buf + 4);
  return retVal;
}
MAD_STATUS displayPTPTsQueue(PtpTsQueueStruct *tsQueue)
{
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS ptp_get_tsql_part ( int portnum,  PTP_Blk path, int count, PtpTsQueueStruct *tsQueue)
{
  MAD_STATUS   retVal = MAD_OK;
  PTP_TYPE base_addr_id;
  unsigned short reg_addr;
  int j;
  unsigned short tsQuBuf[8] = { 0 };
...
MAD_STATUS ptp_get_tsqh_part ( int portnum,  PTP_Blk path, int count, PtpTsQueueStruct *tsQueue)
{
  MAD_STATUS   retVal = MAD_OK;
  PTP_TYPE base_addr_id;
  unsigned short reg_addr;
...
MAD_STATUS ptp_get_tsql ( int portnum,  PTP_Blk path, PtpTsQueueStruct *tsQueue)
{
  return ptp_get_tsql_part(portnum, path, 7, tsQueue);
}

MAD_STATUS ptp_get_tsqh ( int portnum,  PTP_Blk path, PtpTsQueueStruct *tsQueue)
{
  return ptp_get_tsqh_part(portnum, path, 7, tsQueue);
}

MAD_STATUS get_ptp_tsql_data ( int portnum,  PTP_Blk path, int numhwords){
  MAD_STATUS   retVal = MAD_OK;
   PTP_TYPE base_addr_id;
  unsigned short reg_addr;
  int i, j;
...
MAD_STATUS get_ptp_tsqh_data ( int portnum,  PTP_Blk path, int numhwords){
  MAD_STATUS   retVal = MAD_OK;
   PTP_TYPE base_addr_id;
  unsigned short reg_addr;
  int i, j;
...
MAD_STATUS get_ptp_tsqh_drain ( int portnum,  PTP_Blk path ) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS get_ptp_tsql_drain ( int portnum,  PTP_Blk path ) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS get_ptp_latadj (int portnum ) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_ptp_latadj (int portnum, int ns ) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS get_ptp_stt_timer (int portnum ) {
  MAD_STATUS   retVal = MAD_OK;
   PTP_TYPE base_addr_id;
  unsigned short reg_addr;
...
MAD_STATUS set_ptp_stt_timer (int portnum, unsigned int cycles) {
  MAD_STATUS   retVal = MAD_OK;
   PTP_TYPE base_addr_id;
  unsigned short reg_addr;
...
MAD_STATUS get_ptp_disp_timer (int portnum ) {
  MAD_STATUS   retVal = MAD_OK;
   PTP_TYPE base_addr_id;
  unsigned short reg_addr;
...
MAD_STATUS set_ptp_disp_timer (int portnum, unsigned int cycles) {
  MAD_STATUS   retVal = MAD_OK;
   PTP_TYPE base_addr_id;
  unsigned short reg_addr;
...
MAD_STATUS get_ptp_stuff (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS ptp_lut_update (int port, 
...
/* Set PTP register */
MAD_STATUS set_ptp_tai_reg (int port, PTP_1STEP_TAI_REGS regname, unsigned int value ) {
  MAD_STATUS   retVal = MAD_OK;
...
/* Set PTP register */
MAD_STATUS get_ptp_tai_reg (int port, PTP_1STEP_TAI_REGS regname, unsigned int *value) {
  MAD_STATUS   retVal = MAD_OK;
/*  set reg_addr [lsearch $ptp_tai_regs_list $regname] */
  unsigned int reg_addr = regname;
  unsigned int my_base_addr;
...
/* Clear ELU entry */
MAD_STATUS clr_elu_entry (int portnum, int num) {
  MAD_STATUS   retVal = MAD_OK;
...
/* Clear ILU entry */
MAD_STATUS clr_ilu_entry (int portnum, int num)  {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_igr_rply (int portnum, int num)  {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS disable_macsec_both (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS get_ptp_tai_timereg (int port, PTP_1STEP_TAI_REGS regname,unsigned int *sec_hi,unsigned int *sec_lo,unsigned int *ns, unsigned int *fs) {
  MAD_STATUS   retVal = MAD_OK;
...
/* Valid regname is one of: tod_0, frc_0, trig_gen_tod_0, trig_gen_msk_0, tod_load_0, tod_cap_0, tod_cap1_0, next_rt_0 */
MAD_STATUS set_ptp_tai_timereg (int port, PTP_1STEP_TAI_REGS regname,unsigned int sec_hi, unsigned int sec_lo,unsigned int ns, unsigned int fs) {
  MAD_STATUS   retVal = MAD_OK;
/*  set my_base_addr [expr $base_addr(TAI_CFG)+ [lsearch $ptp_tai_regs_list $regname] ] */
  unsigned int my_base_addr = ptp_1step_base_addr[TAI_CFG]+regname;
  unsigned long long  sec;
...
/* ------------------------------------------------------------------------------------ zzz */
MAD_STATUS set_tai_pulse_in_cnt (int port, unsigned int val) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS get_tai_pulse_in_cnt (int port) {
  MAD_STATUS   retVal = MAD_OK;
#ifdef DEBUG_PTP
/*  set addr [expr $base_addr(TAI_CFG)+ [lsearch $ptp_tai_regs_list pulse_in_cnt] ] */
/*   unsigned int addr = ptp_1step_base_addr[TAI_CFG]+ pulse_in_cnt; */
...
MAD_STATUS set_tai_pulse_delay (int port, unsigned int val) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS hard_reset (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS msec_reset (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned int data;
...
MAD_STATUS ptp_lut_default (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
...
MAD_STATUS ptp_lut_interrupt (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
  unsigned long lut_key_top=0;
...
MAD_STATUS set_hard_drop_size (int port, unsigned int size) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_tai_ntp_mode (int port) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_wmac_ipg (int port, unsigned int ipg ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short addr;
  unsigned int regval;
...
MAD_STATUS set_smac_ipg (int port, unsigned int ipg ) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned short addr;
  unsigned int regval;
...
MAD_STATUS set_wiremac_ipg_10 (int port) {
  return (set_wmac_ipg (port, 10));
}
MAD_STATUS set_wiremac_ipg_11 (int port) {
  return (set_wmac_ipg (port, 11));
}
MAD_STATUS set_wiremac_ipg_12 (int port) {
  return (set_wmac_ipg (port, 12));
}

MAD_STATUS set_sysmac_ipg_11 (int port) {
  return (set_smac_ipg (port, 11));
}
MAD_STATUS set_sysmac_ipg_12 (int port) {
...
MAD_STATUS set_tai_tod_func (int port, unsigned int tod_func, unsigned int trig){
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_tod_cap_cfg (int port, unsigned int val) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_elu_entry_match_da (int portnum, int en, int da) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_egr_def_drop (int port, int flag, int portnum) {
  MAD_STATUS   retVal = MAD_OK;
...
MAD_STATUS set_ptp_timeout_ctrl(int cycles, int portnum) {
  MAD_STATUS   retVal = MAD_OK;
   PTP_TYPE base_addr_id;
  unsigned short reg_addr;
  base_addr_id = get_ptp_base_addr_id( portnum, PTP_egr);
...
MAD_STATUS set_asymmetry(unsigned int asym, PTP_Blk path, int port) {
  MAD_STATUS   retVal = MAD_OK;
   /* Both ingress and egress paths */
  if (path == PTP_igr) {
...
MAD_STATUS set_link_delay(int ldelay, int port) {
  MAD_STATUS   retVal = MAD_OK;
