...
*
*******************************************************************************/
MAD_STATUS ptp_lut_config_eg_TS_queue_lo (int port) {
  MAD_STATUS   retVal = MAD_OK;
  unsigned long long lut_key;
  unsigned long long lut_vld;
  unsigned long lut_key_upper=0;
...
MAD_STATUS ntp_test_config (int port) {
  MAD_STATUS   retVal = MAD_OK;
  int i;
/*
...
*/
  for(i=0; i<2; i++)
  {
    retVal = prgm_lut_ent_ntp(port, i, igrActionList[i], PTP_igr); 
    if(retVal!=MAD_OK) return retVal;
    retVal = prgm_lut_ent_ntp(port, i, egrActionList[i], PTP_egr); 
    if(retVal!=MAD_OK) return retVal;
  }
...
MAD_STATUS ntp_test_config (int port) {
  MAD_STATUS   retVal = MAD_OK;
  int i;
  retVal = prgm_lut_ent_ntp_list(port, 0, igrActionList, PTP_igr); 
  if(retVal!=MAD_OK) return retVal;
  retVal = prgm_lut_ent_ntp_list(port, 0, egrActionList, PTP_egr); 
  if(retVal!=MAD_OK) return retVal;

/* For second UDP port */
  retVal = prgm_lut_ent_ntp_list(port, 1, igrActionList_1, PTP_igr); 
  if(retVal!=MAD_OK) return retVal;
  retVal = prgm_lut_ent_ntp_list(port, 1, egrActionList_1, PTP_egr); 
  if(retVal!=MAD_OK) return retVal;
...
int numwords;
  ...
  do
  {
	...
	if((numwords/7==0)&&(numwords%7))
	{
      ...
      if(retVal!=MAD_OK) break;
      retVal = ptp_enable_lut_action(port, idx, PTP_Act_StampLow, PTP_igr,MAD_FALSE); 
      if(retVal!=MAD_OK) break;
      retVal = ptp_get_tsql_part(port, PTP_igr, numwords%7, &ts);
      if(retVal!=MAD_OK) break;
      retVal = ptp_enable_lut_action(port, idx, PTP_Act_StampLow, PTP_igr,MAD_TRUE); 
      if(retVal!=MAD_OK) break;
	}
#endif
  } while (numwords!=0);
  ...
  do
  {
	...
	if((numwords/7==0)&&(numwords%7))
	{
      ...
      if(retVal!=MAD_OK) break;
      retVal = ptp_enable_lut_action(port, idx, PTP_Act_StampLow, PTP_egr,MAD_FALSE); 
      if(retVal!=MAD_OK) break;
      retVal = ptp_get_tsql_part(port, PTP_egr, numwords%7, &ts);
      if(retVal!=MAD_OK) break;
      retVal = ptp_enable_lut_action(port, idx, PTP_Act_StampLow, PTP_egr,MAD_TRUE); 
      if(retVal!=MAD_OK) break;
	}
#endif
  } while (numwords!=0);
...
int numwords;
  ...
  do
  {
	...
	if((numwords/7==0)&&(numwords%7))
	{
      ...
      if(retVal!=MAD_OK) break;
      retVal = ptp_enable_lut_action(port, idx, PTP_Act_StampHigh, PTP_igr,MAD_FALSE); 
      if(retVal!=MAD_OK) break;
      retVal = ptp_get_tsql_part(port, PTP_igr, numwords%7, &ts);
      if(retVal!=MAD_OK) break;
      retVal = ptp_enable_lut_action(port, idx, PTP_Act_StampHigh, PTP_igr,MAD_TRUE); 
      if(retVal!=MAD_OK) break;
	}
#endif
  ...
  do
  {
	...
	if((numwords/7==0)&&(numwords%7))
	{
      ...
      if(retVal!=MAD_OK) break;
      retVal = ptp_enable_lut_action(port, idx, PTP_Act_StampHigh, PTP_egr,MAD_FALSE); 
      if(retVal!=MAD_OK) break;
      retVal = ptp_get_tsql_part(port, PTP_egr, numwords%7, &ts);
      if(retVal!=MAD_OK) break;
      retVal = ptp_enable_lut_action(port, idx, PTP_Act_StampHigh, PTP_egr,MAD_TRUE); 
      if(retVal!=MAD_OK) break;
	}
#endif
  } while (numwords!=0);
