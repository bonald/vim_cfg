...
static MAD_U32		orgCopperSpeed = 2;
...
MAD_STATUS startMACIfLoopback(MAD_DEV *dev,MAD_LPORT port,MAD_MAC_LOOPBACK_MODE mode)
{
    ...
    */
    if ((dev->deviceId!=MAD_88E1116)&&(dev->deviceId!=MAD_88E3016)
		...
		&& (dev->subDevId != MAD_SUB_88E1510 && (dev->deviceId != MAD_88E1780)))
        /* E1116/E3016/E3082/E1121/E1510 has no SGMII */
    {
    ...
    {
		if ((status = mdCopperGetAutoNeg(dev, port, &orgSGMIIState, &orgCopperSpeed)) != MAD_OK)
        {
    ...
    */
    if((status = mdDiagSetMACIfLoopback(dev,port,MAD_TRUE,mode, MAD_SPEED_1000M)) != MAD_OK)
    {
...
MAD_STATUS stopMACIfLoopback(MAD_DEV *dev,MAD_LPORT port,MAD_MAC_LOOPBACK_MODE mode)
{
    ...
    */
    ...
    {
		if ((status = mdCopperSetAutoNeg(dev, port, orgSGMIIState, orgCopperSpeed)) != MAD_OK)
        {
