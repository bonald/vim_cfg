...
*/
MAD_STATUS sampleTempIntEnable
(
MAD_DEV         *dev,
MAD_LPORT       port
)
{
	...
	for (port = 0; port<dev->numOfPorts; port++)
	{
		if ((status = mdTempIntSetEnable(dev, port, 1)) != MAD_OK)
		{
			MSG_PRINT(("mdTempIntSetEnable returned fail.\n"));
			testMADDisplayStatus(status);
			return status;
		}
	}
...
*/
MAD_STATUS sampleTempIntDisable
(
MAD_DEV         *dev,
MAD_LPORT       port
)
{
	...
	for (port = 0; port<dev->numOfPorts; port++)
	{
		if ((status = mdTempIntSetEnable(dev, port, 0)) != MAD_OK)
		{
			MSG_PRINT(("mdTempIntSetEnable returned fail.\n"));
			testMADDisplayStatus(status);
			return status;
...
/*
*    Read Temperature sensor Interrupt status.
*/
MAD_STATUS ReadIntTempStatus
(
MAD_DEV         *dev,
MAD_LPORT       port
)
{
	...
	if ((status = mdTempIntGetStatus(dev, port, &st)) != MAD_OK)
	{
		MSG_PRINT(("mdTempIntGetStatus returned fail.\n"));
		testMADDisplayStatus(status);
		return status;
	}

	if (st == MAD_TRUE)
		MSG_PRINT(("Temperature Reached Threshold.\n"));
	else
		MSG_PRINT(("Temperature Below Threshold.\n"));
