...
class InterfaceTableHandler(SnmpTableBase):
    ...
    def check(self, colnum, var_type, data):
        ...
        if colnum == TBL_INTERFACE_RESET_CNT or colnum == TBL_INTERFACE_RESET_FLOWCNT:
            if var_type != ASN_INTEGER:
                return self.set_request_error(SNMP_ERR_WRONGTYPE)
            reset = data
            if reset != 1:
        ...
        if colnum == TBL_INTERFACE_ENGRESS_FLOWNAME:
            ...
            if len(flow_name) != 0 and \
                len(re.search("^[0-9a-zA-Z._-]*", flow_name[0:len(flow_name)]).group(0)) != len(flow_name):
                return self.set_request_error(SNMP_ERR_WRONGVALUE)
    ...
    def set(self, colnum, data):
        ...
        if colnum == TBL_INTERFACE_RESET_FLOWCNT:
            szmsg = execute_cmd('cdbctl update/cdb/tap/clear_tap_flow_counter/{0}'.format(if_name))

        if colnum == TBL_INTERFACE_RESET_EGS_FLOWCNT:
            szmsg = execute_cmd('cdbctl update/cdb/l2/clear_acl_counter/out/{0}'.format(if_name))
        ...
        if colnum == TBL_INTERFACE_ENGRESS_FLOWNAME:
            flow_name = data
            if len(flow_name) != 0:
                szmsg = execute_cmd('cdbctl read/cdb/l2/acl_config/{0}'.format(flow_name))
                if szmsg.find('%') != -1:
                    return self.set_request_error(SNMP_ERR_WRONGVALUE)

                szmsg = execute_cmd('cdbctl update/cdb/interface/{0}/egs_acl/{1}'.format(if_name, flow_name))
                if szmsg.find('%') != -1:
                    return self.set_request_error(SNMP_ERR_WRONGVALUE)

            else:
                szmsg = execute_cmd('cdbctl read/cdb/interface/{0}/egs_acl/'.format(if_name))
                if szmsg.find('%') != -1:
                    return self.set_request_error(SNMP_ERR_WRONGVALUE)
                flow_name = szmsg.replace("/esg_acl=","")
                if len(flow_name) == 0:
                ...
                szmsg = execute_cmd('cdbctl update/cdb/interface/{0}/egs_acl/""'.format(if_name))
                if szmsg.find('%') != -1:
                    return self.set_request_error(SNMP_ERR_WRONGVALUE)
    ...
    def subscriber(self, tbl_id, operation, ds_id, ds_id2, field_id, data):
        ...
        if operation == CDB_OPER_ADD:
            if ds_if_stats_flag != True:
                ...
                newRow.setRowCell(TBL_INTERFACE_ENGRESS_FLOWNAME, AGENT.DisplayString(tbl_interface.egs_acl))
                ...
                for key in InterfaceTableHandler.if_index_table:
                    #if ds_if_stats.parent_if.name == InterfaceTableHandler.if_index_table[key]["if_key_name"]:
                    #    #print "ds_if_stats_flag : True, operation ADD"
                    #    #print key, ds_if_stats.parent_if.name
                    #    newRow = INTERFACE_TABLE.addRow([AGENT.Integer32(key)])
                    #    newRow.setRowCell(TBL_INTERFACE_5MIN_IN_BITS_RATE, AGENT.Counter64(ds_if_stats.octets_rcv_rate))
                    #    newRow.setRowCell(TBL_INTERFACE_5MIN_IN_PKTS_RATE, AGENT.Counter64(ds_if_stats.pkts_rcv_rate))
                    #    newRow.setRowCell(TBL_INTERFACE_5MIN_OUT_BITS_RATE, AGENT.Counter64(ds_if_stats.octets_send_rate))
                    #    newRow.setRowCell(TBL_INTERFACE_5MIN_OUT_PKTS_RATE, AGENT.Counter64(ds_if_stats.pkts_send_rate))
                    #    break
                    print key
        ...
        if operation == CDB_OPER_SET:
            if ds_if_stats_flag != True:
                ...
                if field_id == TBL_INTERFACE_FLD_EGS_ACL:
                    curRow.setRowCell(TBL_INTERFACE_ENGRESS_FLOWNAME, AGENT.DisplayString(tbl_interface.egs_acl))
            else:
                for key in InterfaceTableHandler.if_index_table:
                    #if ds_if_stats.parent_if.name == InterfaceTableHandler.if_index_table[key]["if_key_name"]:
                    #    #print "ds_if_stats_flag : True, operation SET"
                    #    #print key, ds_if_stats.parent_if.name, ds_if_stats.octets_rcv_rate
                    #    curRow = INTERFACE_TABLE.addRow([AGENT.Integer32(key)])
                    #    curRow.setRowCell(TBL_INTERFACE_5MIN_IN_BITS_RATE, AGENT.Counter64(ds_if_stats.octets_rcv_rate))
                    #    curRow.setRowCell(TBL_INTERFACE_5MIN_IN_PKTS_RATE, AGENT.Counter64(ds_if_stats.pkts_rcv_rate))
                    #    curRow.setRowCell(TBL_INTERFACE_5MIN_OUT_BITS_RATE, AGENT.Counter64(ds_if_stats.octets_send_rate))
                    #    curRow.setRowCell(TBL_INTERFACE_5MIN_OUT_PKTS_RATE, AGENT.Counter64(ds_if_stats.pkts_send_rate))
                    #    break
                    print key
...
def init(agent_obj, subscriber):
    ...
    INTERFACE_TABLE = AGENT.Table(
        ...
        columns = [
            ...
            (TBL_INTERFACE_ENGRESS_FLOWNAME, AGENT.DisplayString(""), 1),
            (TBL_INTERFACE_LINK_STATUS,  AGENT.Integer32(0), 0),
            (TBL_INTERFACE_RESET_FLOWCNT,  AGENT.Integer32(0), 1),
            (TBL_INTERFACE_RESET_EGS_FLOWCNT, AGENT.Integer32(0), 1),
