...
/* xuwj: support ifname_prefix */			
			    pchSlash_prefix = strchr(if_name, '*');
                if (pchSlash_prefix != NULL){
                    match_type = MATCH_IFNAME_PREFIX;
                    prefixlen  = pchSlash_prefix - if_name;
                    *pchSlash_prefix = '\0';
                }else{
	...
	)
{
	...
	char cmd_line[100] = {0};
#ifndef _CENTEC_
	static char    line_version[128] = {0};
	FILE    *fp      = NULL;
	char *tmpFile = "/tmp/ntp_version";
#endif
	int rc = 0;
...
# ifdef WORKER
	...
	for (; curr_peer != NULL; curr_peer = curr_peer->link) {
        ...
        /* modify by huxd for bugid 52041. 2019-10-18 */
#ifndef _CENTEC_
                  sprintf(cmd_line,"more /etc/ntp.conf | grep %s | awk '{print $7}'>%s",curr_peer->addr->address,tmpFile);
                  rc = system(cmd_line);
                  if (!rc)
                  {
                    fp = fopen(tmpFile, "r");
                  ...
                  if (NULL != fp)
                  {
                    fgets(line_version, sizeof(line_version), fp);
                    if ((strlen(line_version))&&('\n' != line_version[0]))
                    {   
                        curr_peer->peerversion = (u_char)line_version[0];
                    }
                    else
                    {   
                        curr_peer->peerversion = NTP_VERSION;
                    }
                    fclose(fp);
                  } 
#endif
