...
static  void    broadcast_delay(sockaddr_u *, endpt *, struct req_pkt *);
static  void    set_minimum_distance(sockaddr_u *, endpt *, struct req_pkt *);
static	void	set_ifstatus(sockaddr_u *, endpt *, struct req_pkt *);
/* end */
...
static const struct req_proc ntp_codes[] = {
	...
	/* modify by huxd for bugid 53993. 2019-10-22 */
#ifdef _CENTEC_
	{ REQ_RESET_STATS, NOAUTH, sizeof(struct reset_flags), 0, reset_stats },
#else
...
/* Added by xuwj: new commands 2016-12-01 */
    { REQ_BROADCAST_DELAY,   NOAUTH, sizeof(u_long), sizeof(u_long), broadcast_delay},
    { REQ_MIN_DISTANCE,      NOAUTH, sizeof(u_long), sizeof(u_long), set_minimum_distance},
    { REQ_SET_IF_STATUS,     NOAUTH, sizeof(u_long), sizeof(u_long), set_ifstatus},
/* end */
...
/* Added by xuwj: new commands 2016-12-01 */
/*
 * broadcast_delay: setting broadcast delay
 */
static void
broadcast_delay(
	sockaddr_u *srcadr,
	...
	)
{
	...
	kp = (u_long *)inpkt->u.data;
    sys_bdelay = (double)(*kp) / 1000.0f;
	...
	)
{
	...
	kp = (u_long *)inpkt->u.data;
    sys_mindisp = (double)(*kp) / 1000.0f;
	req_ack(srcadr, inter, inpkt, INFO_OKAY);
}

/*
 * set_ifstatus - set interface status
 */
static void
set_ifstatus(
	sockaddr_u *srcadr,
	...
	)
{
    ...
    while (items-- > 0) {
        ...
        items--;
        status = *kp;
        set_interface_status(ifnum, status);
    }
	...
	)
{
	...
	while (items-- > 0 && ip != NULL) {
        ...
        if (pp->flags & FLAG_TRUE)
			ip->flags |= INFO_FLAG_TRUE;
        /* end */
...
/* Added by xuwj: more display 2016-12-01 */
    is->sys_mindisp = htonl(DTOFP(sys_mindisp));
    is->sys_maxdist = htonl(DTOFP(sys_maxdist));
/* end */
	...
	)
{
	...
	while (items-- > 0) {
        ...
        /* Added by xuwj: ntpdc addserver always trust 2016-12-01 */
        if (temp_cp.hmode == MODE_CLIENT) {
            fl |= FLAG_TRUE;
        }
        /* end */
