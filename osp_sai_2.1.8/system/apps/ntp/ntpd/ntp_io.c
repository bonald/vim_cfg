...
/* modified by liwh for bug 51536, 2019-04-11 */
static  SOCKET last_fd = 0;
/* liwh end */

static char*
if_ethname_to_fullname(char *ethname, char *fullname, int buf_len)
{
    ...
    if (0 == strncmp(GLB_IFNAME_ETH_PREFIX, ethname, GLB_IFNAME_ETH_PREFIX_LEN))
    {
        strncpy(fullname, ethname, buf_len);
        p = strchr(fullname, '_');
        if (p)
        {
            p[0] = '/';
...
/*interface status, used in g_  ifstatus*/
#define M_NTP_LOCK_INTERFACE            (1<<0)
#define M_NTP_BROADCAST_CLIENT          (1<<1)

/*interface status*/
u_long g_ifstatus[M_NTP_MAX_INTERFACE_CNT];
/* end */
	...
	)
{
	...
	if_ethname_to_fullname(ep->name, ifname_ext, 32);
...
#	    endif
	...
	if (ep->bfd != INVALID_SOCKET) {
		msyslog(LOG_INFO,
			...
			stoa(&ep->bcast), ep->ifnum, ifname_ext);
#	    ifdef HAVE_IO_COMPLETION_PORT
	...
	)
{
	...
	rule->action = action;
/* xuwj: support ifname_prefix */
	if (MATCH_IFNAME_PREFIX== match_type) {
		REQUIRE(NULL != if_name);
		rule->if_name = estrdup(if_name);
/* xuwj: support ifname_prefix end */         
	} else if (MATCH_IFNAME == match_type) {
		REQUIRE(NULL != if_name);
		rule->if_name = estrdup(if_name);
	...
	)
{
	...
	if_ethname_to_fullname(if_name, ifname_ext, 32);
	DPRINTF(4, ("interface_action: interface %s ",
		    (if_name != NULL) ? ifname_ext : "wildcard"));
...
//				    action_text(rule->action)));
			...
			break;
/* xuwj: support ifname_prefix */            
        case MATCH_IFNAME_PREFIX:
			if (if_name != NULL
			    && !strncasecmp(if_name, rule->if_name, rule->prefixlen)
			    ) {
...
/* add by xuwj 2016-12-01 */
int 
is_interface_locked(char *ifname)
{
    int module = -1;
    int port = -1;

    if (0 == strncmp (ifname, "eth-", 4))
    {
        sscanf(ifname, "eth-%u-%u", &module, &port);
        port = module * M_NTP_MAX_ETH_IF_CNT_ONE_SLOT + port;
    }
    else if(0 == strncmp (ifname, "lo", 2))
    {
        sscanf(ifname, "loopback%u", &port);
        port = port + M_NTP_MAX_ETH_IF_CNT;
    }
    else if (0 == strncmp (ifname, "vlan", 4))
    {
        sscanf(ifname, "vlan%u", &port);
        port = port + M_NTP_MAX_ETH_IF_CNT + M_NTP_MAX_LOOPBACK_IF_CNT;
    }
    else
    {
        /*modified by ychen in 2009-10-19 for master and standby time sync up*/
        //return 1;
        return 0;
    }

    return (g_ifstatus[port] & M_NTP_LOCK_INTERFACE); 
}
/* end */
	...
	)
{
	...
	for (result = isc_interfaceiter_first(iter);
        ...
        /* Modified by xuwj 2016-12-01 */
		if (is_interface_locked(isc_if.name))  
		{
		    continue;
		}
        /* end */
		...
		if_ethname_to_fullname(enumep.name, ifname_ext, 32);
	...
	)
{
	...
	if (INVALID_SOCKET == iface->fd
	    ...
	    if_ethname_to_fullname(iface->name, ifname_ext, 32);
...
io_setbclient(void)
{
	...
	for (interf = ep_list;
		...
		if (interf->bfd != INVALID_SOCKET) {
			...
			if_ethname_to_fullname(interf->name, ifname_ext, 32);
			msyslog(LOG_INFO,
				"Listen for broadcasts to %s on interface #%d %s",
				stoa(&interf->bcast), interf->ifnum, ifname_ext);
		} else switch (errno) {
	...
	)
{
	...
	if (INVALID_SOCKET == fd) {
		...
		if (errval == EPROTONOSUPPORT ||
		    errval == EAFNOSUPPORT ||
		    errval == EPFNOSUPPORT ||
/* xuwj: fix by open too many socket and exit */		    
		    errval == EMFILE)
/* end */		    
			return (INVALID_SOCKET);
        ...
        if (fd >= FD_SETSIZE) {
            last_fd = fd;
            msyslog(LOG_ALERT, "Too many sockets in use, FD_SETSIZE %d exceeded by fd %d", FD_SETSIZE, fd);
            return INVALID_SOCKET;
	}
        /* liwh end */
	...
	)
{
	...
	do {
		...
		if (cc == -1) {
			...
			src->sent++;
			if(!IPV4_NET127(ntohl((u_int32)dest->sa4.sin_addr.s_addr)))
			{
			    packets_sent++;
			}
		}
	...
	)
{
	...
	if (!consumed) {
		rp->recvcount++;
		if(!IPV4_NET127(ntohl((u_int32)rb->X_from_where.X_recv_srcadr.sa4.sin_addr.s_addr)))
		{
		    packets_received++;
		}
	}
	...
	)
{
	...
	for (rp = refio; rp != NULL; rp = rp->next) {
		...
		flag = get_localhost_flag(fd,rp,ts);
        if (1 == flag)
        {
    	 ++handler_pkts;
    	}
...
/*
 * io_getIfname - get interface name
 */
void
io_getIfname(int ifnum, char *ifname)
{
    int module = 0;

    if (ifnum >= M_NTP_MAX_ETH_IF_CNT)
    {
        snprintf(ifname, 32, "loopback%u", (ifnum -M_NTP_MAX_ETH_IF_CNT));
    }
    else
    {
        snprintf(ifname, 32, "eth-%u-%u", module, ifnum);
    }
    return;
}
...
/*
 * set_interface_status - declare an interface to be locked/unlocked
 */
void
...
{
    g_ifstatus[ifnum] = status;

    msyslog(LOG_DEBUG, "interface: %ld, lock status: %ld", ifnum, status);

    if (g_ifstatus[ifnum] & M_NTP_BROADCAST_CLIENT)
        __io_setbclient(ifnum);
    else
        __io_unsetbclient(ifnum);
	...
	)
{
	...
	if (NULL == iface) {
		...
		DPRINTF(4, ("Found interface #%d %s for address %s\n",
			    iface->ifnum, ifname_ext, stoa(addr)));
