...
/* Added by xuwj: new commands 2016-12-01 */
static	void	do_broadcastdelay (struct parse *, FILE *);
static	void	do_setmindist (struct parse *, FILE *);
static	void	setifstatus	(struct parse *, FILE *);
/* end */
...
struct xcmd opcmds[] = {
    ...
    /* Added by xuwj: new commands 2016-12-01 */
	{ "broadcastdelay", do_broadcastdelay, { NTP_UINT, NO, NO, NO },
	  { "delay", "", "", "" },
	  "broadcast delay value (millseconds)" }, 
    { "setifstatus", setifstatus, { NTP_UINT, NTP_UINT, NO, NO },
	  { "interface", "status", "", "" },
	  "set interface status" },
	{ "setmindist", do_setmindist, { NTP_UINT, NO, NO, NO },
	  { "min_dist", "", "", "" },
	  "minimum distance value (millseconds) (1000 ~ 1)" },
    /* end */
...
/* Added by xuwj: new commands 2016-12-01 */
/*
 * do_broadcastdelay- do setting broadcast delay
 */
static void
do_broadcastdelay(
	struct parse *pcmd,
...
{
	u_long bdelay = pcmd->argval[0].uval;
    size_t items = 0;
    size_t itemsize = 0;
    int res = 0;
...
again:
	res = doquery(impl_ver, REQ_BROADCAST_DELAY, 0, 1, sizeof(u_long),
		      (char *)&bdelay, &items, &itemsize, &dummy, 0, 
		      sizeof(*dummy));
...
/*
 * setifstatus - set minimum distance
 */
static void
do_setmindist(
	struct parse *pcmd,
...
{
	u_long mindis = pcmd->argval[0].uval;
    size_t items = 0;
    size_t itemsize = 0;
    int res = 0;
    const char *dummy = NULL;
...
again:
	res = doquery(impl_ver, REQ_MIN_DISTANCE, 0, 1, sizeof(u_long),
		      (char *)&mindis, &items, &itemsize, &dummy, 0, 
		      sizeof(*dummy));
...
/*
 * setifstatus - set interface status
 */
static void
setifstatus(
	struct parse *pcmd,
	...
	)
{
	...
	for (i = 0; i < pcmd->nargs; i++) {
		ports[ritems++] = pcmd->argval[i].uval;
	}

again:
	res = doquery(impl_ver, REQ_SET_IF_STATUS, 0, ritems, sizeof(u_long),
		      (char *)ports, &items, &itemsize, &dummy, 0, 
		      sizeof(*dummy));
...
/* 
 * const char* ntp_transform_sourceAddress(const char *destAddress, const char* srcAddress)
 * modified by huxd for bug 52078 52317, 2019-9-26. 
 * stratum is a new para for show ntp peer loacl item in association infomation.
 */
const char* ntp_transform_sourceAddress(const char *destAddress, const char* srcAddress, const void* ips)
{    
    FILE    *fp      = NULL;
    static char    line[128] = {};
    int32_t bFind    = 0;
    char   *tmpFile  = "/tmp/ntp-tables_XXXXXX";
    /* modified by liwh for bug 49264, 2018-09-29 */
    static char    str1[128] = {};
    int32            index = 0;
    /* liwh end */
    struct info_peer_summary * plist = NULL;

    /* modified by huxd for bug 52078 52317 53989, 2019-9-26 */
    plist = (struct info_peer_summary *)ips;
    if ( (STRATUM_UNSPEC <= plist->stratum) && !(plist->flags & INFO_FLAG_SYSPEER) )
    {
        if (0 == plist->v6_flag)
        {
            sprintf(str1, "%s", "0.0.0.0");
        }
        else
        {
            sprintf(str1, "%s", "::");
    ...
    /* modified by liwh for bug 49264, 2018-09-29 */
    if (0 == strcmp(srcAddress, "fec0:fffe:fffe::2:1"))
    {
        sprintf(line, " ip netns exec mgmt ifconfig eth0 | grep 'inet6 addr:' | grep 'Global' | awk '{print $3}' > %s", tmpFile);
    ...
    /* liwh end */
    else if(0 != strcmp(srcAddress, "169.254.2.1"))
    {
        if(system("ntp-iptables.sh") != 0)
        {
            return NULL;
        }
        ...
        {
            if(strstr(line, "Chain POSTROUTING"))
            {
                bFind = 1;
                break;
            }
        ...
        if(bFind)
        {
            while(fgets(line, sizeof(line), fp) != NULL)
            {
                if(strstr(line, "SNAT") && strstr(line, "dpt:ntp"))
                {
                    if(strstr(line, destAddress))
                    {
                        if(line[strlen(line)-1] == '\n')
                        {
                            line[strlen(line)-1] = '\0';
        ...
        sprintf(line, "mgmt-ip.sh > %s", tmpFile);
        if(system(line) != 0)
        {
            return NULL;
        }
...
again:
    ...
    /* Added by xuwj: more display 2016-12-01 */
    if (!dmstyle) {
        printf("* synced,      + symmetric active mode, - symmetric passive mode,\n"
               "= client mode, ^ broadcast mode,        ~ broadcast client mode\n"); 
    } else {
        printf("* synced, + selected, . candidate\n");
    }
    printf("\n");
    /* end */
	...
	while (items > 0) {
		...
		if ((pcmd->nargs == 0) ||
		    ((pcmd->argval->ival == 6) && (plist->v6_flag != 0)) ||
		    ((pcmd->argval->ival == 4) && (plist->v6_flag == 0)))
		    str = ntp_transform_sourceAddress(nntohost(&srcadr), nntohost(&dstadr), (void*)plist);
        
			(void) fprintf(fp,
			    "%c%-15.15s %-15.15s %2u %4d  %3o %7.7s %9.9s %7.7s\n",
			    c, nntohost(&srcadr), str ? str : stoa(&dstadr),
               ...
               /* modified by liwh for bug 49264, 2018-09-29 */
               str_remote = nntohost(&srcadr);
               if (strlen(str_remote) > 15)
               {
...
/* Added by xuwj: more display 2016-12-01 */
    if (flags & INFO_FLAG_TRUE) {
		fprintf(fp, "%s true", dlim);
		dlim = comma;
	}
/* end */
...
again:
	...
	(void) fprintf(fp, "Current NTP status:\n");
	(void) fprintf(fp, "============================================================\n");
    ...
    /* Added by xuwj: more display 2016-12-01 */
    (void) fprintf(fp, "minimum distance     : %s s\n",
		       fptoa(NTOHS_FP(is->sys_mindisp), 5));
    (void) fprintf(fp, "selection threshold  : %s s\n",
		       fptoa(NTOHS_FP(is->sys_maxdist), 5));
    /* end */
...
again:
	...
	if (!checkitemsize(itemsize, sizeof(*io)))
       ...
       fprintf(fp, "Current NTP I/O statistics:\n" );
	fprintf(fp, "============================================================\n" );
	fprintf(fp, "time since reset     :%lu\n",
