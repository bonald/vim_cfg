	...
	)
{
	...
	if (numcmds == 0) {
		...
		/* Modified by xuwj for default auth (keyid = 64567; md5 = "m6~6^b?z$}HTC<b'" only use ntpdc -nc cmd) (ntpdc<->ntpd) 2016-12-01 */
		/* same as /etc/ntp/key.conf */
		#define NTP_DEFAULT_REQUEST_KEY_NO  64567
		#define NTP_DEFAULT_REQUEST_KEY     "m6~6^b?z$}HTC<b'"

        info_auth_keyid = NTP_DEFAULT_REQUEST_KEY_NO;
        keyid_entered = 1;
		
        authusekey(info_auth_keyid, info_auth_keytype,(u_char *)NTP_DEFAULT_REQUEST_KEY);
