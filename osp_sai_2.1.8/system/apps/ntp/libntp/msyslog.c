	...
	)
{
    ...
    static int		setting_level = LOG_CRIT;
    static int      bGetLevel     = 0;
    char	buf_debug_level[8] = {0};
...
/* SYSTEM MODIFY: xuwj support ntp debug level */
    if(0 == bGetLevel)
    {
        term_file = fopen("/etc/ntp/ntpd.debug","r");
        if(NULL == term_file)
        {
            setting_level = LOG_CRIT;
        }else{
            fgets(buf_debug_level,sizeof(buf_debug_level),term_file);
            setting_level = atoi(buf_debug_level);
        }

        bGetLevel = 1;
    }
        
    if(level <= setting_level)
    {
        syslog(level, "%s", msg);
    }
/* end */
