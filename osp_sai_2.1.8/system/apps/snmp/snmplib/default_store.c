...
unsigned int netsnmp_ctc_rx_count = 0;
unsigned int netsnmp_ctc_tx_count = 0;
unsigned int netsnmp_ctc_trap_count = 0;

int
netsnmp_ds_set_ctc_stats(int action)
{
    FILE *fp = NULL;
    if (!action)
    {
        netsnmp_ctc_tx_count = 0;
        netsnmp_ctc_rx_count = 0;
        netsnmp_ctc_trap_count = 0;
    }

    /* update counter to tmp file */
    fp = fopen("/tmp/snmp_ctc_stats", "w+");
    if (NULL == fp)
    {
        return SNMPERR_SUCCESS;
    }
    fprintf(fp, "Received count: %u\n", netsnmp_ctc_rx_count);
    fprintf(fp, "Transmit count: %u\n", netsnmp_ctc_tx_count);
    fprintf(fp, "Trap count:     %u\n", netsnmp_ctc_trap_count);
...
int
netsnmp_ds_set_ctc_debug(int enable)
{
    netsnmp_ds_ctc = enable;
    return SNMPERR_SUCCESS;
}

int
netsnmp_ds_get_ctc_debug()
{
    return (netsnmp_ds_ctc) ? 1 : 0;
}
