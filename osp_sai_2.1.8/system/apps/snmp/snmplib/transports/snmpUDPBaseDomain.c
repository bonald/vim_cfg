...
_netsnmp_udp_sockopt_set(int fd, int local)
{
    ...
    /* Bug 45843, snmp response's ip dscp should be bigger */
#ifdef _CENTEC_
#ifdef IP_TOS
    unsigned char dscp_type = 0xe0 | IPTOS_LOWDELAY | IPTOS_RELIABILITY;
    if (setsockopt(fd, IPPROTO_IP, IP_TOS, (void *) &dscp_type, sizeof(dscp_type)) < 0)
    {
        perror("SNMP UDP: setsockopt TOS (ignored)");
    }
#endif
#endif
    /*
