...
#ifdef _CENTEC_
    /* added by huwx for snmpv3 usm user delete, 2017-11-22 */
    register_config_handler(app, "delusmUser",
                                  usm_parse_delete_usmUser, NULL, "username");
#endif
...
#ifdef _CENTEC_
struct usmUser *
snmp_get_user_from_list(char *username)
{
    struct usmUser *uptr = userList;
    while (uptr != NULL)
    {  
        if (!strcmp(uptr->secName, username))
        {
            return uptr;
        }
        uptr = uptr->next;
...
void
usm_parse_delete_usmUser(const char *token, char *line)
{
    struct usmUser *uptr;

    if ((uptr = snmp_get_user_from_list(line)) != NULL)
    {
        usm_remove_user(uptr);
    }
    else
    {
        config_perror("Invalid usm user name");
    }
