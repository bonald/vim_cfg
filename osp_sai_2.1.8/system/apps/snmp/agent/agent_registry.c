...
clear_context(void) {
	...
	for (t = ptr->first_subtree; t; t = u) {
            u = t->next;
#ifdef _CENTEC_
        /*add by huwx for bug 45958 current other context tree should not free */
        if (0 != strcmp(ptr->context_name,""))
            continue;
#endif
	    clear_subtree(t);
	}
...
#ifdef _CENTEC_ /* Modified by Alexander 2014-12-10 08:43 */
    if (check_agent_version(pdu->version) != 0) {
        return -1;
    }
#endif /* !_CENTEC_ */
...
#ifdef _CENTEC_ 
void 
add_lookup_cache_by_name(char *context_name)
{
    lookup_cache_context *ptr = NULL;
    
    /* add to cache*/
    ptr = SNMP_MALLOC_TYPEDEF(lookup_cache_context);
    ptr->next = thecontextcache;
    ptr->context = strdup(context_name);
    thecontextcache = ptr;
...
void 
clear_lookup_cache_by_name(char *context_name)
{
    lookup_cache_context *ptr = NULL;
    lookup_cache_context *next = NULL;
    lookup_cache_context *pre = NULL;
        
    ptr = thecontextcache;
    pre = ptr;
    if (ptr->context != NULL && strcmp(ptr->context, context_name) == 0) 
    {
        ptr = ptr->next;
        thecontextcache = ptr;
        SNMP_FREE(pre->context);
        SNMP_FREE(pre);
        return ;
    }
    else
    ...
    while (ptr) 
    {
        next = ptr->next;
        if (ptr->context != NULL &&
            strcmp(ptr->context, context_name) == 0) 
        {
            SNMP_FREE(ptr->context);
            SNMP_FREE(ptr);
            pre->next = next;
...
int
clear_context_by_name(char *context_name) 
{
    subtree_context_cache *ptr = NULL;
    subtree_context_cache *next = NULL;
    subtree_context_cache *pre = NULL;
...
#if 0 /* current tree should not free */        
        if (pre->first_subtree) 
        {
            clear_subtree(pre->first_subtree);
        }
#endif        
        SNMP_FREE(pre->context_name);
        SNMP_FREE(pre);       
        clear_lookup_cache_by_name(context_name);
        return 0;
    }
    else
    {
...
void parse_agent_context_add(const char *token, char *cptr)
{
    subtree_context_cache *ptr = NULL;
    subtree_context_cache *uptr = NULL;

    for (uptr = context_subtrees; uptr != NULL; uptr = uptr->next) 
    {
        if (uptr->context_name != NULL && strcmp(uptr->context_name, "") == 0) 
        {
            break;
        }
    }

    ptr = SNMP_MALLOC_TYPEDEF(subtree_context_cache);
    ...
    ptr->next = context_subtrees;
    ptr->first_subtree = uptr->first_subtree;
    ptr->context_name = strdup(cptr);
    context_subtrees = ptr;  

    add_lookup_cache_by_name(cptr);
    return;

}

void parse_agent_context_del(const char *token, char *cptr)
{
    clear_context_by_name(cptr);
    return;
}
