...
snmp_find_notify_entry(char *notifyname)
{
    struct header_complex_index *hciptr = NULL;
    struct snmpNotifyTable_data *nptr = NULL;

    for (hciptr = snmpNotifyTableStorage; hciptr != NULL; hciptr = hciptr->next)
    {
        nptr = (struct snmpNotifyTable_data *) hciptr->data;
        if (strcmp(nptr->snmpNotifyName, notifyname) == 0)
        {
            break;
        }
    }
...
void
snmp_delete_snmpNotifyTable(const char *token, char *line)
{
    struct snmpNotifyTable_data *StorageDel = NULL;
    struct header_complex_index *hciptr = NULL;

    hciptr = snmp_find_notify_entry(line);
    if (hciptr == NULL)
    {
        return;
    }
    
    StorageDel = (struct snmpNotifyTable_data *) header_complex_extract_entry((struct
                            header_complex_index **) &snmpNotifyTableStorage, hciptr);
...
init_snmpNotifyTable(void)
{
     ...
     * register our config handler(s) to deal with registrations 
     */
    snmpd_register_config_handler("snmpNotifyTableDel", snmp_delete_snmpNotifyTable,
                                  NULL, "notifyname");
#endif
