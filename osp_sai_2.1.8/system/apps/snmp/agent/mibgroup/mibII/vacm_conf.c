...
#ifdef _CENTEC_
    snmpd_register_config_handler("groupdel", vacm_delete_group,
                                  NULL,"name v1|v2c|usm|... security");

    snmpd_register_config_handler("accessdel", vacm_delete_access,
                                  NULL,"name exact|prefix auth|noauth|priv");
#endif  
...
#ifdef _CENTEC_
void
vacm_delete_group(const char *token, char *param)
{
    char group[VACMSTRINGLEN], model[VACMSTRINGLEN], security[VACMSTRINGLEN];
    int imodel = 0;
    struct vacm_groupEntry *gp = NULL;
    ...
    if (strcasecmp(model, "v1") == 0)
        ...
        imodel = SNMP_SEC_MODEL_SNMPv2c;
    else if (strcasecmp(model, "usm") == 0) 
    {
        imodel = SNMP_SEC_MODEL_USM;
    } 
    else if (strcasecmp(model, "any") == 0) 
    {
        config_perror
    ...
    {
        if ((imodel = se_find_value_in_slist("snmp_secmods", model)) == SE_DNE)
        {
            config_perror
                ("bad security model, should be: v1, v2c or usm or a registered security plugin name");
            return;
    ...
    gp = vacm_getGroupEntry(imodel, security);
    if (NULL == gp) 
    {
        config_perror("failed to find group entry");
        return;
    }
    else
    {
        vacm_destroyGroupEntry(imodel, security);
    }
    return;
}

void
vacm_delete_access(const char *token, char *param)
{
    char group[VACMSTRINGLEN], contextpre[VACMSTRINGLEN], level[VACMSTRINGLEN];
    char *st;
    int secleve = 0;

    st = copy_nword(param, group, sizeof(group)-1);
    st = copy_nword(st, contextpre, sizeof(contextpre)-1);
    st = copy_nword(st, level, sizeof(level)-1);
    ...
    if (contextpre[0] == 0) 
    {
        config_perror("missing CONTEXPRE parameter");
        return;
    }
    if (level[0] == 0) 
    {
    ...
    if (strcasecmp(level, "noauth") == 0)
        secleve = SNMP_SEC_LEVEL_NOAUTH;
    else if (strcasecmp(level, "auth") == 0)
        secleve = SNMP_SEC_LEVEL_AUTHNOPRIV;
    else if (strcasecmp(level, "priv") == 0)
        secleve = SNMP_SEC_LEVEL_AUTHPRIV;
    else if (strcasecmp(level, "authpriv") == 0)
        secleve = SNMP_SEC_LEVEL_AUTHPRIV;
    
    if (vacm_getAccessEntry(group, contextpre, SNMP_SEC_MODEL_USM, secleve) == NULL)
    {
        config_perror("failed to find access entry");
        return;
    }
    else
    {
        vacm_destroyAccessEntry(group, contextpre, SNMP_SEC_MODEL_USM, secleve);
    }
    return;
}
#endif
