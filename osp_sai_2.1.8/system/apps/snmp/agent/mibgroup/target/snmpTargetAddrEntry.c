...
init_snmpTargetAddrEntry(void)
{
    ...
    snmpd_register_config_handler("targetAddr",
                                  ...
                                  (void (*)(void))0, NULL);
#ifdef _CENTEC_
    snmpd_register_config_handler("targetAddrdel",
                                  snmpd_parse_del_targetAddr,
                                  (void (*)(void))0, "name");
#endif
...
#ifdef _CENTEC_
struct targetAddrTable_struct *
search_snmpTargetAddrTableByName(char* name)
{
    static struct targetAddrTable_struct *temp_struct = NULL;

    for (temp_struct = aAddrTable; temp_struct != 0; temp_struct = temp_struct->next) 
    {
        if (strcmp(temp_struct->name, name) == 0)
        {
            return temp_struct;
        }
    }
...
void
snmpd_parse_del_targetAddr(const char *token, char *char_ptr)
{
    struct targetAddrTable_struct *tptr = NULL;  

    tptr = search_snmpTargetAddrTableByName(char_ptr);
    if (tptr != NULL)
    {
        snmpTargetAddrTable_remFromList(tptr, &aAddrTable);
    }
    return;  
}
#endif
