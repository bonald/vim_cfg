                                ...
                                netsnmp_table_registration_info *table_info)
{
    ...
    /* modified by yaom for MIB node number skipping (getnext issue) 20161227 */
    if (!table_info->valid_columns && data_set->default_row)
    {
        int column_cnt = 0, i;
        netsnmp_table_data_set_storage *row;
        ...
        if (table_info->valid_columns == NULL)
            return SNMP_ERR_GENERR;
        table_info->valid_columns->isRange = 0;
        table_info->valid_columns->list_count = column_cnt;
        table_info->valid_columns->next = NULL;
        table_info->valid_columns->details.list = (unsigned int*)calloc(column_cnt, sizeof(unsigned int));
        if (table_info->valid_columns->details.list == NULL)
        {
            SNMP_FREE(table_info->valid_columns);
            return SNMP_ERR_GENERR;
        }

        for (i = 0, row = data_set->default_row; row; row = row->next) 
        {
            table_info->valid_columns->details.list[i++] = row->column;
        }
    }
