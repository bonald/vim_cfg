...
/* init sig pipe */
extern int netsnmp_set_non_blocking_mode(int sock, int non_blocking_mode);

void ctc_init_sigpipe(void) 
{
    if (g_sigpipe[0] == -1) {
        pipe(g_sigpipe);
        netsnmp_set_non_blocking_mode(g_sigpipe[0], 1);
        netsnmp_set_non_blocking_mode(g_sigpipe[1], 1);
    }
}

/* write a byte to pipe */
void ctc_notify_main_thread(void)
{
    int ch = 'S';
...
agent_check_and_process(int block)
{
    ...
    int             fakeblock = 0;
    int sigfd;
    //bool is_lock; /* modified by yaom for netsnmp python agent 20170105 */
    ...
    if (count > 0) {
        ...
        /* modified by yaom for netsnmp python agent 20170105 */
        snmp_call_callbacks(SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_USER_FUNC_1, (void*)TRUE);
        snmp_read(&fdset);
        snmp_call_callbacks(SNMP_CALLBACK_LIBRARY, SNMP_CALLBACK_USER_FUNC_1, (void*)FALSE);
    } else
        switch (count) {
        case 0:
            snmp_timeout();
...
/* Added by kcao 2019-02-28, add SNMP debug and stats */
extern unsigned int netsnmp_ctc_rx_count;
extern unsigned int netsnmp_ctc_tx_count;
...
{
    if (result == 0) {
        if (AGENTX_MSG_RESPONSE == pdu->command)
        {
            netsnmp_ctc_tx_count++;
        }
        else
        {
            netsnmp_ctc_rx_count++;
        }
	    ...
	    netsnmp_ds_get_boolean(NETSNMP_DS_APPLICATION_ID, 
            ...
            case AGENTX_MSG_RESPONSE:
                snmp_log(LOG_DEBUG, "  AGENTX RESPONSE message\n");
                break;
