...
#ifdef _CENTEC_ /* Modified by Alexander 2014-12-10 08:28 */
#include "snmpd.h"
#endif /* !_CENTEC_ */
...
#ifdef _CENTEC_ /* Modified by Alexander 2014-12-10 08:23 */
agent_version_e g_agent_version = E_AGENT_VERSION_ALL; 

int check_agent_version(long version)
{
    switch (g_agent_version) {
    case E_AGENT_VERSION_V1:
        if (SNMP_VERSION_1 != version) {
            return -1;
        }
        break;
    case E_AGENT_VERSION_V2C:
        if (SNMP_VERSION_2c != version) {
            return -1;
        }
        break;
    case E_AGENT_VERSION_V3:
        if (SNMP_VERSION_3 != version) {
            return -1;
        }
        break;
    case E_AGENT_VERSION_ALL:
    default:
        break;
    }
...
void parse_agent_version_conf(const char *token, char *cptr)
{
    if (NULL == cptr) {
        snmp_log(LOG_WARNING,
                "please specify snmp agent version\n");
        return;
    }

    if (!strcasecmp(cptr, "all")) {
        g_agent_version = E_AGENT_VERSION_ALL;
    } else if (!strcasecmp(cptr, "v1")) {
        g_agent_version = E_AGENT_VERSION_V1;
    } else if (!strcasecmp(cptr, "v2c")) {
        g_agent_version = E_AGENT_VERSION_V2C;
    } else if (!strcasecmp(cptr, "v3")) {
        g_agent_version = E_AGENT_VERSION_V3;
    } else {
        snmp_log(LOG_WARNING,
                "unknown snmp agent version %s\n", cptr);
    }
    return;
}
#endif /* !_CENTEC_ */
...
{
#ifdef _CENTEC_ /* Modified by Alexander 2014-12-10 08:22 */
    snmpd_register_config_handler("agentversion", 
            parse_agent_version_conf,
            NULL, "agentversion all|v1|v2c|v3");
    snmpd_register_config_handler("addContext", 
            parse_agent_context_add,
            NULL, "name");
    snmpd_register_config_handler("delContext", 
            parse_agent_context_del,
            NULL, "name");
