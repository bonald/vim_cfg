...
static gboolean
log_writer_fd_prepare(GSource * source, gint * timeout)
{
    ...
    if (self->writer->partial ||
        ...
        (self->writer->options->flush_lines > 0
         && num_elements >= self->writer->options->flush_lines))
    {
...
log_writer_fd_check(GSource * source)
{
    ...
    if (self->writer->queue->length || self->writer->partial)
    {
        ...
        if (self->flush_waiting_for_timeout)
        {
            ...
            return self->flush_target.tv_sec <= tv.tv_sec
                || (self->flush_target.tv_sec == tv.tv_sec
                    && self->flush_target.tv_usec <= tv.tv_usec);
        }
    }
    return !!(self->pollfd.
              revents & (G_IO_OUT | G_IO_ERR | G_IO_HUP | G_IO_IN));
}
                       ...
                       GSourceFunc callback, gpointer user_data)
{
    ...
    if (self->pollfd.revents & (G_IO_HUP | G_IO_IN | G_IO_ERR))
    {
#if 0
        /* Deleted by kcao for bug 43771 */
        msg_error("EOF occurred while idle",
                  evt_tag_int("fd", self->fd->fd), NULL);
#endif
...
{
    LogWriterWatch *self =
        (LogWriterWatch *) g_source_new(&log_writer_source_funcs,
                                        sizeof(LogWriterWatch));
    ...
    /* Added by Alexander Liu 2008-08-05 14:03 */
    {
        LogWriterOptions *lopt = writer->options;
        gint cur_size = 0;

        if (lopt && (lopt->options & LWO_SIZE_LIMITATION))
        {
            struct stat filestat;
            if ((writer->flags & LW_FORMAT_FILE) 
                    && fstat(fd->fd, &filestat) == 0)
            {
                cur_size = filestat.st_size;
            }

            lopt->cur_size = cur_size;
        }
    }
    return &self->super;
}
...
log_writer_queue(LogPipe * s, LogMessage * lm, gint path_flags)
{
    ...
    if ((self->queue->length / 2) == self->options->fifo_size)
    {
        ...
        msg_debug("Destination queue full, dropping message",
                  ...
                  evt_tag_int("fifo_size", self->options->fifo_size), NULL);
...
log_writer_format_log(LogWriter * self, LogMessage * lm, GString * result)
{
    ...
    if (template)
    {
        log_template_format(template, lm,
                            ((self->options->
                              options & LWO_TMPL_ESCAPE) ? MF_ESCAPE_RESULT : 0)
                            | (self->options->
                               use_time_recvd ? MF_STAMP_RECVD : 0),
                            self->options->ts_format,
                            self->options->zone_offset,
                            self->options->frac_digits, result);

         /* Added by zhoucheng 2008-03-25 11:04, bug 4741
            append the [n times from XX:XX:XX] suffix for merge */
        g_string_erase(result, result->len-1, 1);
        g_string_append(result, lm->suffix->str);
    ...
    else
    {
        ...
        {
            log_stamp_format(stamp, ts, self->options->ts_format,
                             self->options->zone_offset,
        ...
        else if (self->flags & LW_FORMAT_PROTO)
        {
            ...
            log_stamp_format(stamp, ts, TS_FMT_BSD, self->options->zone_offset,
                             self->options->frac_digits);
...
static inline int
log_writer_size_overflow(LogWriter *self, gint rc)
{
    LogWriterOptions *lopt = self->options;

    if (lopt && (lopt->options & LWO_SIZE_LIMITATION))
    {
        lopt->cur_size += rc;
        if (lopt->cur_size >= lopt->max_size)
        {
            return 1;
        }
    }
...
log_writer_flush_log(LogWriter * self, FDWrite * fd)
{
    ...
    if (self->partial)
    {
        ...
        else if (rc != len)
        {
            ...
            /* Added by Alexander Liu 2008-08-05 10:59 */
            if (log_writer_size_overflow(self, rc))
            {
                log_pipe_notify(self->control, &self->super, NC_SIZE_OVERFLOW, self);
            }
            return TRUE;
        }
    ...
    while (!g_queue_is_empty(self->queue))
    {
        ...
        path_flags =
            GPOINTER_TO_UINT(g_queue_pop_head(self->queue)) & 0x7FFFFFFF;
  ...
  write_error:
    {
        ...
        {
#if 0  /* modified by yanxa, for bug 14555, 2011-04-27 13:20 */
            msg_error("I/O error occurred while writing",
                      evt_tag_int("fd", fd->fd),
                      evt_tag_errno(EVT_TAG_OSERROR, errno), NULL);
#endif
...
log_writer_init(LogPipe * s, GlobalConfig * cfg, PersistentConfig * persist)
{
    ...
    if ((self->options->flags & LWOF_NO_STATS) == 0 && !self->dropped_messages)
        stats_register_counter(SC_TYPE_DROPPED, self->options->stats_name,
                               &self->dropped_messages,
                               !!(self->options->flags & LWOF_SHARE_STATS));
    return TRUE;
}
...
log_writer_deinit(LogPipe * s, GlobalConfig * cfg, PersistentConfig * persist)
{
    ...
    if (self->dropped_messages)
        stats_orphan_counter(SC_TYPE_DROPPED, self->options->stats_name,
                             &self->dropped_messages);
...
void
log_writer_options_set_template_escape(LogWriterOptions * options,
                                       gboolean enable)
{
    ...
    {
        msg_notice("Macro escaping can only be specified for inline templates",
                   NULL);
    }
}

void
log_writer_options_init(LogWriterOptions * options, GlobalConfig * cfg,
                        guint32 flags, const gchar * stats_name)
{
    ...
    if (options->fifo_size < options->flush_lines)
    {
        msg_error("The value of flush_lines must be less than fifo_size",
                  ...
                  evt_tag_int("flush_lines", options->flush_lines), NULL);
