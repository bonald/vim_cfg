...
/* Modified by zhoucheng for log merge */
%token KW_MERGE_ENABLE KW_MERGE_FIFO_SIZE KW_MERGE_TIMEOUT 

/* Added by zhoucheng 2008-01-03 13:43, bug 3973 */
%token KW_MERGE_TIMEFROM
...
%token KW_TEMPLATE KW_TEMPLATE_ESCAPE KW_TEMPLATE_CTCFAC
...
/* dest output size limitation */
%token KW_SIZE_LIMITATION
%token KW_SYNCSCRIPT
%token  DOTDOT
...
template_item
    ...
    | KW_TEMPLATE_CTCFAC '(' NUMBER ')' { log_template_set_ctcfac(last_template, $3); }
	;
...
*/
	| KW_SIZE_LIMITATION '(' NUMBER ')'		{ affile_dd_set_size_limitation(last_driver, $3); }
	| KW_SYNCSCRIPT '(' string ')'		{ affile_dd_set_sync_script(last_driver, $3); free($3); } /*for bug 19965, memory leak*/
...
options_item
	...
	| KW_MERGE_ENABLE '(' yesno ')'	{ configuration->merge_enable = $3; } 
	| KW_MERGE_FIFO_SIZE '(' NUMBER ')'	{ configuration->merge_fifo_size = $3; }
	| KW_MERGE_TIMEOUT '(' NUMBER ')'	{ configuration->merge_timeout = $3; }

    /* Added by zhoucheng 2008-01-03 13:44, bug 3974 */
	| KW_MERGE_TIMEFROM '(' yesno ')'	{ configuration->merge_timefrom = $3; }
	;
