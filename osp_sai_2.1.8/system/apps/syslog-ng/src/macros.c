...
#include <stdio.h>
#include <stdlib.h>
/*For Stacking logging*/
gint
log_macro_stack_member()
{
#define GLB_SLOT_NO_FILE "/etc/slot_no"
...
#define GLB_STACK_MASTER_SLOT_FILE "/tmp/stack_master_slot"
    ...
    fp = fopen(GLB_STACK_MASTER_SLOT_FILE, "r");
    if (fp)
    {
        fgets (buf, BUFSIZ, fp);
        master = atoi(buf);
...
log_macro_expand(GString *result, gint id, guint32 flags, gint ts_format, glong zone_offset, gint frac_digits, LogMessage *msg)
{
  ...
  switch (id)
    ...
    case M_LEVEL_NUM:
      {
#ifdef _CENTEC_     
        gint member = log_macro_stack_member();
        gchar buf[128];
        if (member)
        {
            snprintf(buf, 128, "stack%d", member);
            result_append(result, buf, strlen(buf), !!(flags & MF_ESCAPE_RESULT));
        }
        else
#endif            
        g_string_sprintfa(result, "%d", (msg->pri & LOG_PRIMASK));
    ...
    case M_CTCPRI:
      {
        g_string_sprintfa(result, "%d", LOG_PRI(msg->pri));
        break;
      }
    /* Added by kcao 2015-01-20 for bug 31672, support log brief */
    case M_CTCBRIEF:
      {
        /* brief */
        if (msg->brief->len)
          {
            result_append(result, msg->brief->str, msg->brief->len, !!(flags & MF_ESCAPE_RESULT));
          }
        break;
      }
    default:
