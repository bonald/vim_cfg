...
afsocket_setup_socket(gint fd, SocketOptions *sock_options, AFSocketDirection dir)
{
  if (dir & AFSOCKET_DIR_RECV)
    {
#ifdef _CENTEC_ /* Modified by zhoucheng 2008-01-17 19:02, bug 3973 */
#define RCV_BUF_SIZE (32*1024)
      gint rcv_buf_size = RCV_BUF_SIZE;
      setsockopt(fd, SOL_SOCKET, SO_RCVBUF, &rcv_buf_size, sizeof(sock_options->rcvbuf));
#else
...
afsocket_open_socket(GSockAddr *bind_addr, int stream_or_dgram, int *fd)
{
  ...
  char buf[128];
  snprintf(buf, sizeof(buf), "afsocket_open_socket");
...
#ifdef _CENTEC_ 
  if (self->super.pipe_next == self->reader)
  {
    self->super.pipe_next = self->reader->pipe_next;
  }
#endif
...
afsocket_dd_reconnect(AFSocketDestDriver *self)
{
  ...
  if (rc == G_IO_STATUS_NORMAL)
    ...
    {
#ifdef _CENTEC_  /*modified by weij for bug 6655*/
        if(ENOENT != errno)
        {
          if (AF_INET == self->dest_addr->sa.sa_family)
          {
            g_inet_ntoa(ip_add, sizeof(ip_add), 
                ((struct sockaddr_in *) &self->dest_addr->sa)->sin_addr);
          }
          else if (AF_INET6 == self->dest_addr->sa.sa_family)
          {
            addr6 = g_sockaddr_inet6_get_sa(self->dest_addr);
            inet_ntop(AF_INET6, &addr6->sin6_addr, ip_add, sizeof(ip_add));
          }
          /*for bug 17899 modified by liuyang 2012-2-23*/
          if (AF_INET == self->dest_addr->sa.sa_family || AF_INET6 == self->dest_addr->sa.sa_family)
          {
            snprintf(buf, sizeof(buf), "Connecting to %s", ip_add);
            msg_error(buf,
                    evt_tag_errno(EVT_TAG_OSERROR, errno),
                    evt_tag_int("reconnect", self->time_reopen),
                    NULL);
      ...
      self->reconnect_timer = g_timeout_add(self->time_reopen * 1000, afsocket_dd_reconnect_timer, self);
#if 1
      /* Modified by Alexander Liu 2006-12-07 
       * We need startup syslog-ng before IMI and other PMs
       * So we need ignore this error and let it reconnect later.
       */
      return TRUE;
#else
      return FALSE;
...
{
  AFSocketDestDriver *self = (AFSocketDestDriver *) s;
#ifdef _CENTEC_ /*added by yanxa, for bug 14555*/
      gchar ip_add_buf[100];
      gchar buf[100];
#endif /* !_CENTEC_ */
...
#ifdef _CENTEC_ /*added by yanxa, for bug 14555*/
       /*for bug 17899 modified by liuyang 2012-2-23*/
      snprintf(buf, sizeof(buf), "Connection %s broken", 
                g_sockaddr_format(self->dest_addr, ip_add_buf, sizeof(ip_add_buf)));
      msg_error(buf,
                evt_tag_int("time_reopen", self->time_reopen),
                NULL);
#else
