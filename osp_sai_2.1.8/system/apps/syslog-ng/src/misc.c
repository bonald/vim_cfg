...
#ifdef _CENTEC_ /* Modified by zhoucheng 2008-01-18 17:44, bug 4185 */
extern gboolean sig_usr1_received;
#endif /* !_CENTEC_ */
...
resolve_hostname(GString *result, GSockAddr *saddr, gboolean usedns, gboolean usefqdn)
{
  ...
  if (saddr && saddr->sa.sa_family != AF_UNIX)
      ...
      if (saddr->sa.sa_family == AF_INET || saddr->sa.sa_family == AF_INET6)
        {
          ...
          if (!hname) 
            {
              /* Added by Alexander Liu 2008-11-12 16:41 */
              if (saddr->sa.sa_family == AF_INET)
              {
                  unsigned char *uc = (unsigned char *)addr; /* network byte */
                  if (uc[0] == 0xAC && uc[1] == 0x10) /* 172.16 */
                  {
                      gethostname(buf, sizeof(buf));
                      /* sprintf(buf + strlen(buf), "/LineCard%d", uc[2] - 2); */
                  }
                  else
                  {
    ...
    {
    /* Modified by zhoucheng 2008-01-18 17:48, bug 4185 */
      if (sig_usr1_received || !local_hostname[0]) 
	{
