...
log_center_init(LogPipe *s, GlobalConfig *cfg, PersistentConfig *persist)
{
    ...
    gint i, j;
#ifdef HAVE_MERGE
    LogMergerOptions *mgopt = NULL; /*Added by zhoucheng for log merge */
#endif  
    self->cfg = cfg;
    ...
    if (cfg->merge_enable)
    {
        self->state = LC_STATE_INIT_MERGER;
        mgopt = log_merger_options_new();
        log_merger_options_init(mgopt, cfg, 0, "");
        
        /* log_merger_options_defaults(mgopt); */
        cfg->merger = (LogMerger*)log_merger_new(0, self,  mgopt);
        log_merger_init(&cfg->merger->super, cfg, persist);
    }
#endif
...
log_center_deinit(LogPipe *s, GlobalConfig *cfg, PersistentConfig *persist)
{
    ...
    if (cfg->merge_enable)
    {
        log_merger_deinit(&cfg->merger->super, cfg,  persist);
        log_merger_free(&cfg->merger->super);
        cfg->merger = NULL;
    }
 #endif
...
#ifdef HAVE_MERGE
static void
log_center_fetch( LogPipe *s, LogMessage *lm, gint path_flags)
{
    ...
    log_msg_ref(lm);
    log_msg_ack_block_start(lm, log_center_ack, NULL);
    ...
    for (match = 0, fallbacks = 0; !match && have_fallbacks && (fallbacks <= 1); fallbacks++)
    {
        ...
        for (ci = 0; ci < self->cfg->connections->len; ci++)
        {
            ...
            {
                /* check source */
                if (!g_hash_table_lookup(conn->source_cache, lm->source_group->name->str))
                {
            ...
            for (di = 0; di < conn->destinations->len; di++)
            {
                ...
                if (conn->flags & LC_FLOW_CONTROL)
                    log_msg_ack_block_inc(lm);
              
                dest = (LogDestGroup *) ep->ref;
                log_pipe_queue(&dest->super, log_msg_ref(lm), path_flags | ((conn->flags & LC_FLOW_CONTROL) ? 0 : PF_FLOW_CTL_OFF));
...
log_center_queue(LogPipe *s, LogMessage *msg, gint path_flags)
{
   ...
   /*
     * Added by zhoucheng for log merge,
     * Forward every messsage to log merge buffer, 
     * instead of directly to the destinations.
     */
    if (self->cfg->merge_enable)
    {   /*add by weij for bug 6558*/
        afinter_postpone_mark(self->cfg->mark_freq);
        log_pipe_queue(&self->cfg->merger->super, msg, path_flags);
    }
    else
    {
        log_center_fetch(s, msg, path_flags);
    }
#else
     afinter_postpone_mark(self->cfg->mark_freq);
