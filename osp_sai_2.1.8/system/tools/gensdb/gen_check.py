...
filename_input_cdb_ds_c	= '../../sdb/core/src/sdb_ds.c'
filename_input_cmp_c	= '../../sdb/core/src/sdb_data_cmp.c'
...
def parse_memory_h_file(memory_h_array, input_file):
	...
	for line_no, eachLine in enumerate(file_in):
		if ('MEM_TBL_' in eachLine or 'MEM_DS_' in eachLine or 'MEM_STBL_' in eachLine):
...
def parse_cdb_ds_c_file_referlist(referlist_val2str_array, input_file):
	...
	func_range = 0
	for line_no, eachLine in enumerate(file_in):
		if ('sdb_reference_list_val2str' in eachLine):
			func_range = 1
			if ('}' == eachLine[0]):
				func_range = 0		
		if (func_range):
			if ('case STBL_' in eachLine):
...
def check_memory_name_in_array(memory_name, memory_h_array, memory_c_array):
	...
	for i, memory in enumerate(memory_h_array):
		if (memory_name == memory):
			...
			break
	if (0 == is_found):
		print "%s: 111 not found %s in ctclib_memory.h" % (cdb_util.inred('FAIL'), cdb_util.ingreen(memory_name))
		glb_pass = 0
...
for i, tbl_one in enumerate(api_tbl_array):
	...
	for j, field in enumerate(data_array):
		if ('REFERLIST_STBL_' in field[0]):
			...
			if (None == val2str):
				print "%s: %s not found case %s in function sdb_reference_list_val2str()" % (cdb_util.inred('FAIL'), cdb_util.ingreen(tbl_name), cdb_util.ingreen(referlist_tbl_name))
				glb_pass = 0
...
for i, cmp_tbl in enumerate(need_cmp_array):
	...
	if (None == ret_data):
		print "%s: not found function %s in sdb_data_cmp.c" % (cdb_util.inred('FAIL'), cdb_util.ingreen(dt_cmp_func))
		glb_pass = 0
...
for i, val2str_func in enumerate(oper_val2str_array):
	...
	if (None == ret_data):
		if ('ctc_task_val2str' != val2str_func):
			print "%s: not found function %s in sdb_ds.c" % (cdb_util.inred('FAIL'), cdb_util.ingreen(val2str_func))
			glb_pass = 0
