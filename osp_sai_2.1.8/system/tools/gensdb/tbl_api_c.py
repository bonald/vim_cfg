...
#		print >> fobj, "    int32 rc = SDB_E_NONE;"
	...
	print >> fobj, "    {"
	print >> fobj, "        return SDB_E_NO_MEMORY;"
	print >> fobj, "    }"
	print >> fobj, ""
	...
	for i, referlist_field in enumerate(referlist_array):
		...
		print >> fobj, "    sdb_reference_list_init(&%s->%s, %s, %s, %s);" % (dbvalue, object_info[0], object_info[1], object_info[2], cmp_func)
		cmp_func_array.append(cmp_func)
...
#	print >> fobj, ""
	...
	for i, referlist_field in enumerate(referlist_array):
		...
		print >> fobj, "    sdb_reference_list_deinit(&%s->%s);" % (dbvalue, field_name)
	print >> fobj, ""
...
def gen_tbl_func_set_implement(fobj, table, key_array, master_array, is_global=0):
	...
	if (is_global):
		...
		print >> fobj, "    if (NULL == %s)" % (dbvalue)
		print >> fobj, "    {"
		print >> fobj, "        return SDB_E_NOT_EXIST;"
		print >> fobj, "    }"
	print >> fobj, ""
...
def gen_tbl_func_set_field_evaluate(fobj, field, tbl_name_upper, dbvalue, paramvalue):
	...
	if ('DS_' in dt_type[1]):
		return
	if ('sdb_reference_list_t' == dt_type[1]):
		return
	if ('key' == field[1]):
		return
	...
	if ('UINT' == dt_type[1] or 'UINT64' == dt_type[1] or 'INT' == dt_type[1] or 'INT64' == dt_type[1]):
		...
		print >> fobj, "        %s->%s = %s->%s;" % (dbvalue, field_name, paramvalue, field_name)
	elif ('sai_object_id_t' == dt_type[1]):
		print >> fobj, "        %s->%s = %s->%s;" % (dbvalue, field_name, paramvalue, field_name)
	else:
		if ('memcpy' == dt_info[1]):
...
def gen_tbl_func_set_field_sync_implement(fobj, table, key_array, field_array, is_global=0):
	...
	print >> fobj, "        break;"
	print >> fobj, "    default:"
	print >> fobj, "        return SDB_E_NOT_SUPPORT;"
	print >> fobj, "    }"
	print >> fobj, ""
...
def gen_tbl_func_iterate_implement(fobj, table, key_array, data_array, master_array, is_global=0):
	...
	param1_type = 'STBL_ITER_CB_FUNC'
	param1_value = 'fn'
	param2_type = 'stbl_iter_args_t'
	param2_value = 'pargs'
	...
	if (is_global):
		...
		print >> fobj, "    return (rc) ? SDB_E_FAIL : SDB_E_NONE;"
	else:
		print >> fobj, "    %s *%s = %s;" % (tbl_mst_type, mst_name, tbl_g_mst_name)
		if ('USERDEFINE' == tbl_db_type):
			...
			print >> fobj, "    }"
			print >> fobj, ""
			print >> fobj, "    ctclib_hash_iterate2(p_master->%s, %s, %s, %s);" % (tbl_hash_name, 'stbl_hash_iter_adpt_fn', param1_value, param2_value)
			print >> fobj, "    return SDB_E_NONE;"
...
def gen_tbl_func_key_val2str_implement(fobj, table, key_array, field_array, ds_array, dt_array, val2str_func_array, referlist_tbl_array):
	...
	for i, field in enumerate(field_array):
		...
		if (has_val2str):
			...
			print >> fobj, "    char buf[SDB_MAX_CMD_STR_LEN];"
			print >> fobj, ""
			...
			print >> fobj, "    sal_snprintf(str, str_len, \"%s\", sdb_enum_val2str(%s, %s, %s));" % (field_name_string, enum_type_str, enum_type_max, param_str)
		else:
...
def gen_tbl_func_key_str2val_implement(fobj, table, key_array, field_array, ds_array, dt_array, val2str_func_array,
	...
	for i, field in enumerate(field_array):
		...
		if (has_val2str):
			...
			print >> fobj, ""
			if ('sdb_bitmap_val2str' == func_val2str):
				print >> fobj, "    char err[SDB_MAX_CMD_STR_LEN];"
...
def gen_tbl_func_dump_one_implement(fobj, table, key_array, field_array, ds_array, dt_array, val2str_func_array, referlist_tbl_array, array_ds_array):
	...
	bak_field_id = 'bak_field_id'
	listnode = 'listnode'
	funcstr = "%s_dump_one(%s %s, %s %s)" % (tbl_name, paramtype, '*' + paramvalue, 'stbl_iter_args_t', '*' + pargs_value)
	print >> fobj, "int32"
	print >> fobj, "%s" % (funcstr)
	print >> fobj, "{"

	print >> fobj, "    sdb_node_t *%s = sdb_get_tbl(%s);" % (node_value, tbl_name_upper)
	print >> fobj, "    field_parse_t *%s = %s->argv[0];" % (field_value, pargs_value)
	print >> fobj, "    FILE *%s = %s->argv[1];" % (fp_value, pargs_value)
	print >> fobj, "    sdb_tbl_info_t *%s = %s->tbl_info;" % (tbl_info_value, node_value)
	print >> fobj, ""
	...
	for i, referlist_field in enumerate(referlist_array):
		...
		if ('SDB_NODE_DS' == refer_object_info[1]):
			refer_object_array.append(string.lower(refer_object_info[2]))
		elif ('SDB_NODE_TBL' == refer_object_info[1]):
			has_buf2 = 1
	...
	for i, field in enumerate(field_array):
		...
		print >> fobj, "    if (SDB_FLD_MATCH(%s, %s->field_id[0]))" % (field_id_enum, field_value)
		print >> fobj, "    {"
		field_name_string = """/%s"""
		if (None != dt_type[2]):
			field_value_string = dt_type[2][2]
		...
		if (has_val2str):
			func_val2str = dt_type[2][4]
			val2str_func_array.append(func_val2str)
			if ('sdb_bitmap_val2str' == func_val2str or 'sdb_int32_array_val2str' == func_val2str or 'sdb_uint32_array_val2str' == func_val2str or 'sdb_uint64_array_val2str' == func_val2str):
			...
			elif ('sdb_int8_array_val2str' == func_val2str or 'sdb_uint8_array_val2str' == func_val2str):
				print >> fobj, "        sal_fprintf(fp, \"%s\"CMD_EQUAL_STR\"%s\", %s->field[%s].name, " % (field_name_string, field_value_string, tbl_info_value, field_id_enum)
			...
			enum_type_max = string.upper(enum_type) + '_MAX'
			print >> fobj, "        sal_fprintf(fp, \"%s\"CMD_EQUAL_STR\"%s\", %s->field[%s].name, " % (field_name_string, field_value_string, tbl_info_value, field_id_enum)
			print >> fobj, "            sdb_enum_val2str(%s, %s, %s));" % (enum_type_str, enum_type_max, param_str)
		elif ('STRING' == dt_type[1]):
			print >> fobj, "        sal_fprintf(fp, \"%s\"CMD_EQUAL_STR\"%s\", %s->field[%s].name," % (field_name_string, field_value_string, tbl_info_value, field_id_enum)
			print >> fobj, "            %s);" % (param_str)
		elif ('FLAGS8' == dt_type[1] or 'FLAGS32' == dt_type[1]):
			...
			print >> fobj, "        }"
			print >> fobj, "        sal_fprintf(fp, \"%s\");" % ('}')
		elif ('sdb_reference_list_t' in dt_type[1]):
			if ('REFERLIST_DS_' in field[0]):
				...
				print >> fobj, "        }"
				print >> fobj, "        sal_fprintf(fp, \"%s\");" % ('}')
			elif ('REFERLIST_STBL_' in field[0]):
				name_upper = refer_object_info[2]
				referlist_tbl_array.append(name_upper)
				param_str = '&' + param_str
				func_val2str = 'sdb_reference_list_val2str'
				val2str_func_array.append(func_val2str)
			...
			print >> fobj, "        sal_fprintf(fp, \"/%s\"CMD_EQUAL_STR\"%s\","
			print >> fobj, "            %s->field[%s].name," % (tbl_info_value, field_id_enum)
			print >> fobj, "            ctc_task_val2str(buf, SDB_MAX_CMD_STR_LEN, %s));" % (param_str)
		elif ('DOUBLE' == dt_type[1]):
			print >> fobj, "        sal_fprintf(fp, \"%s\"CMD_EQUAL_STR\"%s\", %s->field[%s].name," % (field_name_string, field_value_string, tbl_info_value, field_id_enum)
			print >> fobj, "            %s);" % (param_str)
		else:
...
def gen_tbl_func_key_name_dump_implement(fobj, table, key_array, field_array, ds_array, dt_array, val2str_func_array,
	...
	print >> fobj, "    sdb_node_t *p_node = sdb_get_tbl(%s);" % (tbl_name_upper)
	if (is_global):
	...
	for i, field in enumerate(field_array):
		...
		if (has_val2str):
			...
			enum_type_max = string.upper(enum_type) + '_MAX'
			print >> fobj, "    sal_sprintf(str, \"%s\"KEY_CONNECT_STR"
			print >> fobj, "        \"%s\", p_node->name, sdb_enum_val2str(%s, %s, %s));" % (
...
def gen_tbl_func_key_value_dump_implement(fobj, table, key_array, field_array, ds_array, dt_array, val2str_func_array,
	...
	for i, field in enumerate(field_array):
		...
		if (has_val2str):
			...
			enum_type_str = enum_type + '_strs'
			enum_type_max = string.upper(enum_type) + '_MAX'
			print >> fobj, "    sal_sprintf(str, \"%s\", sdb_enum_val2str(%s, %s, %s));" % (
			field_name_string, enum_type_str, enum_type_max, param_str)
		else:
			cdb_util.error_return("unhandled field in TBL key_val2str: %s, dt_type: %s", field, dt_type[1])
	print >> fobj, "    return str;"
...
def gen_tbl_func_field_name_dump_implement(fobj, table, key_array, field_array, ds_array, dt_array, val2str_func_array,
	...
	print >> fobj, ""
	print >> fobj, "    sdb_node_t *p_node = sdb_get_tbl(%s);" % (tbl_name_upper)
	print >> fobj, "    sdb_tbl_info_t *p_tbl_info = p_node->tbl_info;"
...
def gen_tbl_func_field_value_dump_implement(fobj, table, key_array, field_array, ds_array, dt_array, val2str_func_array,
	...
	for i, field in enumerate(field_array):
		...
		print >> fobj, "    if (SDB_FLD_MATCH(%s, field_id))" % (field_id_enum)
		print >> fobj, "    {"
...
def gen_tbl_func_table_parser_implement(fobj, table, key_array, field_array, ds_array, dt_array, val2str_func_array,
	...
	print >> fobj, "    sdb_tbl_info_t *p_tbl_info = p_node->tbl_info;"
	print >> fobj, ""
	print >> fobj, "    %s_key_str2val(key_value, %s);" % (tbl_name, paramvalue)
	print >> fobj, ""
	print >> fobj, "    for(i=1; i<%s_FLD_MAX; i++)" % (tbl_name_upper)
	print >> fobj, "    {"
	print >> fobj, "        if(sal_strncmp(array[j++], p_tbl_info->field[i].name, sal_strlen(p_tbl_info->field[i].name)))"
	print >> fobj, "        {"
	print >> fobj, "             return SDB_E_INVALID_PARAM;"
	print >> fobj, "        }"
	print >> fobj, "        %s_field_value_parser( array[j++], i, %s);" % (tbl_name, paramvalue)
	print >> fobj, "    }"
	print >> fobj, ""
	print >> fobj, "    return SDB_E_NONE;"
	print >> fobj, "}"
...
#	print >> fobj, "    cdb_sync_refer_hdr_t refer;"
	...
	print >> fobj, "    {"
	print >> fobj, "        sdb_reference_list_add_object(&%s->%s, %s);" % (dbvalue, field_name, p_object)
	print >> fobj, "    }"
	print >> fobj, "    else"
	print >> fobj, "    {"
	print >> fobj, "        sdb_reference_list_del_object(&%s->%s, %s);" % (dbvalue, field_name, p_object)
	print >> fobj, "    }"
	print >> fobj, ""
	print >> fobj, "    return SDB_E_NONE;"
	print >> fobj, "}"
