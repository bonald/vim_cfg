...
def gen_tbl_dump_one_func_prototype(fobj, table):
	...
	paramtype = tbl_name + '_t'
	paramvalue = '*' + 'p_' + tbl_value_name
	funcstr = "%s_dump_one(%s %s, %s %s)" % (tbl_name, paramtype, paramvalue, 'stbl_iter_args_t', '*pargs')
	print >> fobj, "int32"
	print >> fobj, "%s;" % (funcstr)
	print >> fobj, ""
...
def gen_tbl_iterate_func_prototype(fobj, table):
	tbl_name = string.lower(table[0])
	funcstr = "%s_iterate(%s %s, %s %s)" % (tbl_name, 'STBL_ITER_CB_FUNC', 'fn', 'stbl_iter_args_t', '*pargs')
	print >> fobj, "int32"
	print >> fobj, "%s;" % (funcstr)
	print >> fobj, ""
