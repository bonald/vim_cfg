...
/* current vty for execute this command */
struct vty *g_curVty = NULL;
...
/* Added by Alexander 2012-01-17 10:34 */
static afx_mio_t *g_pMainMio = NULL; /* main mio */
...
int
vtysh_vty_out (struct vty *vty, const char *format, ...)
{
  va_list args;
  int len = 0;
  int size = 1024;
  ...
  if (vtysh_vty_shell (vty))
	...
	p = buf;
      ...
      if (vtysh_vty_shell_serv (vty))
	write (vty->fd, (u_char *) p, len);
      else
	vtysh_buffer_write (vty->obuf, (u_char *) p, len);
...
void
vtysh_vty_time_print (struct vty *vty, int cr)
{
  int ret;
  char buf [TIME_BUF];
...
vty_prompt (struct vty *vty)
{
  ...
  if (vty->type == VTY_TERM)
      ...
      if (!hostname)
	{
      ...
      vtysh_vty_out (vty, cmd_prompt (vty->node), hostname);
    }
}

/* Send WILL TELOPT_ECHO to remote server. */
void
vtysh_vty_will_echo (struct vty *vty)
{
  char cmd[] = { IAC, WILL, TELOPT_ECHO, '\0' };
  vtysh_vty_out (vty, "%s", cmd);
}
...
vtysh_vty_new ()
{
  ...
  new->obuf = (struct buffer *) vtysh_buffer_new (100);
  ...
  if (! fail)
      ...
      if (vty->fail >= 3)
	{
	  if (vty->node == AUTH_NODE)
	    {
	      vtysh_vty_out (vty, "%% Bad passwords, too many failures!%s", VTY_NEWLINE);
	  ...
	  else			
	    {
	      /* AUTH_ENABLE_NODE */
	      vty->fail = 0;
	      vtysh_vty_out (vty, "%% Bad enable passwords, too many failures!%s", VTY_NEWLINE);
	      vty->node = VIEW_NODE;
	    }
	}
    }
...
vtysh_vty_command (struct vty *vty, char *buf)
{
  ...
  if (ret != CMD_SUCCESS)
	...
	if (vty->type == VTY_FILE)
	  vtysh_vty_out (vty, "Warning...%s", VTY_NEWLINE);
	break;
      case CMD_ERR_AMBIGUOUS:
	vtysh_vty_out (vty, "%% Ambiguous command.%s", VTY_NEWLINE);
	break;
      case CMD_ERR_NO_MATCH:
	vtysh_vty_out (vty, "%% Unknown command.%s", VTY_NEWLINE);
	break;
      case CMD_ERR_INCOMPLETE:
	vtysh_vty_out (vty, "%% Command incomplete.%s", VTY_NEWLINE);
	break;
      }
...

char vtysh_telnet_backward_char = 0x08;
char vtysh_telnet_space_char = ' ';
...
vty_write (struct vty *vty, char *buf, size_t nbytes)
{
  ...
  vtysh_buffer_write (vty->obuf, (u_char *)buf, nbytes);
}
#endif
...
vty_self_insert (struct vty *vty, char c)
{
  ...
  for (i = 0; i < length; i++)
    vty_write (vty, &vtysh_telnet_backward_char, 1);
...
{
  vtysh_vty_out (vty, "%s", VTY_NEWLINE);
...
void
vtysh_vty_end_config (struct vty *vty)
{
  vtysh_vty_out (vty, "%s", VTY_NEWLINE);
  ...
  switch (vty->node)
    ...
    case CTC_SDK_OAM_CHAN_MODE:
      ...
      break;
    default:
      vtysh_vty_out(vty, "%% Unknown mode: %d, please process it\r\n", vty->node); 
...
vty_delete_char (struct vty *vty)
{
  ...
  vty_write (vty, &vtysh_telnet_space_char, 1);
...
vty_complete_command (struct vty *vty)
{
  ...
  switch (ret)
    ...
    case CMD_ERR_AMBIGUOUS:
      ...
      break;
    case CMD_ERR_NO_MATCH:
      /* vtysh_vty_out (vty, "%% There is no matched command.%s", VTY_NEWLINE); */
      vty_prompt (vty);
      vty_redraw_line (vty);
      break;
      ...
      for (i = 0; matched[i] != NULL; i++)
	{
	  if (i != 0 && ((i % 6) == 0))
	    vtysh_vty_out (vty, "%s", VTY_NEWLINE);
	  vtysh_vty_out (vty, "%-10s ", matched[i]);
		   ...
		   int desc_width, struct desc *desc)
{
  ...
  if (desc_width <= 0)
    {
      vtysh_vty_out (vty, "  %-*s  %s%s", cmd_width, cmd, desc->str, VTY_NEWLINE);
      return;
    }
  ...
  for (p = desc->str; strlen (p) > desc_width; p += pos + 1)
	...
	break;
      ...
      strncpy (buf, p, pos);
      buf[pos] = '\0';
      vtysh_vty_out (vty, "  %-*s  %s%s", cmd_width, cmd, buf, VTY_NEWLINE);

      cmd = "";
    }

  vtysh_vty_out (vty, "  %-*s  %s%s", cmd_width, cmd, p, VTY_NEWLINE);
...
vty_describe_command (struct vty *vty)
{
  ...
  for (i = 0; i < vector_max (describe); i++)
	...
	if (!desc->str)
	  vtysh_vty_out (vty, "  %-s%s",
		   desc->cmd[0] == '.' ? desc->cmd + 1 : desc->cmd,
		   VTY_NEWLINE);
	else if (desc_width >= strlen (desc->str))
	  vtysh_vty_out (vty, "  %-*s  %s%s", width,
		   desc->cmd[0] == '.' ? desc->cmd + 1 : desc->cmd,
...
void
vtysh_vty_clear_buf (struct vty *vty)
{
...
vty_telnet_option (struct vty *vty, unsigned char *buf, int nbytes)
{
  ...
  for (i = 0; i < nbytes; i++)
	...
	{
	case IAC:
	  vtysh_vty_out (vty, "IAC ");
	  break;
	case WILL:
	  vtysh_vty_out (vty, "WILL ");
	  break;
	case WONT:
	  vtysh_vty_out (vty, "WONT ");
	  break;
	case DO:
	  vtysh_vty_out (vty, "DO ");
	  break;
	case DONT:
	  vtysh_vty_out (vty, "DONT ");
	  break;
	case SB:
	  vtysh_vty_out (vty, "SB ");
	  break;
	case SE:
	  vtysh_vty_out (vty, "SE ");
	  break;
	case TELOPT_ECHO:
	  vtysh_vty_out (vty, "TELOPT_ECHO %s", VTY_NEWLINE);
	  break;
	case TELOPT_SGA:
	  vtysh_vty_out (vty, "TELOPT_SGA %s", VTY_NEWLINE);
	  break;
	case TELOPT_NAWS:
	  vtysh_vty_out (vty, "TELOPT_NAWS %s", VTY_NEWLINE);
	  break;
	default:
	  vtysh_vty_out (vty, "%x ", buf[i]);
	  break;
	}
    }
  vtysh_vty_out (vty, "%s", VTY_NEWLINE);
  ...
  switch (buf[0])
    {
    case SB:
      vtysh_buffer_reset(vty->sb_buffer);
...
#endif
    default:
      ret = vtysh_vty_command (vty, vty->buf);
      if (vty->type == VTY_TERM)
	vty_hist_add (vty);
      break;
    }
...
vty_read (struct thread *thread)
{
  ...
  for (i = 0; i < nbytes; i++) 
	...
	{
	  vtysh_buffer_putc(vty->sb_buffer, buf[i]);
	  continue;
	}
...
vty_flush (struct thread *thread)
{
  ...
  if (vty->status == VTY_START || vty->status == VTY_CONTINUE)
	...
	erase = 0;
      ...
      if (vty->lines == 0)
	vtysh_buffer_flush_window (vty->obuf, vty->fd, vty->width, 25, 0, 1);
      else if (vty->status == VTY_MORELINE)
	vtysh_buffer_flush_window (vty->obuf, vty->fd, vty->width, 1, erase, 0);
      else
	vtysh_buffer_flush_window (vty->obuf, vty->fd, vty->width,
			     vty->lines >= 0 ? vty->lines : vty->height,
			     erase, 0);
  
      if (vtysh_buffer_empty (vty->obuf))
	{
	  if (vty->status == VTY_CLOSE)
	    vtysh_vty_close (vty);
	  else
...
vty_create (int vty_sock, union sockunion *su)
{
  ...
  vty->hp = 0;
  vty->hindex = 0;
  vtysh_vector_set_index (vtyvec, vty_sock, vty);
  vty->status = VTY_NORMAL;
  ...
  vty->iac = 0;
  vty->iac_sb_in_progress = 0;
  vty->sb_buffer = vtysh_buffer_new (1024);
  ...
  if (! no_password_check)
	...
	{
	  vtysh_vty_out (vty, "Vty password is not set.%s", VTY_NEWLINE);
	  vty->status = VTY_CLOSE;
	  vtysh_vty_close (vty);
	  return NULL;
	}
  ...
  vty_hello (vty);
  if (! no_password_check)
    vtysh_vty_out (vty, "%sUser Access Verification%s%s", VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE);

  /* Setting up terminal. */
  vtysh_vty_will_echo (vty);
...
vty_accept (struct thread *thread)
{
  ...
  if (vty_sock < 0)
    {
      printf ("can't accept vty socket : %s", strerror (errno));
      return -1;
    }
  ...
  if (p->family == AF_INET && vty_accesslist_name)
	...
	{
	  char *buf;
	  printf("Vty connection refused from %s", (buf = sockunion_su2str (&su)));
	  free (buf);
	  close (vty_sock);
  ...
  ret = setsockopt (vty_sock, IPPROTO_TCP, TCP_NODELAY, 
		    (char *) &on, sizeof (on));
  if (ret < 0)
    printf("can't set sockopt to vty_sock : %s", strerror (errno));
...
void mio_vtysh_accept (int fd, afx_io_dir_t dir, void *arg);
void mio_vtysh_read (int fd, afx_io_dir_t dir, void *arg);
...
vty_serv_un (char *path)
{
  ...
  ret = afx_mio_create(&g_pMainMio, sock, AFX_IO_IN, mio_vtysh_accept, NULL);
  if (0 != ret)
  {
      fprintf(stderr, "Start VTYSH accept thread failed\n");
      return;
  }
...
void mio_vtysh_accept (int accept_sock, afx_io_dir_t dir, void *arg)
{
  int sock;
  int ret;
  size_t client_len;
  ...
  ret = afx_mio_create(&vty->mio_read, sock, AFX_IO_IN, mio_vtysh_read, vty);
  if (0 != ret)
  {
      fprintf(stderr, "Start VTY read thread for socket %d failed\n", sock);
      vtysh_vty_close(vty);
      return;
  }
...
void mio_vtysh_read (int sock, afx_io_dir_t dir, void *arg)
{
  int ret;
  int nbytes;
  struct vty *vty;
...
void
vtysh_vty_serv_sock (const char *hostname, unsigned short port, char *path)
{
#ifdef HAVE_VTY
...
/* Close vty interface. */
void
vtysh_vty_close (struct vty *vty)
{
  int i;
  ...
  if (vty->mio_read)
  {
      afx_mio_destroy(vty->mio_read);
      vty->mio_read = NULL;
  }
  if (vty->mio_write)
  {
      afx_mio_destroy(vty->mio_write);
      vty->mio_write = NULL;
  }
  if (vty->mio_timeout)
  {
      afx_mio_destroy(vty->mio_timeout);
      vty->mio_timeout = NULL;
  }
  if (vty->mio_output)
  {
      afx_mio_destroy(vty->mio_output);
      vty->mio_output = NULL;
  }

  /* Flush buffer. */
  if (! vtysh_buffer_empty (vty->obuf))
    vtysh_buffer_flush_all (vty->obuf, vty->fd);
  ...
  /* Unset vector. */
  vtysh_vector_unset (vtyvec, vty->fd);
...
vty_timeout (struct thread *thread)
{
  ...
  /* Clear buffer*/
  vtysh_buffer_reset (vty->obuf);
  vtysh_vty_out (vty, "%sVty connection is timed out.%s", VTY_NEWLINE, VTY_NEWLINE);
...
void
vtysh_vty_log (const char *proto_str, const char *format, va_list va)
{
  int i;
  struct vty *vty;
...
int
vtysh_vty_config_unlock (struct vty *vty)
{
...
vty_event (enum event event, int sock, struct vty *vty)
{
  ...
  switch (event)
    ...
    case VTY_SERV:
      ...
      vtysh_vector_set_index (Vvty_serv_thread, sock, vty_serv_thread);
      break;
#ifdef VTYSH
    case VTYSH_SERV:
       ...
       "Display who is on vty\n")
{
  ...
  for (i = 0; i < vector_max (vtyvec); i++)
    if ((v = vector_slot (vtyvec, i)) != NULL)
      vtysh_vty_out (vty, "%svty[%d] connected from %s.%s",
	       v->config ? "*" : " ",
       ...
       "IP access list\n")
{
  if (! vty_accesslist_name || (argc && strcmp(vty_accesslist_name, argv[0])))
    {
      vtysh_vty_out (vty, "Access-class is not currently applied to vty%s",
	       VTY_NEWLINE);
      return CMD_WARNING;
    }
...
/* Clear vty access class. */
CTC_CLI (no_vty_ipv6_access_class,
       ...
       "IPv6 access list\n")
{
  if (! vty_ipv6_accesslist_name ||
    ...
    {
      vtysh_vty_out (vty, "IPv6 access-class is not currently applied to vty%s",
	       VTY_NEWLINE);
      return CMD_WARNING;
    }
...
CTC_CLI (vtysh_service_advanced_vty,
...
CTC_CLI (no_service_advanced_vty,
       ...
       "Display the session command history\n")
{
  ...
  for (index = vty->hindex + 1; index != vty->hindex;)
	...
	vtysh_vty_out (vty, "  %s%s", vty->hist[index], VTY_NEWLINE);
...
{
  vtysh_vty_out (vty, "line vty%s", VTY_NEWLINE);

  if (vty_accesslist_name)
    vtysh_vty_out (vty, " access-class %s%s",
	     vty_accesslist_name, VTY_NEWLINE);

  if (vty_ipv6_accesslist_name)
    vtysh_vty_out (vty, " ipv6 access-class %s%s",
	     vty_ipv6_accesslist_name, VTY_NEWLINE);

  /* exec-timeout */
  if (vty_timeout_val != VTY_TIMEOUT_DEFAULT)
    vtysh_vty_out (vty, " exec-timeout %ld %ld%s", 
	     vty_timeout_val / 60,
	     vty_timeout_val % 60, VTY_NEWLINE);

  /* login */
  if (no_password_check)
    vtysh_vty_out (vty, " no login%s", VTY_NEWLINE);
...
int
vtysh_vty_shell (struct vty *vty)
{
  return vty->type == VTY_SHELL ? 1 : 0;
}

int
vtysh_vty_shell_serv (struct vty *vty)
{
...
vty_init_vtysh ()
{
  vtyvec = vtysh_vector_init (VECTOR_MIN_SIZE);
}
...
vty_init ()
{
  ...
  Vvty_serv_thread = vtysh_vector_init (VECTOR_MIN_SIZE);

#if 0
  /* Install bgp top node. */
  ctc_install_node (&vty_node, vty_config_write);
  ...
  install_element (VTY_NODE, &no_vty_login);
#ifdef HAVE_IPV6
...
ctc_vti_create (int mode)
{
  ...
  vti->fail = 0;
  vti->cp = 0;
  vtysh_vty_clear_buf (vti);
  vti->length = 0;
  memset(vti->hist, 0, sizeof (vti->hist));
  vti->hp = 0;
  vti->hindex = 0;
  vtysh_vector_set_index (vtyvec, 0, vti);
  vti->status = VTY_NORMAL;
  vti->v_timeout = vty_timeout_val;
