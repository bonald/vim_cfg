...
int
vtysh_buffer_empty (struct buffer *b)
{
...
void
vtysh_buffer_reset (struct buffer *b)
{
...
void
vtysh_buffer_add (struct buffer *b)
{
  struct buffer_data *d;
...
int
vtysh_buffer_write (struct buffer *b, u_char *ptr, size_t size)
{
  struct buffer_data *data;
...
int
vtysh_buffer_putc (struct buffer *b, u_char c)
{
  vtysh_buffer_write (b, &c, 1);
  return 1;
}
...
{
  vtysh_buffer_write (b, (u_char *)&c, 2);
  return 1;
}
...
int
vtysh_buffer_flush_all (struct buffer *b, int fd)
{
  int ret;
  struct buffer_data *d;
  int iov_index;
		      ...
		      int no_more_flag)
{
  ...
  if (! vtysh_buffer_empty (b) && !no_more_flag)
    {
...
/* Flush buffer to the file descriptor.  Mainly used from vty
   interface. */
int
vtysh_buffer_flush_vty (struct buffer *b, int fd, int size, 
...
/* Calculate size of outputs then flush buffer to the file
   descriptor. */
int
vtysh_buffer_flush_window (struct buffer *b, int fd, int width, int height, 
		     int erase, int no_more)
{
 ...
 flush:
  ...
  return vtysh_buffer_flush_vty (b, fd, size, erase, no_more);
}
