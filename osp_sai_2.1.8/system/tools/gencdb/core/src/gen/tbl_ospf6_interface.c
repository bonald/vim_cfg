...
tbl_ospf6_interface_key_val2str(tbl_ospf6_interface_t *p_ospf6_interface, char *str, uint32 str_len)
{
    ...
    sal_snprintf(str, str_len, "%s", p_ospf6_interface->key.name);
    return str;
}
...
tbl_ospf6_interface_key_str2val(char *str, tbl_ospf6_interface_t *p_ospf6_interface)
{
    ...
    sal_strcpy(p_ospf6_interface->key.name, str);
    return ret;
}
...
{
    sal_sprintf(str, "%s", p_ospf6_interface->key.name);
    return str;
}
...
tbl_ospf6_interface_dump_one(tbl_ospf6_interface_t *p_ospf6_interface, tbl_iter_args_t *pargs)
{
    ...
    {
        sal_fprintf(fp, "/%s"CMD_EQUAL_STR"%s", p_tbl_info->field[TBL_OSPF6_INTERFACE_FLD_KEY].name,
            p_ospf6_interface->key.name);
    }
