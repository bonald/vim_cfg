...
filename_def		= CONST.CDB_H_PATH + 'cdb_define.h'
filename_dir		= CONST.CDB_H_PATH + 'cdb_dir_priv.h'
filename_tbl		= CONST.CDB_H_PATH + 'cdb_tbl_priv.h'
filename_ds	        = CONST.CDB_H_PATH + 'cdb_ds_priv.h'
filename_act		= CONST.CDB_H_PATH + 'cdb_act_priv.h'
filename_bat		= CONST.CDB_H_PATH + 'cdb_bat_priv.h'
filename_inc		= CONST.CDB_H_PATH + 'cdb_inc_priv.h'
filename_dest_pm	= CONST.CDB_H_PATH + 'cdb_dest_pm_priv.h'
...
def build_entity_dir(fobj, fullname_array, entity, i):
	...
	if ('NULL' == entity[1]):
		...
		print >> fobj, "        &g_dir_node_info[%s]," % (entity[1])
	print >> fobj, "    },"

def build_entity_tbl(fobj, fullname_array, entity, i):
	'build cdb_node_t g_tbl_node_info'
	print >> fobj, "    {"
	print >> fobj, "        %s," % (entity[0])
	print >> fobj, "        %s," % ('CDB_NODE_TBL')
	print >> fobj, "        %s," % (CONST.PRODUCT_PREFIX + entity[3])
	print >> fobj, "        \"%s\"," % (string.lower(entity[0][4:]))
	print >> fobj, "        \"%s\"," % (fullname_array[i])
	print >> fobj, "        &g_dir_node_info[%s]," % (entity[1])
	print >> fobj, "        &g_cdb_tbl_info[%s]," % (entity[0])
	print >> fobj, "    },"
...
def build_entity_ds(fobj, fullname_array, entity, i):
	'build cdb_node_t g_ds_node_info'
	print >> fobj, "    {"
	print >> fobj, "        %s," % (entity[0])
	print >> fobj, "        %s," % ('CDB_NODE_DS')
	print >> fobj, "        %s," % (CONST.PRODUCT_PREFIX + entity[2])
	print >> fobj, "        \"%s\"," % (string.lower(entity[0][3:]))
	print >> fobj, "        \"%s\"," % (fullname_array[i])
	print >> fobj, "        &g_tbl_node_info[%s]," % (entity[1])
	print >> fobj, "        &g_cdb_ds_info[%s]," % (entity[0])
	print >> fobj, "    },"

def build_entity_act(fobj, fullname_array, entity, i):
	'build cdb_node_t g_act_node_info'
	print >> fobj, "    {"
	print >> fobj, "        %s," % (entity[0])
	print >> fobj, "        %s," % ('CDB_NODE_ACT')
	print >> fobj, "        %s," % (CONST.PRODUCT_PREFIX + entity[3])
	print >> fobj, "        \"%s\"," % (string.lower(entity[0][4:]))
	print >> fobj, "        \"%s\"," % (fullname_array[i])
	print >> fobj, "        &g_dir_node_info[%s]," % (entity[1])
	print >> fobj, "        &g_cdb_act_info[%s]," % (entity[0])
	print >> fobj, "    },"

def build_entity_bat(fobj, fullname_array, entity, i):
	'build cdb_node_t g_bat_node_info'
	print >> fobj, "    {"
	print >> fobj, "        %s," % (entity[0])
	print >> fobj, "        %s," % ('CDB_NODE_BAT')
	print >> fobj, "        %s," % (CONST.PRODUCT_PREFIX + entity[2])
	print >> fobj, "        \"%s\"," % (string.lower(entity[0][4:]))
	print >> fobj, "        \"%s\"," % (fullname_array[i])
	print >> fobj, "        &g_dir_node_info[%s]," % (entity[1])
...
def build_fullname(fullname_array, dir_fullname_array, dir_array, entity, i, array_type, parent_ds=None):
	...
	if ('DS_' == entity[0][:3]):
		...
		entity_name += string.lower(entity[0][4:])
...
print >> file_def, ""
print >> file_def, "#ifndef __CDB_DEFINE_H__"
print >> file_def, "#define __CDB_DEFINE_H__"
print >> file_def, ""
...
for i, entity in enumerate(bat_array):
	build_enum(file_def, entity, i)
print >> file_def, "    %sMAX" % (CONST.NODE_PREFIX_BAT)
print >> file_def, "} cdb_bat_t;"
print >> file_def, ""

print >> file_def, "#endif /* !__CDB_DEFINE_H__ */"
print >> file_def, ""
...
_macro_h_ = "__CDB_DIR_PRIV_H__"
print >> file_dir, ""
print >> file_dir, "#ifndef %s" % (_macro_h_)
print >> file_dir, "#define %s" % (_macro_h_)
print >> file_dir, ""
print >> file_dir, "cdb_node_t g_dir_node_info[DIR_MAX] ="
print >> file_dir, "{"
for i, entity in enumerate(dir_array):
...
_macro_h_ = "__CDB_TBL_PRIV_H__"
print >> file_tbl, ""
print >> file_tbl, "#ifndef %s" % (_macro_h_)
print >> file_tbl, "#define %s" % (_macro_h_)
print >> file_tbl, ""
print >> file_tbl, "cdb_node_t g_tbl_node_info[TBL_MAX] ="
print >> file_tbl, "{"
for i, entity in enumerate(tbl_array):
...
_macro_h_ = "__CDB_DS_PRIV_H__"
print >> file_ds, ""
print >> file_ds, "#ifndef %s" % (_macro_h_)
print >> file_ds, "#define %s" % (_macro_h_)
print >> file_ds, ""
print >> file_ds, "cdb_node_t g_ds_node_info[DS_MAX] ="
print >> file_ds, "{"
for i, entity in enumerate(ds_array):
...
_macro_h_ = "__CDB_ACT_PRIV_H__"
print >> file_act, ""
print >> file_act, "#ifndef %s" % (_macro_h_)
print >> file_act, "#define %s" % (_macro_h_)
print >> file_act, ""
print >> file_act, "cdb_node_t g_act_node_info[ACT_MAX] ="
print >> file_act, "{"
for i, entity in enumerate(act_array):
...
#########################################################
# build cdb_bat_priv.h
#########################################################
file_bat = open(filename_bat, 'w')
_macro_h_ = "__CDB_BAT_PRIV_H__"
print >> file_bat, ""
print >> file_bat, "#ifndef %s" % (_macro_h_)
print >> file_bat, "#define %s" % (_macro_h_)
print >> file_bat, ""
print >> file_bat, "cdb_node_t g_bat_node_info[BAT_MAX] ="
print >> file_bat, "{"
for i, entity in enumerate(bat_array):
	build_entity_bat(file_bat, bat_fullname_array, entity, i)
print >> file_bat, "};"
print >> file_bat, ""
print >> file_bat, "#endif /* !%s */" % (_macro_h_)
print >> file_bat, ""

file_act.close()
print "generate file %s" % (filename_bat)
