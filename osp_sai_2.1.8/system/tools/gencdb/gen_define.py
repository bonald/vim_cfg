...
def build_one_tbl_info(fobj, table, data_array):
        ...
        print >> fobj, "        sizeof(%s)/sizeof(cdb_field_t)," % (tbl_name_field)
        print >> fobj, "        %s," % (tbl_name_field)
        print >> fobj, "        {"
        print >> fobj, "            %s_iterate," % (tbl_name_low)
        print >> fobj, "            (TBL_DUMP_FUNC)%s_dump_one," % (tbl_name_low)
        if (with_key):
            print >> fobj, "            (TBL_KEY_V2S_FUNC)%s_key_val2str," % (tbl_name_low)
        else:
            print >> fobj, "            NULL,"
        print >> fobj, "            NULL"
        print >> fobj, "        },"
        print >> fobj, "        {"
        print >> fobj, "            (TBL_KEY_NAME_DUMP_FUNC)%s_key_name_dump," % (tbl_name_low)
        print >> fobj, "            (TBL_KEY_VALUE_DUMP_FUNC)%s_key_value_dump," % (tbl_name_low)
        print >> fobj, "            (TBL_FIELD_NAME_DUMP_FUNC)%s_field_name_dump," % (tbl_name_low)
        print >> fobj, "            (TBL_FIELD_VALUE_DUMP_FUNC)%s_field_value_dump," % (tbl_name_low)
        print >> fobj, "            (TBL_ALL_DUMP_FUNC)%s_table_dump," % (tbl_name_low)
        print >> fobj, "            (TBL_PARSER_FUNC)%s_table_parser," % (tbl_name_low)
        print >> fobj, "        }"
        print >> fobj, "    },"

def gen_tbl_init_func(fobj, tbl_array):
	print >> fobj, "int32"
	print >> fobj, "cdb_tbl_pm_init(uint32 pm_id)"
	print >> fobj, "{"
	print >> fobj, "    cdb_node_t *p_tbl = NULL;"
	print >> fobj, "    uint32 i = 0;"
	print >> fobj, ""
	print >> fobj, "    for (i = 0; i < TBL_MAX; i++)"
	print >> fobj, "    {"
	print >> fobj, "        p_tbl = cdb_get_tbl(i);"
	print >> fobj, ""
	print >> fobj, "        if (0)"
	print >> fobj, "        {"
	print >> fobj, "        }"
	...
	for i, entity in enumerate(tbl_array):
		...
		if (len(pms_array) == 1 and 'all' == pms_array[0]):
			...
			for j, pm_item in enumerate(pms_array):
				...
				tmp_str += ' || (' + pm_id + ' == pm_id)'
			print >> fobj, "            if (CHECK_FULL_CDB_PMS%s)" % (tmp_str)
			print >> fobj, "            {"
			print >> fobj, "                %s_init_%s();" % (tbl_name, accusative)
			print >> fobj, "                p_tbl->inited = TRUE;"
			print >> fobj, "            }"
...
def gen_tbl_info_file(fname, tbl_array):
	...
	print >> fobj, "static cdb_tbl_info_t g_cdb_tbl_info[TBL_MAX] ="
	print >> fobj, "{"
...
def build_one_ds_info(fobj, ds):
    ...
    print >> fobj, "        sizeof(%s)/sizeof(cdb_field_t)," % (ds_name_field)
    print >> fobj, "        %s," % (ds_name_field)
    print >> fobj, "        {"
    print >> fobj, "            NULL,"
    print >> fobj, "            (TBL_DUMP_FUNC)%s_dump_one," % (ds_name_low)
    print >> fobj, "            NULL,"
    print >> fobj, "            NULL"
    print >> fobj, "        },"
    print >> fobj, "        {"
...
def gen_ds_info_file(fname, ds_array):
	...
	print >> fobj, "static cdb_tbl_info_t g_cdb_ds_info[DS_MAX] ="
	print >> fobj, "{"
...
def build_one_act_info(fobj, act):
	...
	print >> fobj, "        0,"
	print >> fobj, "        0,"
	print >> fobj, "        sizeof(%s)/sizeof(cdb_field_t)," % (act_name_field)
...
def gen_act_info_file(fname, act_array):
	...
	print >> fobj, "static cdb_tbl_info_t g_cdb_act_info[ACT_MAX] ="
	print >> fobj, "{"
...
def get_dt_type_type(field):
	...
	if ('cdb_reference_list_t' == dt_type_type):
		...
		dt_type_type = 'DS'
	dt_type_type = 'CDB_DT_' + dt_type_type
...
def build_one_act_field(fobj, act, field_array):
	...
	print >> fobj, "static cdb_field_t %s[] = " % (act_name_field)
	print >> fobj, "{"
	for i, field in enumerate(field_array):
		...
		[dt_type_type, ds_name] = ['CDB_DT_NONE', '-1']
...
def get_field_access_enum(field):
	...
	if ('FLAGS' == access_type):
		access_type = field[5]
	if (access_type == 'READ'):
		return 'CDB_ACCESS_RD'
	elif (access_type == 'RW'):
		return 'CDB_ACCESS_RW'
	elif (access_type == 'SUB'):
		return 'CDB_ACCESS_SUB'
	elif (access_type == 'SUB'):
		return 'CDB_ACCESS_SUB'
	else:
...
def build_one_ds_field(fobj, ds, key_array, field_array):
	...
	print >> fobj, "static cdb_field_t %s[] = " % (ds_name_field)
	print >> fobj, "{"
	for i, field in enumerate(field_array):
		if (len(field) > 4):
			field_name = cdb_util.format_field_name(field[3])
...
def build_tbl_data(fobj, tbl_name, data_array):
	...
	for i, field in enumerate(data_array):
		if ('KEY' in field[0]):
			...
			field_type = 'cdb_reference_list_t'
...
# gen cdb_tbl_info_priv.h
if (GEN_TBL_INFO):
	gen_tbl_info_file(CONST.CDB_H_PATH + 'cdb_tbl_info_priv.h', tbl_array)

############################################################
# gen cdb_tbl_field_priv.h
if (GEN_TBL_FIELD):
	gen_tbl_field_file(CONST.CDB_H_PATH + 'cdb_tbl_field_priv.h', tbl_array)

############################################################
# gen cdb_ds_info_priv.h
if (GEN_DS_INFO):
	gen_ds_info_file(CONST.CDB_H_PATH + 'cdb_ds_info_priv.h', ds_array)

############################################################
# gen cdb_ds_field_priv.h
if (GEN_DS_FIELD):
	gen_ds_field_file(CONST.CDB_H_PATH + 'cdb_ds_field_priv.h', ds_array)

############################################################
# gen cdb_act_info_priv.h
if (GEN_ACT_INFO):
	gen_act_info_file(CONST.CDB_H_PATH + 'cdb_act_info_priv.h', act_array)

############################################################
# gen cdb_act_field_priv.h
if (GEN_ACT_FIELD):
	gen_act_field_file(CONST.CDB_H_PATH + 'cdb_act_field_priv.h', act_array)
