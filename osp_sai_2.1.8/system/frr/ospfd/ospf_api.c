...
void
ospf_lsa_ignored_log_ctc(struct lsa_header *lsa, char* reason)
{
  /* modified by liwh for bug 46103, 2018-01-10 */
  u_char link_id[16];
  u_char adv_router[16];
  /* liwh end */
  
  if (!lsa || !reason)
    return;

  /* modified by liwh for bug 46106, 2018-01-16 */
  if (!IS_DEBUG_OSPF (lsa, LSA_INSTALL))
    return;
  /* liwh end */

  /* modified by liwh for bug 46103, 2018-01-10 */
  snprintf((char*)link_id, 16, "%s", inet_ntoa(lsa->id));
  snprintf((char*)adv_router, 16, "%s", inet_ntoa(lsa->adv_router));
  /* liwh end */

  /* modified by liwh for bug 46106, 2018-01-16 */
  // logid_sys(LOG_OSPF_4_LSA_ROUTE,
  zlog_debug ("OSPF lsa %s %s %s ignored for %s", 
    lookup_msg(ospf_lsa_type_msg, lsa->type, NULL),
    link_id,      /* modified by liwh for bug 46103, 2018-01-10 */
    adv_router,   /* modified by liwh for bug 46103, 2018-01-10 */
    reason);
  /* liwh end */
