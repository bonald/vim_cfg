			...
			struct prefix_ipv4 *p, int advertise)
{
	...
	if (CHECK_FLAG(advertise, OSPF_AREA_RANGE_ADVERTISE))
       ...
       { 
               /* modified by liwh for bug 51059, 2019-02-19 
                    when configure area range not advertise, should delete backhole route */
               if (range->specifics != 0)
		    ospf_delete_discard_route(area->ospf->new_table, p);
               /* liwh end */
