...
#ifdef _CENTEC_
        {
          nbr->nbr_down_reason = OSPF_NEIGHOBR_DOWN_INTERFACE_DOWN;
      ...
      if (if_is_up(ifp)) 
        {
          for (ALL_LIST_ELEMENTS_RO (ifp->connected, cnode, connected))
            {
              p = connected->address;
              if (p->u.prefix4.s_addr == address.s_addr)
                {
                  return ifp;
                }
            }
					      ...
					      struct interface *ifp)
{
	...
	for (rn = route_top(IF_OIFS(ifp)); rn; rn = route_next(rn)) {
		...
		/* djd: for vrrp virtual IP, should not recv any ospf packets, so skip it */
