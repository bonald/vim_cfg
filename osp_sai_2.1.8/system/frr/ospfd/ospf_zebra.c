...
DEFINE_MTYPE_STATIC(OSPFD, OSPF_EXTERNAL, "OSPF External route table") /* djd */
DEFINE_MTYPE_STATIC(OSPFD, OSPF_REDISTRIBUTE, "OSPF Redistribute")     /* djd */
...
#ifdef _CENTEC_  
  //if (ospf != NULL)
  if ((ospf != NULL) && (ospf->router_id.s_addr == 0))
#endif
/*liwh end */   
...
/* modified by liwh for bug 44298, 2017-06-22 */
  ...
  for (rn = route_top (IF_OIFS (ifp)); rn; rn = rn_next)
    {
...
/* modified by liwh for bug 44938, 2017-08-09 
  when interface change from down to up, should recalculate output cost again */
#ifdef _CENTEC_
  ospf_if_recalculate_output_cost (ifp);
#endif
/*liwh end */
...
/* modified by liwh for bug 51049, 2019-02-19 */
#ifdef _CENTEC_
        c = ospf_zebra_interface_address_read(command, zclient->ibuf, vrf_id);
#else
...
/* modified by liwh for bug 50625, 2019-01-18 */
#ifdef _CENTEC_ 
       /*if (c->address && (c->address->prefixlen >= IPV4_MAX_PREFIXLEN))
       {
            return 0;
       }*/
#endif
...
/* modified by liwh for bug 45943, 2017-12-21 
   when add new address, must calculate route agagin, 
   delete the route ip address equals local ip address */
#ifdef _CENTEC_ 
   ospf = ospf_lookup ();
   if (ospf)
    {
...
struct ospf_external *ospf_external_add(u_char type, u_short instance)
{
	...
	#else /* djd */
	ext = (struct ospf_external *)XCALLOC(MTYPE_OSPF_EXTERNAL, sizeof(struct ospf_external));
	#endif /* djd */
...
void ospf_external_del(u_char type, u_short instance)
{
	...
	if (ext) {
		...
		if (!om->external[type]->count) {
			...
			#else /* djd */
			list_delete(om->external[type]);
			#endif /* djd */
			om->external[type] = NULL;
		}
        XFREE(MTYPE_OSPF_EXTERNAL, ext); /* djd */
	}
}
				    ...
				    u_short instance)
{
	...
	#else /* djd */
	red = (struct ospf_redist *)XCALLOC(MTYPE_OSPF_REDISTRIBUTE, sizeof(struct ospf_redist));
	#endif /* djd */
...
void ospf_redist_del(struct ospf *ospf, u_char type, u_short instance)
{
	...
	if (red) {
        ...
        XFREE(MTYPE_OSPF_REDISTRIBUTE, red); /* djd */
	}
}
...
/* modified by liwh for bug 44298, 2017-06-22 */
  ...
  for (rn = route_top (ospf->distance_table); rn; rn = rn_next)
    {
    rn_next = route_next (rn);
#endif
/*liwh end */
