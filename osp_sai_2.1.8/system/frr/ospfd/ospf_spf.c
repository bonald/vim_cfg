...
/* modified by liwh for bug 45819, 2017-12-08 */
#ifdef _CENTEC_
char   ignore_lsa_reason[256] = {0};
#endif
/*liwh end */
...
/* modified by liwh for bug 45793, 2017-12-06 */
#ifdef _CENTEC_
  if (IS_DEBUG_OSPF_EVENT_SPF)
#else
...
/* modified by liwh for bug 45819, 2017-12-08 */
#ifdef _CENTEC_              
              sprintf(ignore_lsa_reason, "Invalid LSA link type %d", type);
              ospf_lsa_ignored_log_ctc(v->lsa, ignore_lsa_reason);
#else
...
/* modified by liwh for bug 45819, 2017-12-08 */
#ifdef _CENTEC_              
          ospf_lsa_ignored_log_ctc(v->lsa, "No LSA found");
#else
          if (IS_DEBUG_OSPF_EVENT)
...
/* modified by liwh for bug 45793, 2017-12-06 */
#ifdef _CENTEC_
          ospf_lsa_ignored_log_ctc(v->lsa, "LSA is MaxAge");
#else
          if (IS_DEBUG_OSPF_EVENT)
...
/* modified by liwh for bug 45793, 2017-12-06 */
#ifdef _CENTEC_
          ospf_lsa_ignored_log_ctc(v->lsa, "The LSA doesn't have a link back");
#else
          if (IS_DEBUG_OSPF_EVENT)
...
/* modified by liwh for bug 45793, 2017-12-06 */
#ifdef _CENTEC_
      ospf_lsa_ignored_log_ctc(v->lsa, "The LSA is already in SPF");
#else
      if (IS_DEBUG_OSPF_EVENT)
...
/* modified by liwh for bug 45793, 2017-12-06 */
#ifdef _CENTEC_
          else if (IS_DEBUG_OSPF_EVENT_SPF)
#else
...
/* modified by liwh for bug 45793, 2017-12-06 */
#ifdef _CENTEC_
              ospf_lsa_ignored_log_ctc(v->lsa, "Old vertex distance is less than new");
#endif
/* liwh end */
				continue;
			}
...
#else /* djd */
/* fix ospf crash when do spf calcuation */
#define OSPF_SPF_QUEUE_NUM       500
struct ospf_spf_queue_entry
{
    struct vertex *vertex;
    int parent_is_root;
};

struct ospf_spf_queue
{
    struct ospf_spf_queue_entry entry[OSPF_SPF_QUEUE_NUM];
    int head, tail;
};

void ospf_spf_queue_init(struct ospf_spf_queue *q)
{
    memset(q, 0, sizeof(struct ospf_spf_queue));
}

void ospf_spf_enqueue(struct ospf_spf_queue *q, struct ospf_spf_queue_entry *e)
{
    int i = (q->tail + 1) % OSPF_SPF_QUEUE_NUM;

    if(i = q->head)
    {
        printf("ospf spf queue is full, should enlarge the queue!\n");
    }
    else
    {
        q->entry[q->tail].parent_is_root = e->parent_is_root;
        q->entry[q->tail].vertex = e->vertex;
        q->tail = i;
    }
}

struct ospf_spf_queue_entry* ospf_spf_dequeue(struct ospf_spf_queue *q)
{
    int i;
    struct ospf_spf_queue_entry* temp;

    if(q->head == q->tail)
        return NULL;

    i = (q->head + 1) % OSPF_SPF_QUEUE_NUM;
    temp = &q->entry[q->head];
    q->head = i;
				   ...
				   struct route_table *rt, int parent_is_root)
{
    struct listnode *cnode, *cnnode;
    struct vertex *child;
    struct ospf_spf_queue queue;
    struct ospf_spf_queue_entry entry;
    struct ospf_spf_queue_entry* current;

    if(IS_DEBUG_OSPF_EVENT)
        zlog_debug("ospf_process_stub():processing stubs for area %s", inet_ntoa(area->area_id));

    ospf_spf_queue_init(&queue);
    entry.vertex = v;
    entry.parent_is_root = parent_is_root;
    ospf_spf_enqueue(&queue, &entry);

    while(current = ospf_spf_dequeue(&queue))
    {
        struct vertex *vx = current->vertex;
        if(vx->type == OSPF_VERTEX_ROUTER)
        {
            u_char *p;
            u_char *lim;
            struct router_lsa_link *l;
            ...
            if(IS_DEBUG_OSPF_EVENT)
                zlog_debug("ospf_process_stubs():processing router LSA, id: %s", inet_ntoa(vx->lsa->id));

            rlsa = (struct router_lsa *)vx->lsa;
            if(IS_DEBUG_OSPF_EVENT)
                zlog_debug("ospf_process_stubs(): we have %d links to process",	ntohs(rlsa->links));
            
            p = ((u_char *)vx->lsa) + OSPF_LSA_HEADER_SIZE + 4;
            lim = ((u_char *)vx->lsa) + ntohs(vx->lsa->length);
            ...
            while(p < lim)
            {
                ...
                if(l->m[0].type == LSA_LINK_TYPE_STUB)
                    ospf_intra_add_stub(rt, l, vx, area, current->parent_is_root, lsa_pos);
                lsa_pos++;
            }
            SET_FLAG(current->vertex->flags, OSPF_VERTEX_PROCESSED);
        }    

        ospf_vertex_dump("ospf_process_stubs(): after examining links: ", vx, 1, 1);

        for(ALL_LIST_ELEMENTS(vx->children, cnode, cnnode, child))
        {
            if(CHECK_FLAG(child->flags, OSPF_VERTEX_PROCESSED))
                continue;
            ...
            */
            if(area->spf == vx)
                entry.parent_is_root = 1;
            else if(vx->type == OSPF_VERTEX_ROUTER)
                entry.parent_is_root = 0;
...
/* modified by liwh for bug 44298, 2017-06-22 */
  ...
  for (rn = route_top (rtrs); rn; rn = rn_next)
    {
...
/*liwh end */
	...
	ospf->t_spf_calc = thread_add_timer_msec(
		master, ospf_spf_calculate_timer, ospf, delay);
}
