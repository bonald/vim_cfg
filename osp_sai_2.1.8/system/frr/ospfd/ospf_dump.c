...
/* modified by liwh for bug 43644, 2017-08-01 */
#ifdef _CENTEC_
  else if (ms || t->tv_sec)
...
/* modified by liwh for bug 43681, 2017-07-24 */
#ifdef _CENTEC_
...
/* modified by liwh for bug 46358, 2018-02-02 */
#ifdef _CENTEC_
...
/* modified by liwh for bug 45793, 2017-12-07 */
  ...
  if (vty->node == CONFIG_NODE)
    {
      if (argc == arg_base + 0)
        ...
        {
          if (strncmp (argv[arg_base + 0]->text, "a", 1) == 0)
              DEBUG_ON (event, EVENT_ABR);
          else if (strncmp (argv[arg_base + 0]->text, "i", 1) == 0)
              DEBUG_ON (event, EVENT_IA);
          else if (strncmp (argv[arg_base + 0]->text, "r", 1) == 0)
              DEBUG_ON (event, EVENT_ROUTE);
          else if (strncmp (argv[arg_base + 0]->text, "s", 1) == 0)
              DEBUG_ON (event, EVENT_SPF);
        }
  ...
  if (argc == arg_base + 0)
    ...
    {
      if (strncmp (argv[arg_base + 0]->text, "a", 1) == 0)
        TERM_DEBUG_ON (event, EVENT_ABR);
      else if (strncmp (argv[arg_base + 0]->text, "i", 1) == 0)
        TERM_DEBUG_ON (event, EVENT_IA);
      else if (strncmp (argv[arg_base + 0]->text, "r", 1) == 0)
        TERM_DEBUG_ON (event, EVENT_ROUTE);
      else if (strncmp (argv[arg_base + 0]->text, "s", 1) == 0)
        TERM_DEBUG_ON (event, EVENT_SPF);
    }
#else
  if (vty->node == CONFIG_NODE)
...
/* modified by liwh for bug 45793, 2017-12-07 */
  ...
  if (vty->node == CONFIG_NODE)
    {
      if (argc == arg_base + 0)
        ...
        {
          if (strncmp (argv[arg_base + 0]->text, "a", 1) == 0)
              DEBUG_OFF (event, EVENT_ABR);
          else if (strncmp (argv[arg_base + 0]->text, "i", 1) == 0)
              DEBUG_OFF (event, EVENT_IA);
          else if (strncmp (argv[arg_base + 0]->text, "r", 1) == 0)
              DEBUG_OFF (event, EVENT_ROUTE);
          else if (strncmp (argv[arg_base + 0]->text, "s", 1) == 0)
              DEBUG_OFF (event, EVENT_SPF);
        }
  ...
  if (argc == arg_base + 0)
    ...
    {
      if (strncmp (argv[arg_base + 0]->text, "a", 1) == 0)
        TERM_DEBUG_OFF (event, EVENT_ABR);
      else if (strncmp (argv[arg_base + 0]->text, "i", 1) == 0)
        TERM_DEBUG_OFF (event, EVENT_IA);
      else if (strncmp (argv[arg_base + 0]->text, "r", 1) == 0)
        TERM_DEBUG_OFF (event, EVENT_ROUTE);
      else if (strncmp (argv[arg_base + 0]->text, "s", 1) == 0)
        TERM_DEBUG_OFF (event, EVENT_SPF);
    }
#else
  if (vty->node == CONFIG_NODE)
...
static int show_debugging_ospf_common(struct vty *vty, struct ospf *ospf)
{
	...
	if (IS_DEBUG_OSPF(ism, ISM) == OSPF_DEBUG_ISM)
		...
		if (IS_DEBUG_OSPF(ism, ISM_STATUS))
			vty_out(vty, "  OSPF ISM status debugging is on%s",
				VTY_NEWLINE);
		if (IS_DEBUG_OSPF(ism, ISM_EVENTS))
			vty_out(vty, "  OSPF ISM event debugging is on%s",
				VTY_NEWLINE);
		if (IS_DEBUG_OSPF(ism, ISM_TIMERS))
			vty_out(vty, "  OSPF ISM timer debugging is on%s",
				VTY_NEWLINE);
	}
	...
	if (IS_DEBUG_OSPF(nsm, NSM) == OSPF_DEBUG_NSM)
		...
		if (IS_DEBUG_OSPF(nsm, NSM_STATUS))
			vty_out(vty, "  OSPF NSM status debugging is on%s",
				VTY_NEWLINE);
		if (IS_DEBUG_OSPF(nsm, NSM_EVENTS))
			vty_out(vty, "  OSPF NSM event debugging is on%s",
				VTY_NEWLINE);
		if (IS_DEBUG_OSPF(nsm, NSM_TIMERS))
			vty_out(vty, "  OSPF NSM timer debugging is on%s",
				VTY_NEWLINE);
	}
	...
	if (IS_DEBUG_OSPF(lsa, LSA) == OSPF_DEBUG_LSA)
		...
		if (IS_DEBUG_OSPF(lsa, LSA_GENERATE))
			vty_out(vty, "  OSPF LSA generation debugging is on%s",
				VTY_NEWLINE);
		if (IS_DEBUG_OSPF(lsa, LSA_FLOODING))
			vty_out(vty, "  OSPF LSA flooding debugging is on%s",
				VTY_NEWLINE);
		if (IS_DEBUG_OSPF(lsa, LSA_INSTALL))
			vty_out(vty, "  OSPF LSA install debugging is on%s",
				VTY_NEWLINE);
		if (IS_DEBUG_OSPF(lsa, LSA_REFRESH))
			vty_out(vty, "  OSPF LSA refresh debugging is on%s",
				VTY_NEWLINE);
	}
	...
	if (IS_DEBUG_OSPF(zebra, ZEBRA) == OSPF_DEBUG_ZEBRA)
		...
		if (IS_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
			vty_out(vty, "  OSPF Zebra interface debugging is on%s",
				VTY_NEWLINE);
		if (IS_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
			vty_out(vty,
				"  OSPF Zebra redistribute debugging is on%s",
				VTY_NEWLINE);
	}
...
static int config_write_debug(struct vty *vty)
{
	...
	if (IS_CONF_DEBUG_OSPF(ism, ISM) == OSPF_DEBUG_ISM)
		...
		if (IS_CONF_DEBUG_OSPF(ism, ISM_STATUS))
			vty_out(vty, "debug ospf%s ism status%s", str,
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(ism, ISM_EVENTS))
			vty_out(vty, "debug ospf%s ism event%s", str,
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(ism, ISM_TIMERS))
			vty_out(vty, "debug ospf%s ism timer%s", str,
				VTY_NEWLINE);
	}
	...
	if (IS_CONF_DEBUG_OSPF(nsm, NSM) == OSPF_DEBUG_NSM)
		...
		if (IS_CONF_DEBUG_OSPF(nsm, NSM_STATUS))
			vty_out(vty, "debug ospf%s nsm status%s", str,
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(nsm, NSM_EVENTS))
			vty_out(vty, "debug ospf%s nsm event%s", str,
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(nsm, NSM_TIMERS))
			vty_out(vty, "debug ospf%s nsm timer%s", str,
				VTY_NEWLINE);
	}
	...
	if (IS_CONF_DEBUG_OSPF(lsa, LSA) == OSPF_DEBUG_LSA)
		...
		if (IS_CONF_DEBUG_OSPF(lsa, LSA_GENERATE))
			vty_out(vty, "debug ospf%s lsa generate%s", str,
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(lsa, LSA_FLOODING))
			vty_out(vty, "debug ospf%s lsa flooding%s", str,
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(lsa, LSA_INSTALL))
			vty_out(vty, "debug ospf%s lsa install%s", str,
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(lsa, LSA_REFRESH))
			vty_out(vty, "debug ospf%s lsa refresh%s", str,
				VTY_NEWLINE);
	...
	if (IS_CONF_DEBUG_OSPF(zebra, ZEBRA) == OSPF_DEBUG_ZEBRA)
		...
		if (IS_CONF_DEBUG_OSPF(zebra, ZEBRA_INTERFACE))
			vty_out(vty, "debug ospf%s zebra interface%s", str,
				VTY_NEWLINE);
		if (IS_CONF_DEBUG_OSPF(zebra, ZEBRA_REDISTRIBUTE))
			vty_out(vty, "debug ospf%s zebra redistribute%s", str,
				VTY_NEWLINE);
	...
	if (r == (OSPF_DEBUG_SEND_RECV | OSPF_DEBUG_DETAIL)) {
		vty_out(vty, "debug ospf%s packet all detail%s", str,
			VTY_NEWLINE);
		return 1;
	}
	...
	if (r == OSPF_DEBUG_SEND_RECV) {
		...
		for (i = 0; i < 5; i++)
			if (conf_debug_ospf_packet[i] & OSPF_DEBUG_DETAIL)
				vty_out(vty, "debug ospf%s packet %s detail%s",
					str, type_str[i], VTY_NEWLINE);
		return 1;
	}
	...
	for (i = 0; i < 5; i++) {
		...
		vty_out(vty, "debug ospf%s packet %s%s%s", str, type_str[i],
			detail_str[conf_debug_ospf_packet[i]], VTY_NEWLINE);
		write = 1;
	}
