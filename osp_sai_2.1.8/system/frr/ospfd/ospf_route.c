...
#ifdef _CENTEC_
  //for (rn = route_top (rt); rn; rn = route_next (rn))
  for (rn = route_top (rt); rn; rn = rn_next)
    {
...
/* modified by liwh for bug 45819, 2017-12-08 */
#ifdef _CENTEC_
      ospf_lsa_ignored_log_ctc(&lsa->header, "ospf_intra_add_router: "
...
/* modified by liwh for bug 45819, 2017-12-11 */
#ifdef _CENTEC_              
        {
          if (v->distance > cur_or->cost)
            {
              sprintf(ignore_lsa_reason, "new path distance %d greater than old route cost %d", v->distance,
                cur_or->cost);
          ...
          if (IPV4_ADDR_CMP (&cur_or->u.std.origin->id, &lsa->header.id) > 0)
            {
              sprintf(ignore_lsa_reason, "Old route Link State Origin %s greater than new lsa header id %s", 
                inet_ntoa (cur_or->u.std.origin->id),
                inet_ntoa (lsa->header.id));
              ospf_lsa_ignored_log_ctc(v->lsa, ignore_lsa_reason);
            }
...
/* liwh end */
	 ...
	 */
/* modified by liwh for bug 45759, 2017-12-04 */
#ifdef _CENTEC_
  if (link->link_data.s_addr == 0xffffffff
    /* modified by liwh for bug 45943, 2017-12-21 */
    && (lsa->header.adv_router.s_addr != area->ospf->router_id.s_addr)
    && ospf_zebra_if_lookup_by_local_addr (link->link_id))
    /* liwh end */
#else
...
/* modified by liwh for bug 45819, 2017-12-11 */
#ifdef _CENTEC_
      sprintf(ignore_lsa_reason, "ospf_intra_add_stub(): old route %s/%d is better, exit", inet_ntoa (p.prefix), 
          p.prefixlen);
      ospf_lsa_ignored_log_ctc(v->lsa, ignore_lsa_reason);
#else
      if (IS_DEBUG_OSPF_EVENT)
