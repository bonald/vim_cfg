...
/* modified by liwh for bug 44144, 2017-08-10 */
#ifndef _CENTEC_
  ...
  if (!oi->obuf)
    {
      if (IS_DEBUG_OSPF_PACKET (0, SEND))
        zlog_warn("ospf_packet_add(interface %s in state %d [%s], packet type %s, "
				 ...
				 struct ospf_header *ospfh)
{
  ...
  struct crypt_key *ck = NULL;  /* _CENTEC_ for clear compile warning [-Wmaybe-uninitialized] */
	struct ospf_neighbor *nbr;
	u_int16_t length = ntohs(ospfh->length);
/* modified by liwh for bug 44144, 2017-08-11 */
#ifdef _CENTEC_
  unsigned char digest_zero[OSPF_AUTH_MD5_SIZE] = {0};
...
/* modified by liwh for bug 44144, 2017-08-11 */
  ...
  if (list_isempty (OSPF_IF_PARAM (oi, auth_crypt)))
    {
      auth_key = (const u_int8_t *) digest_zero;
...
#define OSPF_WRITE_IPHL_SHIFT 2
	...
	while ((pkt_count < ospf->write_oi_count) && oi
		...
		pkt_count++;
/* merge from quagga by liwh for bug 44491, 2017-06-27 */
/* SYSTEM MODIFIED by wangjj for ospf crash, 2017-06-02 */
#ifdef _CENTEC_
      if (NULL == oi->obuf)
      {
        oi->on_write_q = 0;
...
/* modified by liwh for bug 51736, 2019-06-22 */
#ifdef _CENTEC_
       if (nbr->address.u.prefix4.s_addr != iph->ip_src.s_addr)
       {
           zlog_warn(
			"Packet %s [Hello:RECV]: source ip %s Neighbor Interface Address %s",
			inet_ntoa(ospfh->router_id), inet_ntoa(iph->ip_src), inet_ntoa(nbr->address.u.prefix4));
           return;
       }
#endif
/* liwh end */
			 ...
			 u_int16_t size)
{
  ...
  /* modified by liwh for bug 43666, 2017-07-26 */
#ifdef _CENTEC_
      //if (IS_DEBUG_OSPF_EVENT) 
      if (IS_DEBUG_OSPF_PACKET((OSPF_DEBUG_DB_DESC -1), RECV))
#endif
/*liwh end */
        zlog_debug ("ospf_db_desc");
			...
			for (ALL_LIST_ELEMENTS(oi->ospf->oiflist, oinode,
				...
				if ((IPV4_ADDR_SAME(&out_if->address->u.prefix4,
				    ...
				    && (!(IPV4_ADDR_SAME(
					       &oi->ospf->router_id,
					       &lsa->data->adv_router)))) {
/* modified by liwh for bug 53424, 2019-09-09 */
#ifndef _CENTEC_
...
/* modified by liwh for bug 44144, 2017-08-11 */
#ifdef _CENTEC_
    bool auth_check_fail = 0;
    if (list_isempty (OSPF_IF_PARAM (oi, auth_crypt)))
      {
        auth_check_fail =  (ospfh->u.crypt.key_id != 0) ||
           (!ospf_check_md5_digest (oi, ospfh));
      }
    else
      {
        auth_check_fail =  (NULL == (ck = listgetdata (listtail(OSPF_IF_PARAM (oi,auth_crypt))))) ||
...
/* modified by liwh for bug 45777, 2017-12-05 */
#ifdef _CENTEC_
static int
ospf_recv_stats_task_timer (struct thread *thread)
{
  struct ospf *ospf = THREAD_ARG (thread);

  ospf->t_ospf_recv_stats_task = NULL;

  if (ospf->ospf_recv_stats > OSPF_MAX_RECV_PKTS)
    zlog_warn ("ospf receive %d packets in 1 second\n", ospf->ospf_recv_stats);

  ospf->ospf_recv_stats = 0;
...
/* modified by liwh for bug 45777, 2017-12-05 */
#ifdef _CENTEC_
  if (ospf->t_ospf_recv_stats_task == NULL)
    ospf->t_ospf_recv_stats_task = thread_add_timer (master, ospf_recv_stats_task_timer,
      ospf, 1);
...
void ospf_hello_send(struct ospf_interface *oi)
{
	...
	/* djd: for vrrp virtual IP, do not send hello from this oi */
...
/* modified by liwh for bug 44873, 2017-08-14 */
   ...
   op = ospf_packet_dup (nbr->last_send);
...
/* modified by liwh for bug 45742, 2017-11-30 */
#ifdef _CENTEC_
                     " your network with areas. Use 'debug ospf packet'"
#else
				   ...
				   struct list *update, struct in_addr addr)
{
	...
	/* djd: Check that we have really something to process */
	if (listcount(update) == 0)
		return;
