...
#ifdef _CENTEC_
    {
      ospf_lsa_ignored_log_ctc(lsa->data, "LSA is MAXAGE");
      return 0;
    }
#else
    return 0;
...
#ifdef _CENTEC_
    {
      ospf_lsa_ignored_log_ctc(lsa->data, "LSA is self originated");
      return 0;
    }
#else
    return 0;
...
#ifdef _CENTEC_
    {
      ospf_lsa_ignored_log_ctc(lsa->data, "LSA match ospf area range");
      return 0;
    }
#else
    return 0;
...
#ifdef _CENTEC_
    {
      ospf_lsa_ignored_log_ctc(lsa->data, "Ignore summary default from a stub area");
      return 0;
    }
#else
...
/* modified by liwh for bug 45943, 2017-12-21 */
#ifdef _CENTEC_
   if (sl->mask.s_addr == 0xffffffff &&
       ospf_zebra_if_lookup_by_local_addr(sl->header.id))
     {
       sprintf(ignore_lsa_reason, "%s: ignoring host route %s/32 to self.",
         __func__, inet_ntoa (sl->header.id));
