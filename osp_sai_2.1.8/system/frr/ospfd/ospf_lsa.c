...
{
	struct ospf_lsa *new;
/* modified by liwh for bug 44334, 2017-06-14 
   if lsa is exist, ospf_external_lsa_new will return fail;
   therefore, delete lsa first */
#ifdef _CENTEC_
    struct ospf_lsa *lsa;
#endif
/* liwh end */
...
/* modified by liwh for bug 44334, 2017-06-14 
   ...
   therefore, delete lsa first */
#ifdef _CENTEC_
  if ((AF_INET == ei->p.family) && (0 == ei->p.prefix.s_addr) && (0 == ei->p.prefixlen))
  {
...
/* modified by liwh for bug 45819, 2017-12-28 */
#ifdef _CENTEC_
  if (0 == rt_recalc)
    {
      ospf_lsa_ignored_log_ctc(lsa->data, "lsa is same as old lsa");
    }
#endif
/* liwh end */
	/*
...
static int ospf_maxage_lsa_remover(struct thread *thread)
{
	...
	struct route_node *rn;
	int reschedule = 0;
/* modified by liwh for bug 44422, 2017-06-21 */
//#ifdef _CENTEC_
#if 0
...
/* modified by liwh for bug 44466, 2017-06-26 
   rn_top will plus rn->lock,
   rn_next will minus rn->lock, plus rn_next->lock 
   therefore, rn_next should be unlock here after optimize route_next */
//#ifdef _CENTEC_
#if 0
            /* modified by liwh for bug 44507, 2017-06-27 
              if rn_next is NULL, shouldn't unlock */
            if (rn_next)
            /* liwh end */
            {
                route_unlock_node(rn_next); 
            }
#else
				route_unlock_node(
...
/*liwh end */    
            ...
            {            
/* modified by liwh for bug 44727, 2017-07-19 */
#ifndef _CENTEC_
...
void ospf_lsa_maxage_delete(struct ospf *ospf, struct ospf_lsa *lsa)
{
    ...
    struct prefix lsa_prefix; /* djd */

    memset(&lsa_prefix, 0, sizeof(struct prefix)); /* djd */
	lsa_prefix.family = 0;
	//lsa_prefix.prefixlen = sizeof(lsa_prefix.prefix) * CHAR_BIT;
	//lsa_prefix.prefix = (uintptr_t)lsa;
	lsa_prefix.prefixlen = sizeof(lsa_prefix.u.ptr) * CHAR_BIT; /* djd */
	lsa_prefix.u.ptr = (uintptr_t)lsa; /* djd */
...
/* modified by liwh for bug 44376, 2017-07-13 */
#ifndef _CENTEC_
  if (IS_DEBUG_OSPF (lsa, LSA_REFRESH))
