...
/* modified by liwh for bug 45973, 2017-12-27 */
#ifdef _CENTEC_
   nbr->nbr_down_reason = OSPF_NEIGHOBR_DOWN_INACTIVITY_TIMTER_EXPIRE;
#endif
/*liwh end */
...
{
/* modified by liwh for bug 45595, 2017-11-19
   solve neighbour relationship can't be built when in ExStart or Exchange */
#ifdef _CENTEC_
  if ((NSM_ExStart == nbr->state) || (NSM_Exchange == nbr->state))
  {
      if (nbr->state == nbr->old_state)
      {
          nbr->num_of_recv_hello_for_unchange_state++;  
      }
      else
      {
          nbr->old_state = nbr->state;
          nbr->num_of_recv_hello_for_unchange_state = 0;
      }
  }
  else
  {
...
/* modified by liwh for bug 45595, 2017-11-19
   solve neighbour relationship can't be built when in ExStart or Exchange */
#ifdef _CENTEC_
  if (nbr->num_of_recv_hello_for_unchange_state <= 15)
  { 
#endif
/* liwh end */
...
/* SYSTEM MODIFIED by kcao for log OSPF neighbour change */
#ifdef _CENTEC_
#include "../../lib/genlog/genlog_id.h"
void
logid_sys(E_LOGID logid, ...);

void
nsm_change_state_log_ctc(struct ospf_neighbor *nbr, int old_state, int state)
{
  struct ospf_interface *oi = nbr->oi;
  char ifname[32];
  char *p_underline = NULL;

  memset(ifname, 0, sizeof(ifname));
  if (oi->ifp)
  {
    strncpy(ifname, (oi->ifp)->name, 32);
    p_underline = strstr(ifname, "_");
    if (p_underline)
    {
      p_underline[0] = '/';
    }
  ...
  if (OSPF_NEIGHOBR_DOWN_INACTIVITY_TIMTER_EXPIRE == nbr->nbr_down_reason)
    {
      logid_sys(LOG_OSPF_4_NEIGHCHG, 
        inet_ntoa(nbr->router_id),
        ifname, 
        lookup_msg(ospf_nsm_state_msg, old_state, NULL),
        lookup_msg(ospf_nsm_state_msg, state, NULL),
        "Don't receive hello packets in dead interval");

      nbr->nbr_down_reason = 0;
    }
  else if (OSPF_NEIGHOBR_DOWN_INTERFACE_DOWN == nbr->nbr_down_reason)
    {
      logid_sys(LOG_OSPF_4_NEIGHCHG, 
        inet_ntoa(nbr->router_id),
        ifname, 
...
#ifdef _CENTEC_
  /* SYSTEM MODIFIED by kcao for log OSPF neighbour change */
  nsm_change_state_log_ctc(nbr, old_state, state);
#endif /* !_CENTEC_ */
