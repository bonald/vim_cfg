...
/* upgrade frr to 3.0, modified by liwh for bug 46090, 2018-01-08 */
#ifdef _CENTEC_
...
/* SYSTEM MODIFIED by kcao for bug 43524 to convert ifname */
#ifdef _CENTEC_
int
vtysh_centec_convert_ifname(char *buf)
{
    char ifname[32];
    char *p = buf;
    char *p_ifname = NULL;
    ...
    p_ifname = strstr(p, "eth-0-");
        
    while (p_ifname)
    {
        memset(ifname, 0, sizeof(ifname));
        strncpy(ifname, p_ifname, 10);
        p_underline = strstr(ifname, "_");
        if (p_underline)
        {
            underline_offset = p_underline - ifname;
            if (underline_offset > 0)
            {
                p_ifname[underline_offset] = '/';
            }
        }
        /* jump over "eth-0-" */
        p = p_ifname + 6;
...
/* SYSTEM MODIFIED by kcao for bug 43524 to convert ifname */
#ifdef _CENTEC_
                vtysh_centec_convert_ifname(buf);
#endif
/* liwh end */
				fputs(buf, fp);
				fputc('\n', fp);
...
static int vtysh_execute_func(const char *line, int pager)
{
	...
	while (ret != CMD_SUCCESS && ret != CMD_SUCCESS_DAEMON
	       && ret != CMD_WARNING && vty->node > CONFIG_NODE) {
...
/* SYSTEM MODIFIED by kcao for debug */
#ifdef _CENTEC_
...
int vtysh_config_from_file(struct vty *vty, FILE *fp)
{
	...
	while (fgets(vty->buf, VTY_BUFSIZ, fp)) {
		...
		case CMD_WARNING:
			...
			retcode = CMD_WARNING; /* once we have an error, we
	...
	"exit-address-family", "Exit from Address Family configuration mode\n")
{
	if (vty->node == BGP_IPV4_NODE || vty->node == BGP_IPV4M_NODE
	    || vty->node == BGP_VPNV4_NODE || vty->node == BGP_VPNV6_NODE
 ...
 * and isisd. */
DEFSH(VTYSH_ZEBRA | VTYSH_RIPD | VTYSH_OSPFD, vtysh_interface_desc_cmd,
      "description LINE...",
      "Interface specific description\n"
      "Characters describing this interface\n")

DEFSH(VTYSH_ZEBRA | VTYSH_RIPD | VTYSH_OSPFD, vtysh_no_interface_desc_cmd,
...
DEFUNSH(VTYSH_ALL, vtysh_enable_password_text, vtysh_enable_password_text_cmd,
