				       ...
				       struct cpu_thread_history *a)
{
	...
	vty_out(vty, " %c%c%c%c%c%c %s%s",
		...
		a->types & (1 << THREAD_BACKGROUND) ? 'B' : ' ', a->funcname,
		VTY_NEWLINE);
}
...
static void cpu_record_print(struct vty *vty, thread_type filter)
{
	...
	hash_iterate(cpu_record, (void (*)(struct hash_backet *,
...
static struct thread *generic_thread_add(struct thread_master *m,
					 int (*func)(struct thread *),
					 void *arg, int fd, int dir,
					 debugargdef)
{
	struct thread *thread;
...
static int fd_select(struct thread_master *m, int size, thread_fd_set *read,
		     thread_fd_set *write, thread_fd_set *except,
...
static int fd_is_set(struct thread *thread, thread_fd_set *fdset, int pos)
{
#if defined(HAVE_POLL_CALL)
	return 1;
#else
...
static int fd_clear_read_write(struct thread *thread)
{
#if !defined(HAVE_POLL_CALL)
...
static struct thread *funcname_thread_add_timer_timeval(
	struct thread_master *m, int (*func)(struct thread *), int type,
	void *arg, struct timeval *time_relative, debugargdef)
{
	struct thread *thread;
	struct pqueue *queue;
...
struct thread *funcname_thread_add_timer(struct thread_master *m,
					 int (*func)(struct thread *),
					 void *arg, long timer, debugargdef)
{
	struct timeval trel;
...
static void thread_cancel_read_or_write(struct thread *thread, short int state)
{
#if defined(HAVE_POLL_CALL)
	nfds_t i;
		...
		if (thread->master->handler.pfds[i].fd == thread->u.fd) {
			...
			if (thread->master->handler.pfds[i].events == 0) {
				memmove(thread->master->handler.pfds + i,
					...
					(thread->master->handler.pfdsize - i
...
static int thread_process_fds_helper(struct thread_master *m,
				     struct thread *thread,
				     thread_fd_set *fdset, short int state,
				     int pos)
{
	struct thread **thread_array;
...
/* check poll events */
static void check_pollfds(struct thread_master *m, fd_set *readfd, int num)
{
	nfds_t i = 0;
	int ready = 0;
...
static void thread_process_fds(struct thread_master *m, thread_fd_set *rset,
			       thread_fd_set *wset, int num)
{
	...
	for (index = 0; index < m->fd_limit && ready < num; ++index) {
		ready += thread_process_fds_helper(m, m->read[index], rset, 0,
						   0);
		ready += thread_process_fds_helper(m, m->write[index], wset, 0,
						   0);
	}
#endif
}
...
struct thread *thread_fetch(struct thread_master *m, struct thread *fetch)
{
	...
	while (1) {
		...
		/* Drain the ready queue of already scheduled jobs, before
		 * scheduling
		 * more.
		 */
		 ...
		 * need to be careful to consider all thread types for
		 * scheduling
...
/* Structure copy.  */
		...
		if (m->ready.count == 0) {
			...
			timer_wait_bg =
				thread_timer_wait(m->background, &timer_val_bg);

			if (timer_wait_bg
			    && (!timer_wait
				|| (timercmp(timer_wait, timer_wait_bg, >))))
		...
		num = fd_select(m, FD_SETSIZE, &readfd, &writefd, &exceptfd,
				timer_wait);
...
void thread_call(struct thread *thread)
{
	...
	if (realtime > CONSUMED_TIME_CHECK) {
		 ...
		 * to fix.
		 */
      /* modified by liwh for bug 45737, 2017-11-27 */
      if (0 == memcmp(thread->funcname, "ospf", strlen("ospf")))
        {
          zlog_warn ("SLOW THREAD: CPU busy %lums for ospf calculate route table",
            cputime/1000);
        }
      else
      /* liwh end */
