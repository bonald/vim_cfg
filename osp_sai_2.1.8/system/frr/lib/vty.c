...
static int vty_command(struct vty *vty, char *buf)
{
	...
	if (ret != CMD_SUCCESS)
		...
		case CMD_ERR_NO_MATCH:
			vty_out(vty, "%% [%s] Unknown command: %s%s",
				protocolname, buf, VTY_NEWLINE);
			break;
...
static void vty_complete_command(struct vty *vty)
{
	...
	case CMD_ERR_AMBIGUOUS:
		...
		break;
	case CMD_ERR_NO_MATCH:
		/* vty_out (vty, "%% There is no matched command.%s",
		 * VTY_NEWLINE); */
		vty_prompt(vty);
		vty_redraw_line(vty);
		break;
	case CMD_COMPLETE_FULL_MATCH:
		if (!matched[0]) {
			...
			vty_out(vty, "%% CLI BUG: FULL_MATCH with NULL str%s",
				VTY_NEWLINE);
			vty_prompt(vty);
			vty_redraw_line(vty);
			break;
			      ...
			      unsigned int desc_width, struct cmd_token *token)
{
	...
	if (desc_width <= 0) {
		vty_out(vty, "  %-*s  %s%s", cmd_width, cmd, token->desc,
			VTY_NEWLINE);
		return;
	}
...
static void vty_describe_command(struct vty *vty)
{
	...
	for (i = 0; i < vector_active(describe); i++)
		if ((token = vector_slot(describe, i)) != NULL) {
			...
			if (!token->desc)
				vty_out(vty, "  %-s%s", token->text,
					VTY_NEWLINE);
			else if (desc_width >= strlen(token->desc))
				vty_out(vty, "  %-*s  %s%s", width, token->text,
	...
	if ((token = token_cr)) {
		if (!token->desc)
			vty_out(vty, "  %-s%s", token->text, VTY_NEWLINE);
		else if (desc_width >= strlen(token->desc))
			vty_out(vty, "  %-*s  %s%s", width, token->text,
				token->desc, VTY_NEWLINE);
		else
...
/* Return result. */
				...
				/* warning: watchquagga hardcodes this result
				 * write */
				header[3] = ret;
...
static int vty_timeout(struct thread *thread)
{
	...
	/* Clear buffer*/
	buffer_reset(vty->obuf);
	vty_out(vty, "%sVty connection is timed out.%s", VTY_NEWLINE,
		VTY_NEWLINE);
...
static void vty_event(enum event event, int sock, struct vty *vty)
{
	...
	case VTY_READ:
		...
		if (vty->v_timeout) {
			if (vty->t_timeout)
				thread_cancel(vty->t_timeout);
			vty->t_timeout = thread_add_timer(
				vty_master, vty_timeout, vty, vty->v_timeout);
		}
		break;
	case VTY_WRITE:
		if (!vty->t_write)
			vty->t_write = thread_add_write(vty_master, vty_flush,
							vty, sock);
		break;
...
static int vty_config_write(struct vty *vty)
{
	...
	if (vty_accesslist_name)
		vty_out(vty, " access-class %s%s", vty_accesslist_name,
			VTY_NEWLINE);
	...
	if (vty_timeout_val != VTY_TIMEOUT_DEFAULT)
		vty_out(vty, " exec-timeout %ld %ld%s", vty_timeout_val / 60,
...
const char *vty_get_arg_value(struct vty_arg *args[], const char *arg)
{
	while (*args) {
...
struct vty_arg *vty_get_arg(struct vty_arg *args[], const char *arg)
{
	while (*args) {
		if (strcmp((*args)->name, arg) == 0)
			return *args;
