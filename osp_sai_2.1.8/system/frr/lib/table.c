...
/* modified by liwh for bug 54074, 2019-10-22 */
#ifdef _CENTEC_
    if (node->p.family == AF_INET)
    {
        inet_ntop(AF_INET, (void*)(&node->p.u.prefix4), buf1, 32);
    }
    else
    {
       inet_ntop(AF_INET6, (void*)(&node->p.u.prefix6), buf1, 32);
    }
#endif
/* liwh end */

/*Added by chenyq to debug the bug*/
#ifdef _CENTEC_
    if(node->lock == 0)
    {
        /* modified by liwh for bug 54074, 2019-10-22 */
        if (node->p.family == AF_INET)
        {
            zlog_debug ("route_unlock_node, lock is 0!!!, route is %s/%d", buf1, node->p.prefixlen);
        }
        /* liwh end */
    }
    else
...
/*Added by chenyq to debug the bug*/
  ...
  if(node->info)
  {
    lsa = (struct ospf_lsa *)(node->info);
    
    zlog(NULL, LOG_CRIT, "route_node_delete, info is not null, route is %x/%d", node->p.u.prefix4, node->p.prefixlen);
    data = lsa->data;
     if(data)
     {
         zlog(NULL, LOG_CRIT, "lsa id %x advrouter %x ", data->id.s_addr, data->adv_router.s_addr);
     }
  }
#else
...
route_table_get_next_internal(const struct route_table *table, struct prefix *p)
{
