 ...
 */
void rib_copy_nexthops(struct rib *rib, struct nexthop *nh)
{
...
void rib_nexthop_delete(struct rib *rib, struct nexthop *nexthop)
{
	if (nexthop->next)
...
struct nexthop *rib_nexthop_ipv4_add(struct rib *rib, struct in_addr *ipv4,
				     struct in_addr *src)
{
	struct nexthop *nexthop;
...
struct nexthop *rib_nexthop_ipv4_ifindex_add(struct rib *rib,
					     struct in_addr *ipv4,
					     struct in_addr *src,
					     ifindex_t ifindex)
{
...
struct nexthop *rib_nexthop_ipv6_ifindex_add(struct rib *rib,
...
/* If force flag is not set, do not modify falgs at all for uninstall
   the route from FIB. */
static int nexthop_active(afi_t afi, struct rib *rib, struct nexthop *nexthop,
			  int set, struct route_node *top)
{
	...
	if (set) {
		...
		zebra_deregister_rnh_static_nexthops(rib->vrf_id,
  ...
  /* SYSTEM MODIFIED by wangjj for check if nexthop address is ourself, 2017-03-28 */
#ifdef _CENTEC_
  ...
  {
    /*modified by chenc for bug 48576/48577*/
    ifc = connected_check_without_prefixlen(ifp, (struct prefix *) &p);
    if (NULL != ifc) {
      return 0;
    }
  }
 ...
 /* SYSTEM MODIFIED by chenc for check if nexthop address is ourself, 2018-08-23, for bug 48576/48577 */
#ifdef _CENTEC_
    //struct connected  *ifc = NULL;
    //struct listnode   *node = NULL;
	...
	while (rn) {
		...
		if (!match) {
			...
			if (match->type == ZEBRA_ROUTE_CONNECT) {
				...
				return 1;
			} else if (CHECK_FLAG(rib->flags,
					      ZEBRA_FLAG_INTERNAL)) {
				resolved = 0;
				#if 0  
				for (newhop = match->nexthop; newhop;
				     newhop = newhop->next)
					if (CHECK_FLAG(newhop->flags,
						...
						if (set) {
							 ...
							 * route,
							 * it means the gateway
							 * we are looking up is
							 * connected
							 * to that interface.
							 * (The actual network
							 * is _not_ onlink).
							 * Therefore, the
							 * resolved route should
							 * have the original
							 * gateway as nexthop as
							 * it is directly
							 * connected.
							 *
							 * On Linux, we have to
...
struct rib *rib_match(afi_t afi, safi_t safi, vrf_id_t vrf_id,
...
struct rib *rib_match_ipv4_multicast(vrf_id_t vrf_id, struct in_addr addr,
				     struct route_node **rn_out)
{
	...
	case MCAST_MIX_MRIB_FIRST:
		rib = mrib = rib_match(AFI_IP, SAFI_MULTICAST, vrf_id, &gaddr,
				       &m_rn);
		if (!mrib)
			rib = urib = rib_match(AFI_IP, SAFI_UNICAST, vrf_id,
					       &gaddr, &u_rn);
		break;
	...
	case MCAST_MIX_PFXLEN:
		...
		urib = rib_match(AFI_IP, SAFI_UNICAST, vrf_id, &gaddr, &u_rn);
		if (mrib && urib)
			rib = u_rn->p.prefixlen > m_rn->p.prefixlen ? urib
								    : mrib;
		else if (mrib)
			rib = mrib;
		else if (urib)
				     ...
				     struct nexthop *nexthop, int set)
{
	...
	case NEXTHOP_TYPE_IPV6_IFINDEX:
		...
		if (IN6_IS_ADDR_LINKLOCAL(&nexthop->gate.ipv6)) {
			...
			if (ifp && if_is_operative(ifp))
			{
                        ...
                        /* SYSTEM MODIFIED by chenc for interface link-local ipv6 address cannot as nexthop, 2018-08-08 */
                        /*interface $DUT1_P1  
                        ipv6 address fe80::1 link-local
                        ipv6 route 2003::/64 fe80::1 $DUT1_P1
                        show ipv6 route should be inactive*/
#ifdef _CENTEC_
                        struct connected *ifc = NULL;
                        struct listnode *node;
                        struct prefix_ipv6 p_tmp;
                        int configured_address_num = 0;

                        /* Make lookup prefix. */
                        memset (&p_tmp, 0, sizeof (struct prefix_ipv6));
                        p_tmp.family = AF_INET6;
                        p_tmp.prefixlen = IPV6_MAX_PREFIXLEN;
                        p_tmp.prefix = nexthop->gate.ipv6;
                        ...
                        for (ALL_LIST_ELEMENTS_RO(ifp->connected, node, ifc)) 
                        {
                            ...
                            {
                                if( 0 == memcmp(&(p_tmp.prefix), &(ifc->address->u.prefix6), sizeof(struct in6_addr)) )
                                {
                                    UNSET_FLAG (nexthop->flags, NEXTHOP_FLAG_ACTIVE);
                                }
                                /*MODIFIED by chenc for bug 48566*/
                                else if (CHECK_FLAG(ifc->conf, ZEBRA_IFC_CONFIGURED))
                                {
                                    configured_address_num++;
                                }
                                /*end*/
                            }
                        }
	...
	if (RIB_SYSTEM_ROUTE(rib) || (family == AFI_IP && p->family != AF_INET)
 ...
 */
/*fix bug 45196 by chenc*/
#ifdef _CENTEC_
int nexthop_active_update(struct route_node *rn, struct rib *rib,
				 int set)
#else
...
/* SYSTEM MODIFIED by chenc for bug 47970, support black hole route, 2018-11-15 */
#ifdef _CENTEC_
	struct nexthop *blackhole_nh = NULL;
#endif
...
/* SYSTEM MODIFIED by chenc for bug 47970, support black hole route, 2018-11-15 */
    ...
    if (blackhole_nh)
    {
        ...
        {
            /* for ecmp, blackhole route not support ecmp  */
            if (1 <= rib->nexthop_active_num)
            {
            ...
            else
            {
                ...
                /* SYSTEM MODIFIED by chenc for bug 51500, 2019-04-02 */
                ...
                rib->nexthop_active_num +=
                nexthop_active_check (rn, rib, nh, set);

                if (CHECK_FLAG (nh->flags, NEXTHOP_FLAG_RECURSIVE))
                {
                    //new_ifindex = nh->rifindex;
                }
                else
                {
                    new_ifindex = nh->ifindex;
                }
 ...
 */
int rib_install_kernel(struct route_node *rn, struct rib *rib, struct rib *old)
{
	int ret = 0;
	struct nexthop *nexthop, *tnexthop;
	rib_table_info_t *info = srcdest_rnode_table_info(rn);
...
int rib_uninstall_kernel(struct route_node *rn, struct rib *rib)
{
	int ret = 0;
	struct nexthop *nexthop, *tnexthop;
	rib_table_info_t *info = srcdest_rnode_table_info(rn);
...
static void rib_process_update_fib(struct zebra_vrf *zvrf,
				   struct route_node *rn, struct rib *old,
				   struct rib *new)
{
	 ...
	 */
	if (new != old || CHECK_FLAG(new->status, RIB_ENTRY_CHANGED)) {
		...
		if (nh_active) {
			if (IS_ZEBRA_DEBUG_RIB) {
				...
				if (new != old)
					zlog_debug(
						"%u:%s: Updating route rn %p, rib %p (type %d) "
						"old %p (type %d)",
		...
		if (!nh_active || !installed) {
			if (IS_ZEBRA_DEBUG_RIB) {
				...
				if (new != old)
					zlog_debug(
						"%u:%s: Deleting route rn %p, rib %p (type %d) "
						"old %p (type %d) - %s",
						zvrf_id(zvrf), buf, rn, new,
						new->type, old, old->type,
					...
					zlog_debug(
						"%u:%s: Deleting route rn %p, rib %p (type %d) - %s",
						zvrf_id(zvrf), buf, rn, new,
						new->type,
						nh_active ? "install failed"
							  : "nexthop inactive");
...
static void rib_process(struct route_node *rn)
{
	...
	RNODE_FOREACH_RIB_SAFE(rn, rib, next)
	{
		if (IS_ZEBRA_DEBUG_RIB_DETAILED)
			zlog_debug(
				"%u:%s: Examine rib %p (type %d) status %x flags %x "
				"dist %d metric %d",
		...
		if (!CHECK_FLAG(rib->status, RIB_ENTRY_CHANGED)
			...
			if (rib->type == ZEBRA_ROUTE_TABLE) {
				 ...
				 * what this part
				 * does or why the RIB_ENTRY_CHANGED test above
				 * is correct
...
/* SYSTEM MODIFIED by chenc for bug 48618 48619, 2018-09-05 */
/*static route dest is same as if addr connect 32 or 128 bit route, should select connect*/
#ifdef _CENTEC_
    ...
    struct connected* ifc = NULL;
    if(IS_PREFIX_MAX_BITLEN (p))
    {
        //if(new_selected && IPI_ROUTE_CONNECT != new_selected->type && new_selected->vrf)
        if(new_selected && new_selected->type == 3)
        {
            if(p->family == AF_INET)
            {
                //if(nsm_arp_api_entry_match(new_selected->vrf, &(p.u.prefix4)))
                //new_selected = NULL;

                /* Check if dest address is ourself.
                added by ychen for local route is affected by interface link state, 2012-03-29, ychen_debug*/
                if (new_selected && (ifc = ipv4_connected_table_lookup (new_selected->vrf_id, p)) && if_is_running(ifc->ifp))
                {
                    new_selected = NULL;
                }
            }
            ...
            {
                //if(nsm_nd_api_neigh_match(nm, &(p.u.prefix6)))
                //new_selected = NULL;

                /* Check if dest address is ourself.
                added by ychen for local route is affected by interface link state, 2012-03-29, ychen_debug*/
                if (new_selected && (ifc = ipv6_connected_table_lookup (new_selected->vrf_id, p)) && if_is_running(ifc->ifp))
                {
                    new_selected = NULL;
                }
            }
...
/* modified by liwh for bug 44526, 2017-06-29 */
#ifdef _CENTEC_
...
static const u_char meta_queue_map[ZEBRA_ROUTE_MAX] = {
		[ZEBRA_ROUTE_SYSTEM] = 4,  [ZEBRA_ROUTE_KERNEL] = 0,
		[ZEBRA_ROUTE_CONNECT] = 0, [ZEBRA_ROUTE_STATIC] = 1,
		[ZEBRA_ROUTE_RIP] = 2,     [ZEBRA_ROUTE_RIPNG] = 2,
		[ZEBRA_ROUTE_OSPF] = 2,    [ZEBRA_ROUTE_OSPF6] = 2,
		[ZEBRA_ROUTE_ISIS] = 2,    [ZEBRA_ROUTE_NHRP] = 2,
		[ZEBRA_ROUTE_BGP] = 3,     [ZEBRA_ROUTE_HSLS] = 4,
		[ZEBRA_ROUTE_TABLE] = 1,
};
...
static void rib_link(struct route_node *rn, struct rib *rib, int process)
{
	struct rib *head;
	rib_dest_t *dest;
	afi_t afi;
...
void rib_addnode(struct route_node *rn, struct rib *rib, int process)
{
	...
	if (CHECK_FLAG(rib->status, RIB_ENTRY_REMOVED)) {
		if (IS_ZEBRA_DEBUG_RIB)
			rnode_debug(rn, rib->vrf_id, "rn %p, un-removed rib %p",
				    (void *)rn, (void *)rib);
 ...
 */
void rib_unlink(struct route_node *rn, struct rib *rib)
{
	rib_dest_t *dest;
	...
	if (IS_ZEBRA_DEBUG_RIB)
		rnode_debug(rn, rib->vrf_id, "rn %p, rib %p", (void *)rn,
			    (void *)rib);
...
void rib_delnode(struct route_node *rn, struct rib *rib)
{
	afi_t afi;

	if (IS_ZEBRA_DEBUG_RIB)
		rnode_debug(rn, rib->vrf_id, "rn %p, rib %p, removing",
			    (void *)rn, (void *)rib);
	...
	if (is_zebra_import_table_enabled(afi, rib->table)) {
		...
		if (IS_ZEBRA_DEBUG_RIB) {
			...
			srcdest_rnode2str(rn, buf, sizeof(buf));
			zlog_debug(
				"%u:%s: Freeing route rn %p, rib %p (type %d)",
...
void _rib_dump(const char *func, union prefixconstptr pp,
	       union prefixconstptr src_pp, const struct rib *rib)
{
	...
	zlog_debug("%s: dumping RIB entry %p for %s%s%s vrf %u", func,
		   (const void *)rib, prefix2str(pp, straddr, sizeof(straddr)),
		   is_srcdst ? " from " : "",
	...
	zlog_debug(
		...
		func, rib->refcnt, (unsigned long)rib->uptime, rib->type,
		rib->instance, rib->table);
	zlog_debug(
		"%s: metric == %u, mtu == %u, distance == %u, flags == %u, status == %u",
		func, rib->metric, rib->mtu, rib->distance, rib->flags,
		rib->status);
	zlog_debug("%s: nexthop_num == %u, nexthop_active_num == %u", func,
		   rib->nexthop_num, rib->nexthop_active_num);
...
void rib_lookup_and_dump(struct prefix_ipv4 *p, vrf_id_t vrf_id)
{
	...
	RNODE_FOREACH_RIB(rn, rib)
	{
		zlog_debug("%s: rn %p, rib %p: %s, %s", __func__, (void *)rn,
			   (void *)rib,
		      ...
		      struct prefix_ipv6 *src_p, struct rib *rib)
{
	...
	/* Lookup table.  */
	table = zebra_vrf_table_with_table_id(family, safi, rib->vrf_id,
					      rib->table);
	if (!table)
		return 0;
...
/* SYSTEM MODIFIED by chenc for kernel route not commit to fea, 2018-07-21 */
/*ff00::/8*/
#ifdef _CENTEC_
        struct in6_addr addr_cp_v6;
        struct in6_addr prefix_check_v6;
        
        memset(&prefix_check_v6, 0, sizeof(struct in6_addr));
        prefix_check_v6.__in6_u.__u6_addr8[0] = 0xff;
    
        memset(&addr_cp_v6, 0, sizeof(struct in6_addr));
        memcpy(&addr_cp_v6, &p->u.prefix6, sizeof(struct in6_addr));
        //if ((prefix_check_v6.__in6_u.__u6_addr16[0]) == (addr_cp_v6.__in6_u.__u6_addr16[0]))
        //if (0xff == (addr_cp_v6.__in6_u.__u6_addr8[0]))
        if ((afi == AFI_IP6) && 0 == memcmp(&prefix_check_v6, &addr_cp_v6, sizeof(struct in6_addr)) && 8 == p->prefixlen)
        {
            return 0;
        }
        ...
        /*link-local address and no shutdown auto generate address not add rib*/
        /*fe80::/64     eth-0-1*/
        if (IN6_IS_ADDR_LINKLOCAL(&p->u.prefix6))
        {
            // TODO: cc linklocal address not add to rib
            return 0;
        }
#endif
/* chenc */

/* SYSTEM MODIFIED by chenc for vedefault & vedefault2 route not add to rib, 2018-07-26 */
/*delete those route*/
/*
C * fe80::/64 is directly connected, vedefault2
C>* fe80::/64 is directly connected, vedefault
*/
#ifdef _CENTEC_
        char ifname[64] = {0};
        strcpy(ifname, ifindex2ifname(ifindex, vrf_id));
        if (0 == memcmp(ifname, "vedefault", sizeof("vedefault")) || 0 == memcmp(ifname, "vedefault2", sizeof("vedefault2")))
        {
            return 0;
        }
#endif
/* chenc */
	...
	rib->vrf_id = vrf_id;
	rib->nexthop_num = 0;
/* modified by liwh for bug 44607, 2017-08-15 */
#ifdef _CENTEC_  
  rib->uptime = zfpm_get_time ();
#else
...
/*liwh end */
	...
	if (gate) {
		if (afi == AFI_IP6) {
			if (ifindex)
				rib_nexthop_ipv6_ifindex_add(rib, &gate->ipv6,
							     ifindex);
			else
			...
			if (ifindex)
				rib_nexthop_ipv4_ifindex_add(
					rib, &gate->ipv4, &src->ipv4, ifindex);
			else
				rib_nexthop_ipv4_add(rib, &gate->ipv4,
						     &src->ipv4);
		}
	} else
