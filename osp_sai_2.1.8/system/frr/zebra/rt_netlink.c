...
/* SYSTEM MODIFIED by wangjj for fix bug 43054, 2017-03-06 */
#ifdef _CENTEC_
    struct in_addr addr_cp;
    struct in_addr prefix_check;
    prefix_check.s_addr = 0xA9FE0000;
    int need_filter = 0;
#endif
/* wangjj */
...
/* SYSTEM MODIFIED by wangjj for fix bug 43054, 2017-03-06 */
#ifdef _CENTEC_
          {
            ...
            if (NULL != gate)
            {
                memset(&addr_cp, 0, sizeof(struct in_addr));
                memcpy(&addr_cp, gate, sizeof(struct in_addr));
                if ((prefix_check.s_addr >> 16) == (addr_cp.s_addr>>16) )
                {
                    need_filter = TRUE;
                }
            }
				   ...
				   int update)
{
	...
	if ((rib->flags & ZEBRA_FLAG_BLACKHOLE)
		...
		/*
		 *modified by chenc start, 2019-05-11
		 *config blackhole route, then config normal route but nexthop is inactive, then config ip address to make nexthop active, so normal route will replace blackhole route, 
		 *then delete ip address to make nexthop inactive, blackhole route will be wrong, blackhole route should be active
		 *reason: kernel accept blackhole route rtm_type should be RTN_BLACKHOLE, 
		 *so here make flags to be ZEBRA_FLAG_BLACKHOLE, then make rtm_type to be RTN_BLACKHOLE
		 */
#ifdef _CENTEC_
        if (0 == discard)
        {
            ...
            {
                if (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_ACTIVE) && cmd == RTM_NEWROUTE && nexthop->type == NEXTHOP_TYPE_BLACKHOLE)
                {
                    discard = 1;
...
/*linux kernel not support ipv6 replace*/
#ifdef _CENTEC_
    int family = PREFIX_FAMILY(p);
    if (family == AF_INET)
    {
