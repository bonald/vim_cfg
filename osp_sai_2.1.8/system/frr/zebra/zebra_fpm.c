	...
	do {
		...
		if (write_msg) {
			data_len = zfpm_encode_route(dest, rib, (char *)data,
                    ...
                    /*configure ecmp 2, but one of ecmp is inactive, then delete the active one, here data_len will be 0,
                    so here change rib to null to make add route to delete route*/
                    #ifdef _CENTEC_
                    if (0 == data_len && is_add)
                    {
                        data_len = zfpm_encode_route(dest, NULL, (char *)data,
						     buf_end - data, &msg_type);
			   is_add = 0;
			   assert(data_len);
                    }
...
#ifdef _CENTEC_
int
ctc_zfpm_init(struct thread_master *master)
{
