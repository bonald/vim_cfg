			  ...
			  struct prefix_ipv6 *src_p, struct static_route *si)
{
	...
	if (rib) {
		...
		case STATIC_IPV6_GATEWAY_IFNAME:
			nexthop = rib_nexthop_ipv6_ifindex_add(
				rib, &si->addr.ipv6, si->ifindex);
			break;
		}
		...
		if (IS_ZEBRA_DEBUG_RIB) {
			...
			if (IS_ZEBRA_DEBUG_RIB) {
				inet_ntop(p->family, &p->u.prefix, buf,
					  INET6_ADDRSTRLEN);
				zlog_debug(
					"%u:%s/%d: Modifying route rn %p, rib %p (type %d)",
					si->vrf_id, buf, p->prefixlen, rn, rib,
					rib->type);
			}
		}
		...
		if (IS_ZEBRA_DEBUG_RIB) {
			...
			if (IS_ZEBRA_DEBUG_RIB) {
				inet_ntop(p->family, &p->u.prefix, buf,
					  INET6_ADDRSTRLEN);
				zlog_debug(
					"%u:%s/%d: Inserting route rn %p, rib %p (type %d)",
					si->vrf_id, buf, p->prefixlen, rn, rib,
					rib->type);
			}
		}
		/* Link this rib to the tree. Schedule for processing or invoke
		 * NHT,
		 * as appropriate.
		 */
		if (si->type == STATIC_IPV4_GATEWAY
...
/* SYSTEM MODIFIED by chenc for bug 47970, support black hole route, 2018-11-15 */
#ifdef _CENTEC_
	int update = false;
	int has_black_hole = false;
	struct nexthop *nexthop_black_hole = NULL;
#endif
	...
	RNODE_FOREACH_RIB(rn, rib)
	{
		...
		if (rib->type == ZEBRA_ROUTE_STATIC
		    && rib->distance == si->distance && rib->tag == si->tag)
			break;
	}
...
/* SYSTEM MODIFIED by chenc for bug 47970, support black hole route, 2018-11-15 */
#ifdef _CENTEC_
	if(has_black_hole && nexthop_black_hole != nexthop && rib->nexthop_active_num == 1)
	{
	    update = true;
	}
#endif
		...
		if (CHECK_FLAG(nexthop->flags, NEXTHOP_FLAG_FIB)) {
			...
			if (rib->nexthop_active_num > 1) {
				...
				if (CHECK_FLAG(rib->flags, ZEBRA_FLAG_SELECTED))
					redistribute_update(
						p, (struct prefix *)src_p, rib,
						NULL);
			} else {
				/* Remove from redistribute if selected route
				 * becomes inactive */
				 /* modified by liwh for bug 51072, 2019-02-22 */
				// if (CHECK_FLAG(rib->flags, ZEBRA_FLAG_SELECTED))
				if (CHECK_FLAG(rib->flags, ZEBRA_FLAG_SELECTED) && (!update))
                              /* liwh end */
					redistribute_delete(
						p, (struct prefix *)src_p, rib);
...
/*fix bug 45196 by chenc*/
	...
	{
	    int active_count = 0;
	    active_count = nexthop_active_update(rn, rib, 1);
	}
	...
	if(update)
	{
	    ...
	    rib_install_kernel(rn, rib, NULL);

            /* modified by liwh for bug 51072, 2019-02-22 */
            redistribute_update(p, NULL, rib, NULL);
            /* liwh end */
	}
#endif
	/* Unlock node. */
		     ...
		     struct static_nh_label *snh_label)
{
	...
	if (!ifname
	    && (type == STATIC_IFNAME || type == STATIC_IPV6_GATEWAY_IFNAME))
		return -1;
