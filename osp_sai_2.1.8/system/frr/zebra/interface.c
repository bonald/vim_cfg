...
/* SYSTEM MODIFIED by kcao to set link-detect, 2017-05-17 */
#ifdef _CENTEC_
...
#ifdef _CENTEC_
int
get_connect_old(struct route_node *rn)
{
    ...
    for (ALL_LIST_ELEMENTS_RO(vrf->iflist, ifnode, ifp)) 
    {
        for (ALL_LIST_ELEMENTS_RO(ifp->connected, node, ifc)) 
        {
            ...
            {
                if( 0 == memcmp(&(rn->p.u.prefix4), &(ifc->address->u.prefix4), sizeof(ifc->address->u.prefix4)) && 32 == rn->p.prefixlen)
                {
                    struct rib *rib;
                    struct rib *next;
                    ...
                    {
                        if (rib->nexthop->type == NEXTHOP_TYPE_IPV4)
                        {
                            return TRUE;
                        }
                    }
...
struct connected *
get_connect(vrf_id_t vrf_id, struct in_addr *addr)
{
    struct vrf *vrf = vrf_lookup_by_id(0);
    struct interface *ifp;
    struct connected *ifc = NULL;
    ...
    for (ALL_LIST_ELEMENTS_RO(vrf->iflist, ifnode, ifp)) 
    {
        for (ALL_LIST_ELEMENTS_RO(ifp->connected, node, ifc)) 
        {
            ...
            {
                //if( 0 == memcmp(&(rn->p.u.prefix4), &(ifc->address->u.prefix4), sizeof(ifc->address->u.prefix4)) && 32 == rn->p.prefixlen)
                if( 0 == memcmp(addr, &(ifc->address->u.prefix4), sizeof(ifc->address->u.prefix4)))
                {
                    return ifc;
                }
            }
...
struct connected *
get_connect_v6(vrf_id_t vrf_id, struct in6_addr *addr)
{
    struct vrf *vrf = vrf_lookup_by_id(0);
    struct interface *ifp;
    struct connected *ifc = NULL;
    ...
    for (ALL_LIST_ELEMENTS_RO(vrf->iflist, ifnode, ifp)) 
    {
        for (ALL_LIST_ELEMENTS_RO(ifp->connected, node, ifc)) 
        {
            ...
            {
                //if( 0 == memcmp(&(rn->p.u.prefix4), &(ifc->address->u.prefix4), sizeof(ifc->address->u.prefix4)) && 32 == rn->p.prefixlen)
                if( 0 == memcmp(addr, &(ifc->address->u.prefix6), sizeof(ifc->address->u.prefix6)))
                {
                    return ifc;
                }
            }
...
struct connected *
ifc_lookup_by_ipv4_address (vrf_id_t vrf_id, struct in_addr *addr)
{
    return get_connect(vrf_id, addr);
}

struct connected *
ifc_lookup_by_ipv6_address (vrf_id_t vrf_id, struct in6_addr *addr)
{
    return get_connect_v6(vrf_id, addr);
}

struct connected *
ipv4_connected_table_lookup (vrf_id_t vrf_id, struct prefix *p)
{
  return ifc_lookup_by_ipv4_address(vrf_id, &(p->u.prefix4));
}

struct connected *
ipv6_connected_table_lookup (vrf_id_t vrf_id, struct prefix *p)
{
    return ifc_lookup_by_ipv6_address(vrf_id, &(p->u.prefix6));
				...
				const char *label)
{
	...
	if (!CHECK_FLAG(ifc->conf, ZEBRA_IFC_QUEUED)
		...
		listnode_delete(ifp->connected, ifc);
		connected_free(ifc);
/* SYSTEM MODIFIED by chenc for bug 49630, 2018-10-23 */
#ifdef _CENTEC_
              return CMD_SUCCESS;
#else
		return CMD_WARNING;
				  ...
				  const char *label, int secondry)
{
	...
	ifc = connected_check(ifp, (struct prefix *)&cp);
	if (!ifc) {
		//modify by zhw for IPv6
		//vty_out(vty, "%% Can't find IPv6 address %s %s", addr_str, VTY_NEWLINE);
		return CMD_WARNING;
	}

	/* This is not configured address. */
	if (!CHECK_FLAG(ifc->conf, ZEBRA_IFC_CONFIGURED))
	/*SYSTEM MODIFIED by chenc for
            default linux generate a link-local address, then first configure link-local address, linux will exist 2 link-local, 
            so delete the linux link-local address first, the address is not configured*/
	#ifdef _CENTEC_
        {
            if (IN6_IS_ADDR_LINKLOCAL(&ifc->address->u.prefix6))
            {
                ...
                if (ret < 0)
                {
                    /*shutdown will delete address by call vty cmd, but normal ipv6 address not call if_prefix_delete_ipv6, 
                    here linklocal will call, so change it to log*/
                    /*errno 19 is No such device, for bug 48567*/
                    if (errno != 19)
                    {
                        vty_out(vty, "%% Can't unset interface linux auto link-local IPv6 address %s: %s.%s",
...
/* SYSTEM MODIFIED by chenc for first add normal ipv6 address, then add link-local address, 
it will delete mac link-local, but mac link-local is not active, cannot been delete,
add this to delete from kernel, 2018-07-21 */
#ifdef _CENTEC_
        if (IN6_IS_ADDR_LINKLOCAL(&ifc->address->u.prefix6))
        {
            ret = if_prefix_delete_ipv6(ifp, ifc);
            if (ret < 0)
            {
                ...
                here linklocal will call, so change it to log*/
                /*errno 19 is No such device, for bug 48567*/
                /*shutdown will delete address by call vty cmd, when if only have one link_local address, 
                shutdown will from linux more fast then vty*/
                /*errno 99 is Cannot assign requested address, for bug */
                if (errno != 19 && errno != 99)
                {
                    vty_out(vty, "%% Can't unset interface link-local IPv6 address %s: %s.%s",
                    prefix2str(ifc->address, addrbuf, sizeof(addrbuf)),
                    safe_strerror(errno), VTY_NEWLINE);
                }
                /* SYSTEM MODIFIED by chenc for bug 48498*/
                #ifdef _CENTEC_
                return CMD_SUCCESS;
                #else
                return CMD_WARNING;
...
/* chenc */
    ...
    #ifdef _CENTEC_
        /*errno 19 is No such device, for bug 48567*/
        /*errno 99 is Cannot assign requested address, for bug 49504*/
        if (errno != 19 && errno != 99)
        {
            memset(addrbuf, 0, sizeof(addrbuf));
            vty_out(vty, "%% Can't unset interface IPv6 address %s: %s.%s",
            prefix2str(ifc->address, addrbuf, sizeof(addrbuf)),
            safe_strerror(errno), VTY_NEWLINE);
            return CMD_WARNING;
        }
