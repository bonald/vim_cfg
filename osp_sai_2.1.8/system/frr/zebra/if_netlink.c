...
#ifdef _CENTEC_
    /* SYSTEM MODIFIED by chenc for interface ipv6 address 7fff::1/48 connect route miss, 2018-10-17 */
    if (ifa->ifa_family != AF_INET6)
    {
        /* SYSTEM MODIFIED by wangjj for filtering some special connect route entry, 2015-12-11 */
        unsigned int  addr_check = 0;
        unsigned int  addr_check2 = 0;  
        struct in_addr addr_cp;

        addr_check = htonl(0xA9FE0000);
        addr_check2 = htonl(0x7f000000);  
        memcpy(&addr_cp, addr, sizeof(struct in_addr));
        if (addr_check == (addr_cp.s_addr & addr_check) || addr_check2 == (addr_cp.s_addr & addr_check2))
        {
          return 0;
        }
    }
			...
			ns_id_t ns_id, int startup)
{
	...
	if (h->nlmsg_type == RTM_NEWLINK) {
		...
		if (ifp == NULL
			...
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug(
					"RTM_NEWLINK for %s(%u) (ifp %p) vrf_id %u flags 0x%x",
					name, ifi->ifi_index, ifp, vrf_id,
					ifi->ifi_flags);
			...
			if (ifp == NULL) {
				...
				if (ifp->vrf_id != vrf_id)
					if_update(ifp, name, strlen(name),
						  vrf_id);
			}
			...
			if (IS_ZEBRA_DEBUG_KERNEL)
				zlog_debug(
					"RTM_NEWLINK status for %s(%u) flags 0x%x",
