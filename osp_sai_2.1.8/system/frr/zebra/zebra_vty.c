		      ...
		      const char *vrf_id_str, const char *label_str)
{
	...
	if (!zvrf) {
		vty_out(vty, "%% vrf %s is not defined%s", vrf_id_str,
			VTY_NEWLINE);
		return CMD_WARNING;
	}
	...
	if (flag_str) {
		...
		case 'B': /* XXX */
			...
			break;
		default:
			vty_out(vty, "%% Malformed flag %s %s", flag_str,
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	}
...
/* SYSTEM MODIFIED by chenc for bug 49515, 2018-11-16 */
#ifdef _CENTEC_
static void vty_show_ipv6_route_linklocal_detail(struct vty *vty)
{
	vty_out(vty, "Routing entry for fe80::/10%s",
		VTY_NEWLINE);
			
	vty_out(vty, "  Known via \"connected");
	vty_out(vty, "\"");
	vty_out(vty, ", distance 0, metric 0");
	vty_out(vty, ", best");
				     ...
				     int mcast)
{
	...
	RNODE_FOREACH_RIB(rn, rib)
	{
		...
		vty_out(vty, "\"");
		vty_out(vty, ", distance %u, metric %u", rib->distance,
			rib->metric);
		if (rib->tag)
...
#ifdef _CENTEC_
      uptime = zfpm_get_time ();
      uptime = uptime - rib->uptime;
      tm = localtime(&uptime);
#else
			uptime = time(NULL);
			uptime -= rib->uptime;
...
static void vty_show_ip_route(struct vty *vty, struct route_node *rn,
			      struct rib *rib, json_object *json)
{
	struct nexthop *nexthop, *tnexthop;
	int recursing;
	int len = 0;
...
/* modified by chenc for bug 51079, 2019-02-20 */
#ifndef _CENTEC_
		if (CHECK_FLAG(rib->flags, ZEBRA_FLAG_BLACKHOLE))
			vty_out(vty, ", bh");
#endif
...
/* modified by chenc for bug 48465, 2018-09-06 */
#ifdef _CENTEC_
/* SYSTEM MODIFIED by chenc for bug 49515, 2018-11-16 */
static int vty_show_ip_route_v6_has_link_local_route()
{

    struct vrf *vrf = vrf_lookup_by_id(0);
    struct interface *ifp;
    struct connected *ifc = NULL;
    struct listnode *ifnode;
    struct listnode *node;

    for (ALL_LIST_ELEMENTS_RO(vrf->iflist, ifnode, ifp)) 
    {
        if (0 == memcmp(ifp->name, "vedefault", sizeof("vedefault")) || 0 == memcmp(ifp->name, "vedefault2", sizeof("vedefault2")))
        {
            continue;
        }
...
static void vty_show_ip_route_v6_link_local(struct vty *vty, int first)
{
    ...
    for (ALL_LIST_ELEMENTS_RO(vrf->iflist, ifnode, ifp)) 
    {
        ...
        for (ALL_LIST_ELEMENTS_RO(ifp->connected, node, ifc)) 
        {
                ...
                && (ifc->address->family == AF_INET6))
            {
                ...
                vty_out(vty, "C>* fe80::/10 is directly connected, Null0");
                vty_out(vty, "%s", VTY_NEWLINE);
                return;
            }
        }
			    ...
			    u_char use_json)
{
	...
	if (!(zvrf = zebra_vrf_lookup_by_name(vrf_name))) {
		if (use_json)
			...
			vty_out(vty, "vrf %s not defined%s", vrf_name,
				VTY_NEWLINE);
		return CMD_SUCCESS;
	}
	...
	if (zvrf_id(zvrf) == VRF_UNKNOWN) {
		if (use_json)
			...
			vty_out(vty, "vrf %s inactive%s", vrf_name,
				VTY_NEWLINE);
		return CMD_SUCCESS;
	}
       ...
       VRF_ALL_CMD_HELP_STR)
{
	...
	if ((zvrf = vrf->info) != NULL) {
		vty_out(vty, "%sVRF %s:%s", VTY_NEWLINE, zvrf_name(zvrf),
			VTY_NEWLINE);
       ...
       "Tag value\n")
{
	...
	if (strmatch(argv[idx_vrf]->text, "vrf")) {
		...
		VTY_GET_INTEGER_RANGE("tag", tag, argv[idx_tag]->arg, 0,
				      4294967295);
	} else {
		idx_tag -= 2;
		VTY_GET_INTEGER_RANGE("tag", tag, argv[idx_tag]->arg, 0,
       ...
       "Show route matching the specified Network/Mask pair only\n")
{
   ...
   /* modified by chenc for bug 45654 */
#ifdef _CENTEC_
   if (1 == first)
   {
...
#define CTCSDK_PRINT(fmt, args...)                             \
    ...
    fp_console = fopen("/tmp/ospf_route_num", "w+");                   \
...
#define ZEBRA_ROUTE_TOTAL (ZEBRA_ROUTE_IBGP + 1)
	...
	vty_out(vty, "%-20s %-20s %s  (vrf %s)%s", "Route Source", "Routes",
		...
		if ((rib_cnt[i] > 0) || (i == ZEBRA_ROUTE_BGP
			...
			if (i == ZEBRA_ROUTE_BGP) {
				vty_out(vty, "%-20s %-20d %-20d %s", "ebgp",
					...
					fib_cnt[ZEBRA_ROUTE_BGP], VTY_NEWLINE);
				vty_out(vty, "%-20s %-20d %-20d %s", "ibgp",
					rib_cnt[ZEBRA_ROUTE_IBGP],
					fib_cnt[ZEBRA_ROUTE_IBGP], VTY_NEWLINE);
			} else
				vty_out(vty, "%-20s %-20d %-20d %s",
					zebra_route_string(i), rib_cnt[i],
					fib_cnt[i], VTY_NEWLINE);
		}
	}

	vty_out(vty, "------%s", VTY_NEWLINE);
	vty_out(vty, "%-20s %-20d %-20d %s", "Totals",
...
/*write ospf fib num to file*/
#ifdef _CENTEC_
static void vty_show_ip_route_summary_write(struct vty *vty,
				      struct route_table *table)
{
	struct route_node *rn;
	struct rib *rib;
...
#define ZEBRA_ROUTE_TOTAL (ZEBRA_ROUTE_IBGP + 1)
	...
	u_int32_t i;
	u_int32_t is_ibgp;
	u_int32_t has_write_file = FALSE;
	...
	for (i = 0; i < ZEBRA_ROUTE_MAX; i++) 
	{
		if ((rib_cnt[i] > 0) || (i == ZEBRA_ROUTE_BGP
					 && rib_cnt[ZEBRA_ROUTE_IBGP] > 0)) 
		{
			if (i != ZEBRA_ROUTE_BGP && 0 == strcmp(zebra_route_string(i), "ospf")) 
			{
			    CTCSDK_PRINT("%-20d", fib_cnt[i]);
			    has_write_file = TRUE;
			}
		}
	}
...
#define ZEBRA_ROUTE_TOTAL (ZEBRA_ROUTE_IBGP + 1)
	...
	for (rn = route_top(table); rn; rn = srcdest_route_next(rn))
		RNODE_FOREACH_RIB(rn, rib)
		{
			 ...
			 */
			cnt = 0;
			for (nexthop = rib->nexthop; (!cnt && nexthop);
	...
	vty_out(vty, "%-20s %-20s %s  (vrf %s)%s", "Route Source",
		"Prefix Routes", "FIB",
...
/*write ospf fib num to file*/
#ifdef _CENTEC_
DEFUN (show_ip_route_summary_write,
       show_ip_route_summary_write_cmd,
       ...
       "Write ospf fib num to file\n")
{
	...
	vty_show_ip_route_summary_write(vty, table);
	...
	RB_FOREACH(vrf, vrf_name_head, &vrfs_by_name)
	{
		...
		for (rn = route_top(table); rn; rn = route_next(rn))
			RNODE_FOREACH_RIB(rn, rib)
			{
				...
				if (vrf_header) {
					vty_out(vty, "%sVRF %s:%s", VTY_NEWLINE,
						zvrf_name(zvrf), VTY_NEWLINE);
					vrf_header = 0;
				}
				vty_show_ip_route(vty, rn, rib, NULL);
			}
       ...
       "Tag value\n")
{
	...
	if (argv[idx_number]->arg)
		VTY_GET_INTEGER_RANGE("tag", tag, argv[idx_number]->arg, 0,
				      4294967295);
		     ...
		     const char *vrf_id_str, const char *label_str)
{
	...
	if (src_str) {
		...
		if (ret <= 0 || src.family != AF_INET6) {
			vty_out(vty, "%% Malformed source address%s",
				VTY_NEWLINE);
			return CMD_WARNING;
		}
	...
	if ((gate_str != NULL)
		...
		if (add_cmd)
			static_add_route(AFI_IP6, SAFI_UNICAST, type, &p, src_p,
					 NULL, ifname, ZEBRA_FLAG_BLACKHOLE,
					 tag, distance, zvrf, &snh_label);
		else
			static_delete_route(AFI_IP6, SAFI_UNICAST, type, &p,
					    src_p, NULL, ifname, tag, distance,
					    zvrf, &snh_label);
		return CMD_SUCCESS;
	}
	...
	if (add_cmd)
		static_add_route(AFI_IP6, SAFI_UNICAST, type, &p, src_p,
				 (union g_addr *)gate, ifname, flag, tag,
				 distance, zvrf, &snh_label);
	else
		static_delete_route(AFI_IP6, SAFI_UNICAST, type, &p, src_p,
				    (union g_addr *)gate, ifname, tag, distance,
				    zvrf, &snh_label);
       ...
       "Output JSON\n")
{
	...
	if (vrf) {
		if (!(zvrf = zebra_vrf_lookup_by_name(vrfname))) {
			if (uj)
				...
				vty_out(vty, "vrf %s not defined%s", vrfname,
					VTY_NEWLINE);
			return CMD_SUCCESS;
		}
		...
		if (zvrf_id(zvrf) == VRF_UNKNOWN) {
			if (uj)
				...
				vty_out(vty, "vrf %s inactive%s", vrfname,
					VTY_NEWLINE);
			return CMD_SUCCESS;
		} else
			vrf_id = zvrf_id(zvrf);
	...
	if (uj) {
		...
		/* modified by chenc for bug 48465, 2018-09-06 */
              #ifdef _CENTEC_
		vty_show_ip_route_v6_link_local(vty, first);
		#endif
            /*chenc end */
	}
...
/* SYSTEM MODIFIED by chenc for bug 49515, 2018-11-16 */
#ifdef _CENTEC_
	    if (IN6_IS_ADDR_LINKLOCAL(&p.prefix) && true == vty_show_ip_route_v6_has_link_local_route())
	    {
	        vty_show_ipv6_route_linklocal_detail(vty);
	        return CMD_SUCCESS;
	    }
#endif
	    vty_out(vty, "%% Network not in table%s", VTY_NEWLINE);
...
/* SYSTEM MODIFIED by chenc for bug 49515, 2018-11-16 */
#ifdef _CENTEC_
	    if (IN6_IS_ADDR_LINKLOCAL(&p.prefix) && p.prefixlen == 10
	        && true == vty_show_ip_route_v6_has_link_local_route())
	    {
	        vty_show_ipv6_route_linklocal_detail(vty);
	        return CMD_SUCCESS;
	    }
       ...
       "route-map name\n")
{
	...
	if (argc == 7 || (argc == 5 && !rmap))
		VTY_GET_INTEGER_RANGE("distance", distance, argv[4]->arg, 1,
				      255);
...
/*write ospf fib num to file*/
#ifdef _CENTEC_
	install_element(VIEW_NODE, &show_ip_route_summary_write_cmd);
#endif
