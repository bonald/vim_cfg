 ...
 */
static struct rib *zebra_rnh_resolve_entry(vrf_id_t vrfid, int family,
					   rnh_type_t type,
...
/*liwh end */
			 ...
			 * registered nexthops.
			 */
			for (nexthop = srib->nexthop; nexthop;
			     nexthop = nexthop->next) {
				switch (nexthop->type) {
				case NEXTHOP_TYPE_IPV4:
				case NEXTHOP_TYPE_IPV4_IFINDEX:
...
static void print_rnh(struct route_node *rn, struct vty *vty)
{
	...
	if (!list_isempty(rnh->zebra_static_route_list))
		vty_out(vty, " zebra[static routes]%s",
