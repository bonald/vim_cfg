...
#ifdef _CENTEC_
struct connected *connected_check_without_prefixlen(struct interface *ifp, struct prefix *p)
{
	struct connected *ifc;
	struct listnode *node;

	for (ALL_LIST_ELEMENTS_RO(ifp->connected, node, ifc))
		if (prefix_same_without_prefixlen(ifc->address, p))
			return ifc;
...
/* SYSTEM MODIFIED by chenc for bug 48729, 2018-08-29 */
/*ipv6 address 1::1/12 after apply mask, is ::/12(unspecified)*/
#ifdef _CENTEC_
#else
#ifndef LINUX
...
/* SYSTEM MODIFIED by chenc for bug 49384, 2018-10-17 */
/*config unspecified ipv6 address, then delete it, show ipv6 route remain existed*/
#ifndef _CENTEC_
	if (IN6_IS_ADDR_UNSPECIFIED(&p.u.prefix6))
		return;
#endif
