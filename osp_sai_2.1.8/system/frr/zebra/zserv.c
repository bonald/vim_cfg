...
static int zserv_flush_data(struct thread *thread)
{
	...
	case BUFFER_ERROR:
		...
		break;
	case BUFFER_PENDING:
		client->t_write = thread_add_write(
			zebrad.master, zserv_flush_data, client, client->sock);
		break;
	case BUFFER_EMPTY:
		break;
	}
...
int zebra_server_send_message(struct zserv *client)
{
	...
	case BUFFER_ERROR:
		...
		/* Schedule a delayed close since many of the functions that
		   ...
		   be
		   deleted. */
		client->t_suicide = thread_add_event(
			zebrad.master, zserv_delayed_close, client, 0);
		return -1;
	case BUFFER_EMPTY:
...
static int zsend_ipv4_nexthop_lookup_mrib(struct zserv *client,
					  struct in_addr addr, struct rib *rib,
	...
	if (CHECK_FLAG(message, ZAPI_MESSAGE_NEXTHOP)) {
		...
		for (i = 0; i < nexthop_num; i++) {
			...
			case NEXTHOP_TYPE_IPV4_IFINDEX:
				nexthop.s_addr = stream_get_ipv4(s);
				ifindex = stream_getl(s);
				rib_nexthop_ipv4_ifindex_add(rib, &nexthop,
							     NULL, ifindex);
				break;
			case NEXTHOP_TYPE_IPV6:
			      ...
			      vrf_id_t vrf_id)
{
	...
	if (command == ZEBRA_MPLS_LABELS_ADD) {
		...
		mpls_lsp_uninstall(zvrf, type, in_label, gtype, &gate, NULL,
				   ifindex);
...
static void zebra_event(enum event event, int sock, struct zserv *client)
{
	...
	case ZEBRA_SERV:
		...
		break;
	case ZEBRA_READ:
		client->t_read = thread_add_read(
			zebrad.master, zebra_client_read, client, sock);
		break;
	case ZEBRA_WRITE:
		/**/
		break;
...
static void zebra_show_client_detail(struct vty *vty, struct zserv *client)
{
	...
	if (client->nh_reg_time) {
		...
		vty_out(vty, "Not registered for Nexthop Updates%s",
			VTY_NEWLINE);
	...
	vty_out(vty, "IPv6        %-12d%-12d%-12d%s", client->v6_route_add_cnt,
		...
		VTY_NEWLINE);
	vty_out(vty, "Redist:v4   %-12d%-12d%-12d%s", client->redist_v4_add_cnt,
		0, client->redist_v4_del_cnt, VTY_NEWLINE);
	vty_out(vty, "Redist:v6   %-12d%-12d%-12d%s", client->redist_v6_add_cnt,
		0, client->redist_v6_del_cnt, VTY_NEWLINE);
       ...
       "Brief Summary\n")
{
	...
	vty_out(vty, "Routes column shows (added+updated)/deleted%s",
		VTY_NEWLINE);
	return CMD_SUCCESS;
}
