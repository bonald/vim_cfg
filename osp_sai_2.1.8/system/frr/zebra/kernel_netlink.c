		...
		for (h = (struct nlmsghdr *)buf;
			...
			if (h->nlmsg_type == NLMSG_ERROR) {
				...
				if (msg_type == RTM_DELNEIGH
					...
					&& (-errnum == ESRCH
					    || -errnum == ENETUNREACH
                                       /* SYSTEM MODIFIED by chenc for bug 48613, 2018-07-21 */
                                        #ifdef _CENTEC_
                                        || -errnum == 19
                                        #endif
                                        /*chenc end*/
                                        /* SYSTEM MODIFIED by chenc for bug 51996, 2018-07-21 */
                                        #ifdef _CENTEC_
                                        || -errnum == 22/*Invalid argument*/
                                        #endif
                                        /*chenc end*/
                                        /* SYSTEM MODIFIED by chenc for bug 51940, 2019-05-09 */
                                        #ifdef _CENTEC_
                                        )) || (nl == &zns->netlink_cmd
					...
					zlog_err(
						...
						err->msg.nlmsg_pid);
                            /* SYSTEM MODIFIED by chenc for bug 51940, 2019-05-09 */
                            #ifdef _CENTEC_
                            if (nl == &zns->netlink_cmd && msg_type == RTM_DELADDR && -errnum == 19)
                            {
                                return 0;
                            }
                            /*chenc end*/
                            /* SYSTEM MODIFIED by chenc for bug 51994, 2019-05-13 */
                            else if (nl == &zns->netlink_cmd && msg_type == RTM_NEWROUTE && -errnum == ENETUNREACH )
                            {
                                return 0;
                            }
                            /*chenc end*/
                            /* SYSTEM MODIFIED by chenc for bug 51996, 2019-05-13 */
                            else if (nl == &zns->netlink_cmd && msg_type == RTM_NEWROUTE && -errnum == 22 )
                            {
                                return 0;
                            }
                            /*chenc end*/
