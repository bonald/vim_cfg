...
hagt_openflow_port_enable(Hsrv2AgtMsg_t* p_msg_data)
{
    ...
    if (req->enable)
    {
        ...
        /* added by hansf for support ipv6 profile */
        if (is_openflow_ipv6_profile)
        {
            ctc_port_set_direction_property(req->gport, CTC_PORT_DIR_PROP_ACL_TCAM_LKUP_TYPE_0, CTC_INGRESS, CTC_ACL_TCAM_LKUP_TYPE_L2_L3);
        }
    ...
    else
    {
       ...
       // HAGT_IF_ERROR_RETURN(ctc_port_set_reflective_bridge_en(req->gport, FALSE));
        ...
        if (is_openflow_ipv6_profile)
        {
            ctc_port_set_direction_property(req->gport, CTC_PORT_DIR_PROP_ACL_TCAM_LKUP_TYPE_0, CTC_INGRESS, CTC_ACL_TCAM_LKUP_TYPE_L2);
        }
