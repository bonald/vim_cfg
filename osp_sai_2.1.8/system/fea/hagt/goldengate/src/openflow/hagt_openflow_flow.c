...
hagt_openflow_map_mac(HalMsgOpenFlowMacRule_t* p_mac, ctc_acl_ipv4_key_t* p_ipv4_key)
{
    ...
    GLB_SET_FLAG(p_ipv4_key->flag, CTC_ACL_IPV4_KEY_FLAG_L3_TYPE);
    p_ipv4_key->l3_type = 0;
    p_ipv4_key->l3_type_mask = 0;
    ...
    {
        if(p_mac->ethTypeMask == 0xffff)
        {
            if(0x0800 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_IPV4;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else if(0x0806 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_ARP;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            /* mod by hansf for bug 50234, support default profile match ipv6 packet */
            #if 0
            else if(0x86DD == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_IPV6;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            #endif
            else if(0x8847 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_MPLS;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else if(0x8848 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_MPLS_MCAST;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else if(0x8906 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_FCOE;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else if(0x22F3 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_TRILL;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else if(0x8902 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_ETHER_OAM;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else if(0x88E7 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_CMAC;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else if(0x8809 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_SLOW_PROTO;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else if(0x88F7 == p_mac->ethType)
            {
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_PTP;
                p_ipv4_key->l3_type_mask = 0xF;
            }
            else
            {
                ...
                p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_NONE;
                p_ipv4_key->l3_type_mask = 0xF;
            }
        }
        else
...
hagt_openflow_map_mac_ipv6(HalMsgOpenFlowMacRule_t* p_mac, ctc_acl_ipv6_key_t* p_ipv6_key)
{
    ...
    if (GLB_FLAG_ISSET(p_mac->flag, GLB_OPENFLOW_MAC_RULE_FLAG_ETHTYPE))
    {
        if(p_mac->ethTypeMask == 0xffff)
        {
            if(0x0800 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_IPV4;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x0806 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_ARP;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x86DD == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_IPV6;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x8847 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_MPLS;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x8848 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_MPLS_MCAST;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x8906 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_FCOE;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x22F3 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_TRILL;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x8902 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_ETHER_OAM;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x88E7 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_CMAC;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x8809 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_SLOW_PROTO;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else if(0x88F7 == p_mac->ethType)
            {
                p_ipv6_key->l3_type = CTC_PARSER_L3_TYPE_PTP;
                p_ipv6_key->l3_type_mask = 0xF;
            }
            else
            {
...
hagt_openflow_map_ipv4(HalMsgOpenFlowIpv4Rule_t* p_ipv4, ctc_acl_ipv4_key_t* p_ipv4_key)
{
    ...
    {
        GLB_SET_FLAG(p_ipv4_key->flag, CTC_ACL_IPV4_KEY_FLAG_ECN);
        p_ipv4_key->ecn = p_ipv4->ecn;
        p_ipv4_key->ecn_mask = p_ipv4->ecnMask;
    }
    if (GLB_FLAG_ISSET(p_ipv4->flag, GLB_OPENFLOW_IPV4_RULE_FLAG_L4_SRC_PORT))
    {
        if(!GLB_FLAG_ISSET(p_ipv4_key->sub_flag, CTC_ACL_IPV4_KEY_SUB_FLAG_VNI))
        {
...
hagt_openflow_map_ipv6(HalMsgOpenFlowIpv6Rule_t* p_ipv6, ctc_acl_ipv6_key_t* p_ipv6_key)
{
    ...
    {
        GLB_SET_FLAG(p_ipv6_key->flag, CTC_ACL_IPV6_KEY_FLAG_ECN);
        p_ipv6_key->ecn = p_ipv6->ecn;
        p_ipv6_key->ecn_mask = p_ipv6->ecnMask;
    }
...
static int32
hagt_openflow_map_mpls(HalMsgOpenFlowMplsRule_t* p_mpls, ctc_acl_ipv4_key_t* p_ipv4_key)
{
    GLB_SET_FLAG(p_ipv4_key->flag, CTC_ACL_IPV4_KEY_FLAG_L3_TYPE);
    p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_MPLS;
    p_ipv4_key->l3_type_mask = 0xF;
    ...
    {
        GLB_SET_FLAG(p_ipv4_key->sub1_flag, CTC_ACL_IPV4_KEY_SUB1_FLAG_MPLS_LABEL_NUM);
        p_ipv4_key->mpls_label_num = p_mpls->mplsLabelNum;
        p_ipv4_key->mpls_label_num_mask = 0xff;
    }
    
    if (p_mpls->mplsLabel0Mask & GLB_MPLS_LABEL_MASK)
    {
        GLB_SET_FLAG(p_ipv4_key->sub1_flag, CTC_ACL_IPV4_KEY_SUB1_FLAG_MPLS_LABEL0);
        p_ipv4_key->mpls_label0 |= (uint32)p_mpls->mplsLabel0;
        p_ipv4_key->mpls_label0_mask |= GLB_MPLS_LABEL_MASK;
    }

    if (p_mpls->mplsLabel1Mask & GLB_MPLS_LABEL_MASK)
    {
        GLB_SET_FLAG(p_ipv4_key->sub1_flag, CTC_ACL_IPV4_KEY_SUB1_FLAG_MPLS_LABEL1);
        p_ipv4_key->mpls_label1 |= (uint32)p_mpls->mplsLabel1;
        p_ipv4_key->mpls_label1_mask |= GLB_MPLS_LABEL_MASK;
    }
    
    if (p_mpls->mplsLabel2Mask & GLB_MPLS_LABEL_MASK)
    {
        GLB_SET_FLAG(p_ipv4_key->sub1_flag, CTC_ACL_IPV4_KEY_SUB1_FLAG_MPLS_LABEL2);
        p_ipv4_key->mpls_label2 |= (uint32)p_mpls->mplsLabel2;
        p_ipv4_key->mpls_label2_mask |= GLB_MPLS_LABEL_MASK;
    }
...
static int32
hagt_openflow_map_mpls_mcast(HalMsgOpenFlowMplsRule_t* p_mpls, ctc_acl_ipv4_key_t* p_ipv4_key)
{
    GLB_SET_FLAG(p_ipv4_key->flag, CTC_ACL_IPV4_KEY_FLAG_L3_TYPE);
    p_ipv4_key->l3_type = CTC_PARSER_L3_TYPE_MPLS_MCAST;
    p_ipv4_key->l3_type_mask = 0xF;
...
static int32
hagt_openflow_map_tun_id_without_tun_port(HalMsgOpenFlowTnlRule_t* p_tnl,HalMsgOpenFlowIpv4Rule_t* p_ipv4, ctc_acl_ipv4_key_t* p_ipv4_key)
{
    uint32 is_set = 0;
    ...
    if(!GLB_FLAG_ISSET(p_tnl->flags, GLB_OPENFLOW_TNL_RULE_FLAG_METADATA))
    {
        if(GLB_FLAG_ISSET(p_ipv4->flag, GLB_OPENFLOW_IPV4_RULE_FLAG_L4_PROTO))
        {
            ...
            if(p_ipv4->l4Protocol == 47)
            {
                ...
                {
                    GLB_SET_FLAG(p_ipv4_key->sub_flag, CTC_ACL_IPV4_KEY_SUB_FLAG_NVGRE_KEY);
                    p_ipv4_key->gre_key = p_tnl->tunId;
                    p_ipv4_key->gre_key_mask = p_tnl->tunId_mask;
                    if (p_ipv4_key->gre_key_mask > 0xFFFFFF)
                    {
                        p_ipv4_key->gre_key_mask = 0xFFFFFF;
                    }
                }
            }
        }
        ...
        if(GLB_FLAG_ISSET(p_ipv4->flag, GLB_OPENFLOW_IPV4_RULE_FLAG_L4_DST_PORT))
        {
            ...
            {
                GLB_SET_FLAG(p_ipv4_key->sub_flag, CTC_ACL_IPV4_KEY_SUB_FLAG_VNI);
                GLB_UNSET_FLAG(p_ipv4_key->sub_flag, CTC_ACL_IPV4_KEY_SUB_FLAG_L4_DST_PORT);
                if(GLB_FLAG_ISSET(p_tnl->flags, GLB_OPENFLOW_TNL_RULE_FLAG_TUN_ID))
                {
                    p_ipv4_key->vni = p_tnl->tunId;
                    p_ipv4_key->vni_mask = p_tnl->tunId_mask;
                    if (p_ipv4_key->vni_mask > 0xFFFFFF)
                    {
                        p_ipv4_key->vni_mask = 0xFFFFFF;
                    }
                }
            }
        }
...
hagt_openflow_map_udf(HalMsgOpenFlowUdfRule_t* p_udf, ctc_acl_ipv4_key_t* p_ipv4_key)
{
    ...
    // udfId==0, mean l3_type, otherwise mean l4_type
    if (p_udf->udfId)
    {
    ...
    if (match_udf)
    {
        ...
        p_ipv4_key->udf[0] = (p_udf->udf0 >> 24) & 0xff;
        p_ipv4_key->udf[1] = (p_udf->udf0 >> 16) & 0xff;
        p_ipv4_key->udf[2] = (p_udf->udf0 >> 8) & 0xff;
        p_ipv4_key->udf[3] = (p_udf->udf0) & 0xff;
        p_ipv4_key->udf_mask[0] = (p_udf->udfMask0 >> 24) & 0xff;
        p_ipv4_key->udf_mask[1] = (p_udf->udfMask0 >> 16) & 0xff;
        p_ipv4_key->udf_mask[2] = (p_udf->udfMask0 >> 8) & 0xff;
        p_ipv4_key->udf_mask[3] = (p_udf->udfMask0) & 0xff;
    }
...
hagt_openflow_map_ipv6_udf(HalMsgOpenFlowUdfRule_t* p_udf, ctc_acl_ipv6_key_t* p_ipv6_key)
{
    ...
    if (match_udf)
    {
        p_ipv6_key->udf[0] = (p_udf->udf0 >> 24) & 0xff;
        p_ipv6_key->udf[1] = (p_udf->udf0 >> 16) & 0xff;
        p_ipv6_key->udf[2] = (p_udf->udf0 >> 8) & 0xff;
        p_ipv6_key->udf[3] = (p_udf->udf0) & 0xff;
        p_ipv6_key->udf_mask[0] = (p_udf->udfMask0 >> 24) & 0xff;
        p_ipv6_key->udf_mask[1] = (p_udf->udfMask0 >> 16) & 0xff;
        p_ipv6_key->udf_mask[2] = (p_udf->udfMask0 >> 8) & 0xff;
        p_ipv6_key->udf_mask[3] = (p_udf->udfMask0) & 0xff;
    }
...
{
    /* Added by weizj for udf */
    uint32 udf_enable = HAGT_OPENFLOW_FLOW_UDF_ENABLE;

    p_acl_key->type = CTC_ACL_KEY_IPV4;
    p_acl_key->u.ipv4_key.key_size = CTC_ACL_KEY_SIZE_DOUBLE;
    ...
    case unionRule_PR_NOTHING:
        ...
        {
            hagt_openflow_set_metadata_flag(udf_enable, p_acl_key, TRUE);
        }
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_mac(&p_rule->macRule, &p_acl_key->u.ipv4_key));
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_tnl(&p_rule->tnlRule, &p_acl_key->u.ipv4_key));
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_udf(&p_rule->udfRule, &p_acl_key->u.ipv4_key));
        break;
    ...
    case unionRule_PR_ipv4Rule:
        ...
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_tun_id_without_tun_port(&p_rule->tnlRule,&p_rule->unionRule.ipv4Rule, &p_acl_key->u.ipv4_key));
        break;
    ...
    case unionRule_PR_mplsRule:
        ...
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_tnl(&p_rule->tnlRule, &p_acl_key->u.ipv4_key));
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_udf(&p_rule->udfRule, &p_acl_key->u.ipv4_key));
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_mpls(&p_rule->unionRule.mplsRule, &p_acl_key->u.ipv4_key));
        break;
    ...
    case unionRule_PR_mplsMcastRule:
        ...
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_tnl(&p_rule->tnlRule, &p_acl_key->u.ipv4_key));
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_udf(&p_rule->udfRule, &p_acl_key->u.ipv4_key));
        HAGT_IF_ERROR_RETURN(hagt_openflow_map_mpls_mcast(&p_rule->unionRule.mplsRule, &p_acl_key->u.ipv4_key));
        break;
...
hagt_openflow_map_flow_action(HalMsgOpenFlowAction_t* p_action, ctc_acl_action_t* p_acl_action)
{
    ...
    {
        GLB_SET_FLAG(p_acl_action->flag, CTC_ACL_ACTION_FLAG_FORCE_MAC_LEARNING);
        GLB_UNSET_FLAG(p_acl_action->flag, CTC_ACL_ACTION_FLAG_DENY_LEARNING);
    }
    ...
    if (GLB_FLAG_ISSET(p_action->groupVlanType, GLB_GROUP_VLAN_PUSH))
    {
        ...
        {
            GLB_SET_FLAG(p_acl_action->flag, CTC_ACL_ACTION_FLAG_VLAN_EDIT);
            p_acl_action->vlan_edit.stag_op = CTC_ACL_VLAN_TAG_OP_ADD;
            p_acl_action->vlan_edit.svid_sl = CTC_ACL_VLAN_TAG_SL_NEW;
            p_acl_action->vlan_edit.svid_new = p_action->groupVlanId;
            if (GLB_FLAG_ISSET(p_action->groupVlanType, GLB_GROUP_VLAN_SET_PCP))
            {
                p_acl_action->vlan_edit.scos_sl = CTC_ACL_VLAN_TAG_SL_NEW;
                p_acl_action->vlan_edit.scos_new = p_action->groupVlanCos;
            }
        }
    }
    ...
    else if (GLB_FLAG_ISSET(p_action->groupVlanType, GLB_GROUP_VLAN_SET))
    {
        ...
        {
            GLB_SET_FLAG(p_acl_action->flag, CTC_ACL_ACTION_FLAG_VLAN_EDIT);
            p_acl_action->vlan_edit.stag_op = CTC_ACL_VLAN_TAG_OP_REP_OR_ADD;
            p_acl_action->vlan_edit.svid_sl = CTC_ACL_VLAN_TAG_SL_NEW;
            p_acl_action->vlan_edit.svid_new = p_action->groupVlanId;
            if (GLB_FLAG_ISSET(p_action->groupVlanType, GLB_GROUP_VLAN_SET_PCP))
            {
    ...
    {
        GLB_SET_FLAG(p_acl_action->flag, CTC_ACL_ACTION_FLAG_VLAN_EDIT);
        p_acl_action->vlan_edit.stag_op = CTC_ACL_VLAN_TAG_OP_DEL;
    }
    /*end hansf*/
    ...
    {
        p_acl_action->packet_strip.start_packet_strip = CTC_ACL_STRIP_START_TO_L4;
        p_acl_action->packet_strip.packet_type = PKT_TYPE_ETH;
    }
    ...
    if (p_action->stripLen)
    {
        p_acl_action->packet_strip.start_packet_strip = CTC_ACL_STRIP_START_TO_L4;
        p_acl_action->packet_strip.packet_type = PKT_TYPE_ETH;
        p_acl_action->packet_strip.strip_extra_len = p_action->stripLen;
    }
    ...
    if (GLB_OPENFLOW_FLOW_TABLE_ID_ACL == req->entryId.table)
    {
        ...
        if (req->action.isDsMplsKey)
        {
            ...
            if (req->action.policerId)
            {
                ...
                ilm_info.policer_id = ctc_policer_id;
            }
