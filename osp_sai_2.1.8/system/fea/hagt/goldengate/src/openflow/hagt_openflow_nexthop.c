...
hagt_openflow_map_tunnel_nexthop(HalMsgOpenFlowTunnelNh_t* p_nh, ctc_ip_tunnel_nh_param_t* p_param)
{
    ...
    /*modified by wangl for bug 46662*/
    p_param->tunnel_info.dscp_select= CTC_NH_DSCP_SELECT_MAP;
...
hagt_openflow_add_tunnel_nh(Hsrv2AgtMsg_t* p_msg_data)
{
    ...
    if(GLB_DROP_PORT == req->nhInfo.gport)
    {
        nh_param.flag = CTC_IP_NH_FLAG_UNROV;
    }
    if(req->isUpdate)
    {
        HAGT_IF_ERROR_RETURN(ctc_nh_update_ip_tunnel(req->nhid, &nh_param));
    }
    else
    {
...
hagt_openflow_nexthop_init(void)
{
    ...
    scl_entry.key.u.tcam_ipv4_key.l3_type = CTC_PARSER_L3_TYPE_MPLS;
    ...
    scl_entry.key.type = CTC_SCL_KEY_TCAM_IPV4;
    scl_entry.key.u.tcam_ipv4_key.flag |= CTC_SCL_TCAM_IPV4_KEY_FLAG_L3_TYPE;
    scl_entry.key.u.tcam_ipv4_key.l3_type = CTC_PARSER_L3_TYPE_MPLS_MCAST;
    scl_entry.key.u.tcam_ipv4_key.l3_type_mask = 0xff;
    scl_entry.key.u.tcam_ipv4_key.key_size = CTC_SCL_KEY_SIZE_DOUBLE;
    ...
    scl_entry.key.u.tcam_ipv4_key.flag |= CTC_SCL_TCAM_IPV4_KEY_FLAG_L3_TYPE;
    scl_entry.key.u.tcam_ipv4_key.l4_protocol = 47;
    ...
    scl_entry.key.u.tcam_ipv4_key.l4_dst_port = 4789;
