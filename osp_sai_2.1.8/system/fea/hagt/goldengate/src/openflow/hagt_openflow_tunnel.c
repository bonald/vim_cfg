...
int32
hagt_openflow_tunnel_set_acl_for_vxlan(uint32 destport)
{
        /*vxlan*/
    ...
    scl_entry.key.u.tcam_ipv4_key.l4_protocol = 17;
    scl_entry.key.u.tcam_ipv4_key.l4_protocol_mask = 0xff;

    scl_entry.key.u.tcam_ipv4_key.sub_flag |= CTC_SCL_TCAM_IPV4_KEY_SUB_FLAG_L4_DST_PORT;
    scl_entry.key.u.tcam_ipv4_key.l4_dst_port = destport;
    scl_entry.key.u.tcam_ipv4_key.l4_dst_port_mask = 0xffff;
...
int32
_hagt_openflow_tunnel_unset_acl_for_vxlan(void)
{
    HAGT_IF_ERROR_RETURN(ctc_scl_uninstall_entry(GLB_FLOW_ENTRY_MAX- 3));
    HAGT_IF_ERROR_RETURN(ctc_scl_remove_entry(GLB_FLOW_ENTRY_MAX- 3));
...
_hagt_openflow_tunnel_set_vxlan_destport(Hsrv2AgtMsg_t* p_msg_data)
{
    ...
    if(req->isSet)
    {
        ...
        HAGT_IF_ERROR_RETURN(_hagt_openflow_tunnel_unset_acl_for_vxlan());
        HAGT_IF_ERROR_RETURN(hagt_openflow_tunnel_set_acl_for_vxlan(destport));
    }
    else
    {
        HAGT_IF_ERROR_RETURN(ctc_global_ctl_set(CTC_GLOBAL_VXLAN_UDP_DEST_PORT, &destport));
        HAGT_IF_ERROR_RETURN(_hagt_openflow_tunnel_unset_acl_for_vxlan());
        HAGT_IF_ERROR_RETURN(hagt_openflow_tunnel_set_acl_for_vxlan(4789));
    }
