...
hagt_openflow_port_set_protected_vlan(Hsrv2AgtMsg_t* p_msg_data)
{
    ...
    if(req->enable)
    {
        ...
        key_field.data = req->vlanId;
        key_field.mask = 0xffff;
        HAGT_IF_ERROR_RETURN(ctc_acl_add_key_field(acl_entry.entry_id, &key_field));
...
hagt_openflow_port_enable(Hsrv2AgtMsg_t* p_msg_data)
{
    ...
    if (req->enable)
    {
        ...
        {
            HAGT_IF_ERROR_RETURN(ctc_port_set_direction_property(req->gport, CTC_PORT_DIR_PROP_ACL_TCAM_LKUP_TYPE_0, CTC_INGRESS, CTC_ACL_TCAM_LKUP_TYPE_L2_L3_EXT));
        }
        else
        {
            HAGT_IF_ERROR_RETURN(ctc_port_set_direction_property(req->gport, CTC_PORT_DIR_PROP_ACL_TCAM_LKUP_TYPE_0, CTC_INGRESS, CTC_ACL_TCAM_LKUP_TYPE_L2_L3));
        }

        HAGT_IF_ERROR_RETURN(ctc_port_set_property(req->gport, CTC_PORT_PROP_METADATA_OVERWRITE_PORT, TRUE)); 
        HAGT_IF_ERROR_RETURN(ctc_port_set_property(req->gport, CTC_PORT_PROP_ADD_DEFAULT_VLAN_DIS, 1));
        //HAGT_IF_ERROR_RETURN(ctc_port_set_untag_dft_vid(req->gport, 0, 0));
        //HAGT_IF_ERROR_RETURN(ctc_port_set_direction_property(req->gport, CTC_PORT_DIR_PROP_ACL_EN, CTC_BOTH_DIRECTION, 1)); 
    ...
    else
    {
        ...
        HAGT_IF_ERROR_RETURN(ctc_port_set_property(req->gport, CTC_PORT_PROP_METADATA_OVERWRITE_PORT, FALSE)); 
        HAGT_IF_ERROR_RETURN(ctc_port_set_direction_property(req->gport, CTC_PORT_DIR_PROP_ACL_EN, CTC_BOTH_DIRECTION, 0)); 
        //HAGT_IF_ERROR_RETURN(ctc_port_set_untag_dft_vid(req->gport, 1, 1));
        HAGT_IF_ERROR_RETURN(ctc_port_set_property(req->gport, CTC_PORT_PROP_ADD_DEFAULT_VLAN_DIS, 0));
...
hagt_openflow_port_enable(Hsrv2AgtMsg_t* p_msg_data)
{
    HalMsgOpenFlowPortEnableReq_t *req;
    ctc_port_scl_property_t prop;
    ctc_acl_property_t p_prop;
    
    req = &p_msg_data->opHsrv2Agt.hsrv2AgtMsgOpenFlowPortEnable;

    sal_memset(&prop, 0x0, sizeof(ctc_port_scl_property_t));
    sal_memset(&p_prop, 0x0, sizeof(ctc_acl_property_t));
    ...
    if (req->enable)
    {
        ...
        p_prop.direction = CTC_INGRESS; 
        HAGT_IF_ERROR_RETURN(ctc_port_get_acl_property(req->gport, &p_prop));

        p_prop.acl_en = TRUE;
        p_prop.acl_priority = 0;
        p_prop.class_id = req->label;
        p_prop.direction = CTC_INGRESS;
        p_prop.tcam_lkup_type = CTC_ACL_TCAM_LKUP_TYPE_L2_L3;
        CTC_SET_FLAG(p_prop.flag, CTC_ACL_PROP_FLAG_USE_METADATA);
        HAGT_IF_ERROR_RETURN(ctc_port_set_acl_property(req->gport, &p_prop));

        p_prop.direction = CTC_EGRESS;
        HAGT_IF_ERROR_RETURN(ctc_port_set_acl_property(req->gport, &p_prop));
...
//        HAGT_IF_ERROR_RETURN(ctc_sai_port_openflow_set_openflow_enable(req->gport, 1));
    ...
    else
    {
        ...
        p_prop.direction = CTC_INGRESS;
        p_prop.tcam_lkup_type = CTC_ACL_TCAM_LKUP_TYPE_L2_L3;
        CTC_UNSET_FLAG(p_prop.flag, CTC_ACL_PROP_FLAG_USE_METADATA);
        HAGT_IF_ERROR_RETURN(ctc_port_set_acl_property(req->gport, &p_prop));
