...
#define MAX_PRINT_STR_LEN 512  
    ...
    {
        sal_snprintf(log_buf, MAX_PRINT_STR_LEN, "Asic log interrupt %d.%d.%d=[0x%x][0x%x][0x%x]\n", p_status->type.intr, 
                     p_status->type.sub_intr, p_status->type.low_intr,p_status->status.bmp[0],p_status->status.bmp[1],p_status->status.bmp[2]);
