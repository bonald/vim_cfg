...
_hagt_qos_get_flow_policer_stats(HalMsgStatsKey_t* p_stats_key, glb_stats_result_t* p_stats)
{
    ...
    HAGT_QOS_POLICER_DEBUG_INFO("confirm stats: pkts (%"PRIu64"), bytes (%"PRIu64")\n", 
                                policer_stats.stats.confirm_pkts, policer_stats.stats.confirm_bytes);
    HAGT_QOS_POLICER_DEBUG_INFO("exceed stats : pkts (%"PRIu64"), bytes (%"PRIu64")\n", 
                                policer_stats.stats.exceed_pkts, policer_stats.stats.exceed_bytes);
    HAGT_QOS_POLICER_DEBUG_INFO("violate stats: pkts (%"PRIu64"), bytes (%"PRIu64")\n", 
...
/*added by wangl for openflow queue stats*/
    ...
    HAGT_QOS_POLICER_DEBUG_INFO("--------------------------------------\n");
    HAGT_QOS_POLICER_DEBUG_INFO("deq stats : pkts (%"PRIu64"), bytes (%"PRIu64")\n", 
                                queue_stats.stats.deq_packets, queue_stats.stats.deq_bytes);
    HAGT_QOS_POLICER_DEBUG_INFO("drop stats: pkts (%"PRIu64"), bytes (%"PRIu64")\n", 
...
//    int32 queueid;
    ...
    for (queueid = 0; queueid < HAGT_QOS_PORT_QUEUE_NUM; queueid++)
    {
        ...
        ctc_get_port_queue_depth(req->port, queueid, &queue_depth);
        if (CTC_E_NONE != ret)
        {
            goto FINALLY;
        }
