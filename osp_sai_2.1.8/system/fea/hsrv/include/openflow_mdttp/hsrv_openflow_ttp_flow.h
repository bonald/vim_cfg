...
struct hsrv_ingress_port_flow_match_s
{
    uint32  inport;     /*  tunnel port id is 0x00010000 | id, or phy port 0x00000000 | id, for phy port is gport */

    uint32  ifindex;    /* for tunnel port, save ifindex */

    uint32  vni;        /* vxlan vni */
};
typedef struct hsrv_ingress_port_flow_match_s hsrv_ingress_port_flow_match_t;
...
struct hsrv_ingress_acl_flow_match_s
{
    ...
    uint64    metadata; /*just for match metadata generating by decap vxlan scl, for acl match ingress tunnel port */
...
struct hsrv_ingress_port_flow_entry_s
{
    hsrv_ingress_port_flow_match_t  match_criteria;
    
    uint32  goto_table;

    uint64  metadata;   /*set fid*/

    uint16 openflow_metadata; /* dsTunnel->metadata, used by acl match key */
};
typedef struct hsrv_ingress_port_flow_entry_s hsrv_ingress_port_flow_entry_t;
...
struct hsrv_openflow_ttp_flow_entry_s
{
    ...
    union
    {
        ...
        hsrv_ingress_port_flow_entry_t             ingress_port_flow_entry;
