 ...
 *************************/
hsrv_openflow_ttp_group_master_t *g_pst_openflow_ttp_group_master;
...
{
    hsrv_openflow_ttp_group_bucket_entry_t p_l2_bucket;
    hsrv_openflow_ttp_tunnel_info_t *p_tunnel = NULL;
    uint32 nexthop_id = 0;

    sal_memset(&p_l2_bucket, 0, sizeof(hsrv_openflow_ttp_group_bucket_entry_t));
    p_l2_bucket.bucket_data.l2_interface.ifindex = p_packet_out->ifindex;
    p_l2_bucket.bucket_data.l2_interface.vni = p_packet_out->tunnel_id;
    /* set overlay tunnel mapping */
    HSRV_IF_ERROR_RETURN(hsrv_openflow_add_metadata_mapping(p_packet_out->tunnel_id, METADATA_TYPE_FID));
    if (GLB_FLAG_ISSET(p_packet_out->flag, GLB_OPENFLOW_MPLS_NH_LABEL_IS_SET_VLAN))
    {
        p_l2_bucket.bucket_data.l2_interface.vlan_id = p_packet_out->vlan_id;
    }

    p_tunnel = hsrv_openflow_tunnel_get_tunnel_info(p_packet_out->ifindex);
    if (!p_tunnel)
    {
    ...
    /*alloc nhid*/
    HSRV_IF_ERROR_RETURN(ctc_ttp_drv_nexthop_alloc_nhid(&nexthop_id));
    *nhid = nexthop_id;   

    HSRV_IF_ERROR_RETURN(ctc_ttp_drv_nexthop_add_vxlan_tunnel_bridge_nh(&p_l2_bucket, nexthop_id, HSRV_INVALID_STATS_ID)); 
...
{
    HSRV_IF_ERROR_RETURN(ctc_ttp_drv_nexthop_del_vxlan_tunnel_bridge_nh(nhid));
    HSRV_IF_ERROR_RETURN(ctc_ttp_drv_nexthop_release_nhid(nhid));
    HSRV_IF_ERROR_RETURN(hsrv_openflow_del_metadata_mapping(tunnel_id));
    return HSRV_E_NONE;
}
...
static int32
hsrv_openflow_group_get_first_bucket(hsrv_openflow_ttp_group_t *p_group, hsrv_openflow_ttp_group_bucket_entry_t** pp_group_bucket)
{
    ...
    p_node = ctclib_list_head(&p_group->bucket_index_list);
    if (p_node)
    {
...
_hsrv_openflow_group_add_l3_unicast_group(hsrv_openflow_ttp_group_t* p_group)
{
    ...
    ctclib_list_for_each_safe(p_node, p_next, p_list)
    {
        ...
        if (p_l3_bucket)
        {
            ...
            if (p_l2_bucket)
            {
                if (HSRV_UINT32_MAX != p_l2_bucket->bucket_data.l2_interface.vni)
                {
                    //output port is tunnel, or IS_TUNNEL_PORT(port_num)
                    HSRV_IF_ERROR_RETURN(ctc_ttp_drv_nexthop_add_vxlan_tunnel_route_nh(p_l3_bucket, p_l2_bucket, p_group->nh_id, p_group->stats_id));
                }
                else
                {
                    //output port is physical port
                    /*add asic nh*/
        ...
        //now l3 unicast group just has a bucket
        break;
    }
...
{
    hsrv_openflow_ttp_group_bucket_entry_t *l2_bucket = NULL;
    hsrv_openflow_ttp_group_bucket_entry_t *l3_bucket = NULL;
    
    HSRV_OPENFLOW_MEM_CHECK(p_group);

    hsrv_openflow_group_get_group_first_bucket(p_group->group_id, &l3_bucket);
    if (!l3_bucket)
    {
        return HSRV_E_ENTRY_NOT_EXIST;
    }

    hsrv_openflow_group_get_group_first_bucket(l3_bucket->reference_group_id, &l2_bucket);
    if (!l2_bucket)
    {
        return HSRV_E_ENTRY_NOT_EXIST;
    }
    
    if (HSRV_UINT32_MAX != l2_bucket->bucket_data.l2_interface.vni)
    {
        //output port is tunnel, or IS_TUNNEL_PORT(port_num)
        HSRV_IF_ERROR_RETURN(ctc_ttp_drv_nexthop_del_vxlan_tunnel_route_nh(p_group->nh_id));
    }
    else
    {
        //output port is physical port
...
_hsrv_openflow_group_add_l2_interface_group(hsrv_openflow_ttp_group_t* p_group)
{
    ...
    ctclib_list_for_each_safe(p_node, p_next, p_list)
    {
        ...
        if (p_db_bucket)
        {
            if (HSRV_UINT32_MAX != p_db_bucket->bucket_data.l2_interface.vni)
            {
                //output port is tunnel, or IS_TUNNEL_PORT(port_num)
                HSRV_IF_ERROR_RETURN(ctc_ttp_drv_nexthop_add_vxlan_tunnel_bridge_nh(p_db_bucket, p_group->nh_id, p_group->stats_id));
            }
            else
            {
                //output port is physical port
        ...
        //now l2_interface_group just has a bucket
        break;
    }
...
_hsrv_openflow_group_del_l2_interface_group(hsrv_openflow_ttp_group_t* p_group)
{
    ...
    hsrv_openflow_group_get_group_first_bucket(p_group->group_id, &l2_bucket);
    if (!l2_bucket)
    {
        return HSRV_E_ENTRY_NOT_EXIST;
    }
    ...
    {
        //output port is tunnel, or IS_TUNNEL_PORT(port_num)
        HSRV_IF_ERROR_RETURN(ctc_ttp_drv_nexthop_del_vxlan_tunnel_bridge_nh(p_group->nh_id));
        HSRV_IF_ERROR_RETURN(hsrv_openflow_del_metadata_mapping(l2_bucket->bucket_data.l2_interface.vni));
    }
    else
    {
        //output port is physical port
...
_hsrv_openflow_group_add_bucket_entry(glb_openflow_bucket_t* p_bucket, uint32 bucket_index, uint32 group_id)
{
    ...
    glb_openflow_port_type_t port_type = GLB_OF_PORT_TYPE_NONE;
    ...
    for(action_num = 0; action_num < p_bucket->n_actions; action_num ++)
    {
        ...
        switch(p_action->type)
        {
            case GLB_OPENFLOW_ACTION_TYPE_OUTPUT:
                ...
                port_type = p_action->value.output.type;
    ...
    {
        if (GLB_OF_PORT_TYPE_TUNNEL == port_type)
        {
            p_hsrv_bucket->bucket_data.l2_interface.vni = tunnel_id;
            p_hsrv_bucket->bucket_data.l2_interface.ifindex = ifindex;
            p_hsrv_bucket->bucket_data.l2_interface.output_port = ofport;
            p_hsrv_bucket->bucket_data.l2_interface.output_port |= TTP_TUNNEL_PORT_TYPE;
            HSRV_IF_ERROR_RETURN(hsrv_openflow_add_metadata_mapping(tunnel_id, METADATA_TYPE_FID));
        }
        else
        {
            ...
            p_hsrv_bucket->bucket_data.l2_interface.output_port = p_port->gport | TTP_PHYIF_PORT_TYPE;
            p_hsrv_bucket->bucket_data.l2_interface.vni = HSRV_UINT32_MAX;
            p_hsrv_bucket->bucket_data.l2_interface.ifindex = ifindex;
        }
...
_hsrv_openflow_group_alloc_group(hsrv_openflow_ttp_group_t* pp_group, glb_openflow_group_t* p_fei_group)
{
    ...
    pp_group->stats_supported = p_fei_group->stats_en;
...
{
    uint32 stats_id = 0;
    hsrv_openflow_ttp_group_bucket_entry_t *p_l2_bucket = NULL;
    /*alloc stats*/
    if (p_group->stats_supported)
    {
        if (GLB_OPENFLOW_TTP_GROUP_TYPE_L2_INTERFACE == GLB_OPENFLOW_TTP_GROUP_TYPE(p_group->group_id))
        {
            hsrv_openflow_group_get_first_bucket(p_group, &p_l2_bucket);
            if (!p_l2_bucket)
            {
                return HSRV_E_ENTRY_NOT_EXIST;
            }
            if (IS_TUNNEL_PORT(p_l2_bucket->bucket_data.l2_interface.output_port))
            {
                //tunnel stats type
                HSRV_IF_ERROR_RETURN(hsrv_openflow_add_stats(GLB_STATS_TYPE_OPENFLOW_GROUP, p_group->group_id, p_group->group_id, TRUE, &stats_id));
            }
            else
            {
                HSRV_IF_ERROR_RETURN(hsrv_openflow_add_stats(GLB_STATS_TYPE_OPENFLOW_GROUP, p_group->group_id, p_group->group_id, FALSE, &stats_id));
            }
        }
        else
        {
...
fei_api_openflow_add_group(glb_openflow_group_t *p_fei_group)
{
    ...
    HSRV_SWITCH_DEBUG("Enter into %s. group id %d process start.", __FUNCTION__, p_fei_group->group_id);
    ...
    HSRV_SWITCH_DEBUG("Enter into %s. group id %d process end.", __FUNCTION__, p_fei_group->group_id);
...
static void
hsrv_openflow_ttp_show_group_brief__(ctclib_hash_backet_t* phb, void *user_data)
{
    hsrv_openflow_ttp_group_t* p_group = (hsrv_openflow_ttp_group_t*)phb->data;
    hsrv_openflow_group_debug_data_t* p_data = user_data;

    sal_fprintf(p_data->fp, "%-6u", p_data->i);
    sal_fprintf(p_data->fp, "0x%08x", p_group->group_id);
    sal_fprintf(p_data->fp, "%13s", GROUP_ID_TO_STR(p_group->group_id));
    sal_fprintf(p_data->fp, "%11u", p_group->mc_group_id);
    sal_fprintf(p_data->fp, "%9u", p_group->bucket_count);
    sal_fprintf(p_data->fp, "%6u", p_group->nh_id);
    sal_fprintf(p_data->fp, "%15s", p_group->stats_supported? "true": "false");
    sal_fprintf(p_data->fp, "%8u\n", p_group->stats_id);
...
hsrv_openflow_ttp_show_group_brief(FILE *fp)
{
    ...
    sal_fprintf(fp, "Group hash count : %d\n", g_pst_openflow_ttp_group_master->p_group_hash->count);
    sal_fprintf(fp, "%6s %10s %13s %6s %9s %6s %15s %8s\n",
                    "index", "group_id", "type", "mc_group_id", "n_buckets", "nh_id", "stats_supported", "stats-id");
    sal_fprintf(fp, "---------------------------------------------------------------------------\n");

    data.fp = fp;
    data.i = 0;
    ctclib_hash_iterate(HSRV_OPENFLOW_TTP_GROUP_HASH, hsrv_openflow_ttp_show_group_brief__, &data);

    return HSRV_E_NONE;
}

static void
hsrv_openflow_ttp_show_group_type_brief__(ctclib_hash_backet_t* phb, void *user_data)
{
    hsrv_openflow_ttp_group_t* p_group = (hsrv_openflow_ttp_group_t*)phb->data;
    hsrv_openflow_group_debug_data_t* p_data = user_data;
    if (GLB_OPENFLOW_TTP_GROUP_TYPE(p_group->group_id) == p_data->group_type)
    {
        sal_fprintf(p_data->fp, "%-6u", p_data->i);
        sal_fprintf(p_data->fp, "0x%08x", p_group->group_id);
        sal_fprintf(p_data->fp, "%13s", GROUP_ID_TO_STR(p_group->group_id));
        sal_fprintf(p_data->fp, "%6u", p_group->mc_group_id);
        sal_fprintf(p_data->fp, "%9u", p_group->bucket_count);
        sal_fprintf(p_data->fp, "%6u", p_group->nh_id);
        sal_fprintf(p_data->fp, "%15s", p_group->stats_supported? "true": "false");
        sal_fprintf(p_data->fp, "%8u\n", p_group->stats_id);
...
{
    hsrv_openflow_group_debug_data_t p_data;

    if (0 == sal_strcmp(group_type, "l2-interface"))
    {
        p_data.group_type = GLB_OPENFLOW_TTP_GROUP_TYPE_L2_INTERFACE;
    }
    else if (0 == sal_strcmp(group_type, "l3-unicast"))
    {
        p_data.group_type = GLB_OPENFLOW_TTP_GROUP_TYPE_L3_UNICAST;
    }
    else if (0 == sal_strcmp(group_type, "l2-flood"))
    {
        p_data.group_type = GLB_OPENFLOW_TTP_GROUP_TYPE_L2_FLOOD;
    }
    else if (0 == sal_strcmp(group_type, "l3-ecmp"))
    {
        p_data.group_type = GLB_OPENFLOW_TTP_GROUP_TYPE_L3_ECMP;
    }
    else
    {
        return HSRV_E_NONE;
    ...
    sal_fprintf(fp, "------------------------HSRV OPENFLOW GROUP TYPE-----------------------------\n");
    sal_fprintf(fp, "%6s %10s %13s %6s %9s %6s %15s %8s\n",
                    "index", "group_id", "type", "mc_gid", "n_buckets", "nh_id", "stats_supported", "stats-id");
    sal_fprintf(fp, "-----------------------------------------------------------------------------\n");
    ctclib_hash_iterate(HSRV_OPENFLOW_TTP_GROUP_HASH, hsrv_openflow_ttp_show_group_type_brief__, &p_data);
...
hsrv_openflow_ttp_show_group(FILE *fp, uint32 group_id)
{
    ...
    sal_fprintf(fp, "%10s %13s %6s %9s %6s %15s %8s\n",
                    "group_id", "type", "mc_gid", "n_buckets", "nh_id", "stats_supported", "stats-id");
    sal_fprintf(fp, "0x%08x", p_group->group_id);
    sal_fprintf(fp, "%13s", GROUP_ID_TO_STR(p_group->group_id));
    sal_fprintf(fp, "%6u", p_group->mc_group_id);
    sal_fprintf(fp, "%9u", p_group->bucket_count);
    sal_fprintf(fp, "%6u", p_group->nh_id);
    sal_fprintf(fp, "%15s", p_group->stats_supported? "true": "false");
    sal_fprintf(fp, "%8u\n", p_group->stats_id);
