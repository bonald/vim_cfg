...
ctc_ttp_drv_acl_add_entry(hsrv_openflow_ttp_flow_t *p_flow)
{
    ...
    if (p_entry->match_criteria.inport_mask)
    {
        ...
        if (IS_TUNNEL_PORT(p_entry->match_criteria.inport))
        {
            if (p_entry->match_criteria.metadata)
            {
                acl_entry.key.u.ipv4_key.metadata = (uint32)p_entry->match_criteria.metadata;
                acl_entry.key.u.ipv4_key.metadata_mask = 0x3FFF;
                GLB_SET_FLAG(acl_entry.key.u.ipv4_key.flag, CTC_ACL_IPV4_KEY_FLAG_METADATA);
            }
...
ctc_ttp_drv_acl_init()
{
    ...
    /* add a high priority entry for match iloop port, action permit, for prevent global acl match again */
    /* copp acl in tcam 1, low priority than tcam 0 */
    ctc_acl_entry_t acl_entry;
    uint32 entry_id = 0;
    ...
    acl_entry.priority = 65535;

    HSRV_IF_ERROR_RETURN(ctc_acl_add_entry(TTP_GLB_COPP_GROUP, &acl_entry));
    HSRV_IF_ERROR_RETURN(ctc_acl_install_entry(acl_entry.entry_id));
