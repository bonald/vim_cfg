...
void iv_run_timers(struct iv_state *st)
{
	...
	while (st->num_timers) {
		...
		t->index = 0;
        p_task = (ctc_task_t *)((char*)t - sizeof(ctc_task_hdr_t));
        ctc_task_list_delete_node(&(ctc_task_get_master()->timer), p_task);
	}
	...
	while (!iv_list_empty(&timers)) {
        ...
        ctc_task_add_unuse(ctc_task_get_master(), p_task);
	}
}
...
void iv_timer_unregister(struct iv_timer *_t)
{
	...
	if (t->index) {
		...
		*m = NULL;
		...
		st->num_timers--;
	    st->numobjs--; /* modified by kcao for STP, in one timer delete other timers, numobjs will removed repeatedly */
...
int iv_timer_get_index(const struct iv_timer *_t)
{
	struct iv_timer_ *t = (struct iv_timer_ *)_t;
