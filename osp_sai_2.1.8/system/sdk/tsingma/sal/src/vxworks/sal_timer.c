...
int ctc_sal_gettimeofday(struct timeval *tv, struct timezone *tz)
{
    int ret;
    struct timespec tp;
    ...
    {
        tv->tv_sec  = tp.tv_sec;
        tv->tv_usec = (tp.tv_nsec + 500) / 1000;   /*ns -> us*/
...
reset_timer(timer_attr_t* timer_attr)
{
    ...
    ret = ctc_sal_gettimeofday(&timer_attr->start, &tz);
...
sal_timer_func(void* param)
{
    ...
    tp.tv_sec = 0;
    tp.tv_nsec = 1000000;/*sleep 1ms*/
    while (sal_timer_activated)
    {
        nanosleep(&tp,NULL);

        ret = ctc_sal_gettimeofday(&current, &tz);
        if (ret != 0)
        {
            SAL_LOG_INFO("gettimeofday: %d\n", ret);
...
ctc_sal_timer_create(sal_timer_t** ptimer,  void (* func)(void*), void* arg)
{
    ...
    ret = ctc_sal_gettimeofday(&timer_storage[idx].start, &tz);
    if (ret != 0)
    {
        ret = -5;
        goto CREATE_TIMER_ERROR;
...
ctc_sal_timer_start(sal_timer_t* timer, uint32 timeout)
{
    ...
    ret = ctc_sal_gettimeofday(&timer_storage[idx].start, NULL);
    if (ret != 0)
    {
        SAL_LOG_INFO("gettimeofday: %d\n", ret);
        ret = -1;
