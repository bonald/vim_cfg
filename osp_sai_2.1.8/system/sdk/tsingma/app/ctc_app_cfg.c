...
#include "ctc_app_cfg_ftm_profile.h"
...
STATIC int32
_ctc_app_get_profile(uint8 lchip, ctc_init_cfg_t * p_init_config, ctc_init_chip_info_t* p_chip_info)
{
    int32 ret = 0;
    uint8 index = 0;
    uint8 loop = 0;
    ...
    ret = ctc_app_get_chip_profile((uint8*)CTC_CHIP_CONFIG, p_init_config, p_chip_info);
    if (ret < CTC_E_NONE)
    {
    ...
    if(g_ctc_app_master.mem_profile)
    {
        p_file_name = g_ctc_app_master.mem_profile;
    }
    else
    {
    ...
    if (ret == CTC_E_NONE)
    {
        CTC_APP_DBG_OUT(CTC_DEBUG_LEVEL_DUMP, "Use user-defined profile to initialize FTM \n");
    ...
    for(loop = 0; loop < p_chip_info->local_chip_num; loop ++)
    {
        if(loop != 0)
        {
            sal_sprintf(file_name,"./datapath_cfg%d.txt",loop);
        }
        else
        {
            sal_strcpy(file_name,CTC_DATAPATH_CONFIG);
        }
        ret = ctc_app_get_datapath_profile((uint8*)file_name, ((p_init_config->p_datapath_cfg) + loop));
        if (ret < 0)
        {
            ...
            {
                CTC_APP_DBG_OUT(CTC_DEBUG_LEVEL_DUMP, "Cannot find datapath_cfg.txt, failed to init SDK! \n");
    ...
    if (ret < 0)
    {
        ...
        CTC_APP_DBG_OUT(CTC_DEBUG_LEVEL_DUMP, "Use default resrc profile \n");
    }
...
_ctc_app_get_init_para(uint8 lchip, ctc_init_cfg_t * p_init_config, ctc_init_chip_info_t* p_chip_info)
{
    ...
    p_init_config->p_pkt_cfg->rx_cb         = ctc_app_packet_sample_rx;
#ifdef _SAL_LINUX_UM
    p_init_config->p_pkt_cfg->socket_tx_cb = ctc_app_packet_sample_tx;
#endif
    ...
    if (p_init_config->p_qos_cfg)
    {
        ...
        p_init_config->p_qos_cfg->max_cos_level = 0;
...
ctc_app_get_config(uint8 lchip, ctc_init_cfg_t * p_init_config,ctc_init_cfg_t * user_init_config)
{
    ...
    CTC_ERROR_RETURN(_ctc_app_get_profile(lchip, p_init_config, &chip_info));
