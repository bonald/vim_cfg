...
#include "goldengate/include/drv_error.h"
#include "goldengate/include/drv_enum.h"
#include "goldengate/include/drv_chip_info.h"
...
struct sram_tcam_info_s
{
    uint16 sram_tcam_bitmap;
    uintptr sram_tcam_sw_data_base[MAX_DRV_TCAM_BLOCK_NUM];
    uint32 sram_tcam_entry_num[MAX_DRV_TCAM_BLOCK_NUM];
    uint32 sram_tcam_entry_start_index[MAX_DRV_TCAM_BLOCK_NUM];
    uint32 sram_tcam_entry_end_index[MAX_DRV_TCAM_BLOCK_NUM];
    uintptr sram_tcam_wbit_sw_data_base[MAX_DRV_TCAM_BLOCK_NUM];
};
typedef struct sram_tcam_info_s sram_tcam_info_t;

struct sram_mem_chip_allocation_info_s
{
    char   mem_name[128];
    uint32 allocated_entry_num;            /* entry size is always 72/80 */
    uint32 max_mem_entry_num;
    uint32 hardware_data_base_address;     /* to dynic memory and TcamAd memory, is 3word mode base address */
    uint32 hardware_data_base_address_6w;  /* to dynic memory and TcamAd memory, is 6word mode base address */
    uint32 hardware_data_base_address_12w;  /* to dynic memory and TcamAd memory, is 12word mode base address */
};
typedef struct sram_mem_chip_allocation_info_s sram_mem_chip_allocation_info_t;
...
struct cmodel_mem_info_s
{
    ...
    sram_tcam_info_t *tcam_info_ptr;
};
...
#define SRAM_DYNAMIC_INFO_PTR(chip_id_offset, tbl_id) (SRAM_DYNAMIC_INFO_T (SRAM_TBL_INFO(chip_id_offset, tbl_id).sram_dynamic_info_ptr))
...
/* data base is for ad, not key */
#define SRAM_TCAM_INFO_T (sram_tcam_info_t *)
#define SRAM_TCAM_INFO_PTR(chip_id_offset, tbl_id) (SRAM_TCAM_INFO_T (SRAM_TBL_INFO(chip_id_offset, tbl_id).tcam_info_ptr))
#define SRAM_TCAM_BITMAP(chip_id_offset, tbl_id)  (SRAM_TCAM_INFO_PTR(chip_id_offset, tbl_id)->sram_tcam_bitmap)
#define SRAM_TCAM_DATA_BASE(chip_id_offset, tbl_id, blk_id) (SRAM_TCAM_INFO_PTR(chip_id_offset, tbl_id)->sram_tcam_sw_data_base[blk_id])
#define SRAM_TCAM_ENTRY_NUM(chip_id_offset, tbl_id, blk_id) (SRAM_TCAM_INFO_PTR(chip_id_offset, tbl_id)->sram_tcam_entry_num[blk_id])
#define SRAM_TCAM_START_INDEX(chip_id_offset, tbl_id, blk_id) (SRAM_TCAM_INFO_PTR(chip_id_offset, tbl_id)->sram_tcam_entry_start_index[blk_id])
#define SRAM_TCAM_END_INDEX(chip_id_offset, tbl_id, blk_id) (SRAM_TCAM_INFO_PTR(chip_id_offset, tbl_id)->sram_tcam_entry_end_index[blk_id])
#define SRAM_TCAM_WBIT_DATA_BASE(chip_id_offset, tbl_id, blk_id)(SRAM_TCAM_INFO_PTR(chip_id_offset, tbl_id)->sram_tcam_wbit_sw_data_base[blk_id])
