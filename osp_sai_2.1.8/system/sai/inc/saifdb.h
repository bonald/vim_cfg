...
typedef enum _sai_fdb_entry_type_t
{
    ...
    /** Dynamic FDB Entry without aging for MLAG */
    SAI_FDB_ENTRY_DYNAMIC_WO_AGING,

    /** Static FDB Entry with Security , added by yejl 2016-08-23*/
    SAI_FDB_ENTRY_STATIC_SECURITY,
    
    /** Dynamic FDB Entry with Security */
    SAI_FDB_ENTRY_DYNAMIC_SECURITY,
    
    /** added by yejl for FDB macfilter*/
    SAI_FDB_ENTRY_MACFILTER,
...
/**
 *  @brief FDB l2protocol flag, added by yejl for l2protocol
 */
typedef enum sai_fdb_l2protocol_flag_t
{
    /**/
    SAI_FDB_L2PROTOCOL_FLAG_PROTOCOL_MAC = 1,

    /**/
    SAI_FDB_L2PROTOCOL_FLAG_GROUP_MAC,
    
}sai_fdb_l2protocol_flag_t;
...
typedef enum _sai_fdb_entry_attr_t
{
    ...
    /* -- */
    /** l2 Protocol flag [sai_fdb_l2protocol_flag_t]
     * [sai_uint32_t] (CREATE_AND_SET)*/
    SAI_FDB_ENTRY_L2PROTOCOL_FLAG,/*Added by yejl for l2protocol*/
...
typedef enum _sai_fdb_flush_attr_t {
   ...
   /**added by yejl: Flush based on MAC [sai_mac_t]*/
   ...
   /* modified by liwh for bug 37200, 2016-08-25 
      only flush hardware fdb */
   SAI_FDB_FLUSH_ATTR_HARDWARE,
   /* liwh end */

   /* modified by yejl for DUET2 flush pending entry , 2017-08-25 */
   SAI_FDB_FLUSH_ATTR_PENDING,
   /* yejl end */
...
typedef sai_status_t (*sai_update_fdb_entry_fn)(
    _In_ const sai_fdb_entry_t* fdb_entry,
    _In_ uint32_t attr_count,
    _In_ const sai_attribute_t *attr_list
    );
...
/**
 * Routine Description:
 *    @brief Create l2protocol FDB entry
 *
 * Arguments:
 *    @param[in] fdb_entry - fdb entry
 *    @param[in] attr_count - number of attributes
 ...
 *            Failure status code on error
 */
typedef sai_status_t (*sai_create_l2protocol_entry_fn)(
    _In_ const sai_fdb_entry_t* fdb_entry,
    _In_ uint32_t attr_count,
    _In_ const sai_attribute_t *attr_list
    );
...
/**
 * Routine Description:
 *    @brief Remove l2protocol fdb entry
 *
 * Arguments:
 *    @param[in] fdb_entry - fdb entry
      @param[in] flag - protocol-mac or group-mac
 *
 * Return Values:
 *    @return SAI_STATUS_SUCCESS on success
 *            Failure status code on error
 */
typedef sai_status_t (*sai_remove_l2protocol_entry_fn)(
    _In_ const sai_fdb_entry_t* fdb_entry
    );
...
typedef sai_status_t (*sai_create_fdb_entry_with_nexthop_fn)(
    _In_ const sai_fdb_entry_t* fdb_entry,
...
typedef struct _sai_fdb_api_t
{
    ...
    sai_create_fdb_entry_fn                     create_macfilter_fdb;
    sai_remove_fdb_entry_fn                     remove_macfilter_fdb;
    sai_create_l2protocol_entry_fn              create_l2protocol_fdb;
    sai_remove_l2protocol_entry_fn              remove_l2protocol_fdb;
    ...
    sai_update_fdb_entry_fn                     update_fdb_entry;    
    sai_create_fdb_entry_with_nexthop_fn                     create_fdb_entry_with_nexthop;
