...
/**
* @brief Attribute data for SAI_SWITCH_ATTR_ISOLATION_MODE
*/
typedef enum _sai_isolation_mode_t
{
    /** Only L2 packets follow port isolation */
    SAI_ISOLATION_MODE_L2 = 1,

    /** both L2 and L3 packets follow port isolation */
    SAI_ISOLATION_MODE_ALL = 2,

} sai_isolation_mode_t;
...
typedef enum _sai_switch_attr_t
{
    ...
    * may be modified */
    ...
    /** added by yejl to set the storm-control enable globally */
    SAI_SWITCH_ATTR_IPG_STORM_CONTROL,
    
    /** added by yejl to set the memory check state */
    SAI_SWITCH_ATTR_MEMORY_CHECK_STATE,
    ...
    */
    ...
    /** Enable EXP -> TC AND COLOR MAP [sai_object_id_t] on switch.
     * MAP id = SAI_NULL_OBJECT_ID to disable map on switch.
     * To enable/disable trust Dot1p, Map ID should be add/remove on switch.
     * Default disabled */
    SAI_SWITCH_ATTR_QOS_EXP_TO_TC_AND_COLOR_MAP,
    ...
    /** added by hansf */
    /** Enable TC + COLOR -> EXP MAP [sai_object_id_t]
       Map id = SAI_NULL_OBJECT_ID to disable map on switch.
       Default no map */
    ...
    /** Set switch's port-isolation mode [sai_isolation_mode_t]*/
    SAI_SWITCH_ATTR_ISOLATION_MODE,

    /** Added by wangyb for monitor [sai_uint32_t] */
    SAI_SWITCH_ATTR_MONITOR_BUFFER_EN,
    
    /** Added by wangyb for monitor [sai_u32_list_t] */
    SAI_SWITCH_ATTR_MONITOR_BUFFER_THRESHOLD,
    
    /** Added by wangyb for monitor [sai_u32_list_t] */
    SAI_SWITCH_ATTR_MONITOR_LATENCY_RANGE,
	...
	/** trunction length [u32], default 0 */
	SAI_SWITCH_ATTR_CUSTOM_TRUNCTION_LENGTH,
	
	/** timestamp [sai_mac_t] */
	SAI_SWITCH_ATTR_CUSTOM_TIMESTAMP_ETHER_DMAC,
	SAI_SWITCH_ATTR_CUSTOM_TIMESTAMP_ETHER_SMAC,
	SAI_SWITCH_ATTR_CUSTOM_TIMESTAMP_ETHER_TYPE,
	
    /* discard pkt option[bool] */
    SAI_SWITCH_ATTR_CUSTOM_DISCARD_SAME_MACDASA_PKT,
    SAI_SWITCH_ATTR_CUSTOM_DISCARD_SAME_IPDASA_PKT,
    SAI_SWITCH_ATTR_CUSTOM_DISCARD_MCAST_SA_PKT,

    /** SAI LAG hash use inner key value[u32] (default to FALSE) */
    SAI_SWITCH_ATTR_CUSTOM_LAG_HASH_INNER_FIELD_EN,

    SAI_SWITCH_ATTR_CUSTOM_ECMP_LOAD_BALANCE_MODE,

    /** Added for configure svlan tpid on parser ethernet control */
    SAI_SWITCH_ATTR_CUSTOM_SVLAN_TPID,

    /** Added for configure cvlan tpid on parser ethernet control */
    SAI_SWITCH_ATTR_CUSTOM_CVLAN_TPID,

    SAI_SWITCH_ATTR_CUSTOM_IPV6_ENABLE, 

   /** configure qos policing&shaping include ipg or not [sai_uint8_t] */
    SAI_SWITCH_ATTR_QOS_IPG_ENABLE,

    SAI_SWITCH_ATTR_QOS_QUEUE_SHAPE_ENABLE
} sai_switch_attr_t;
...
typedef enum
{
    SAI_MONITOR_TYPE_BUFFER,
        
    SAI_MONITOR_TYPE_LATENCY
} sai_monitor_type_t;
 
typedef enum
{
    SAI_MONITOR_EVENT_START,
    
    SAI_MONITOR_EVENT_END
} sai_monitor_event_type_t;
 ...
 *		None
 */
typedef void(*sai_monitor_event_notification_fn)(
...
typedef struct _sai_switch_notification_t
{
    ...
    sai_cpu_packet_debug_fn                 on_cpu_packet_debug;
    sai_monitor_event_notification_fn        on_monitor_event;
    //sai_ipmc_event_notification_fn          on_ipmc_event;    
