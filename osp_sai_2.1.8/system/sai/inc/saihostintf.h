...
typedef enum _sai_hostif_trap_group_attr_t
{
    ...
    * This is equivalent to ACL table priority SAI_ACL_TABLE_ATTR_PRIORITY */
    ...
    SAI_HOSTIF_TRAP_GROUP_ATTR_SSH_POLICER,
    SAI_HOSTIF_TRAP_GROUP_ATTR_TELNET_POLICER,
    SAI_HOSTIF_TRAP_GROUP_ATTR_MLAG_POLICER,

    /** sai policer object id [sai_object_id_t] (GET_ONLY) 
     * (default to SAI_NULL_OBJECT_ID) */
     /*Extened by chenyq for copp module application requirement 2017.05.15*/
    SAI_HOSTIF_TRAP_GROUP_ATTR_COUNTER,
...
typedef enum _sai_hostif_trap_id_t
{
    ...
    SAI_HOSTIF_TRAP_ID_ERPS = 0x00000000c,
    
    SAI_HOSTIF_TRAP_ID_SCL_MATCH = 0x0000000d,

    SAI_HOSTIF_TRAP_ID_IGMP = 0X0000000e,
    ...
    */
    ...
    /** default packet action is forward, extend for copp by chenyq*/
    SAI_HOSTIF_TRAP_ID_ARP = 0x00002010,

    /** default packet action is forward, extend for copp by huwx*/
    SAI_HOSTIF_TRAP_ID_FWD_TO_CPU = 0x00002011,
    ...
    */
    ...
    /** ipmc packets with RPF check fail
     * (default packet action is trap) */
    SAI_HOSTIF_TRAP_ID_IPMC_RPF_FAIL = 0x00004002,

    /** port_security macnum exceeds the maxnum
     * (default packet action is trap) */
    SAI_HOSTIF_TRAP_ID_L2_PORT_MAC_LIMIT = 0x00004003, 

    /** port_security mac mismatch with other port
     * (default packet action is trap) */
    SAI_HOSTIF_TRAP_ID_L2_PORT_MAC_MISMATCH = 0x00004004, 
        
    SAI_HOSTIF_TRAP_ID_CPU_MIRROR = 0x00004005, 

    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_RANGE_BASE = 0x00005000,

    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_OPENFLOW_TO_CONTROLLER = 0x00005001,

    /* Centec extension, added by lixd, 2017-02-08 */
    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_L3_COPY_CPU = 0x00005002,

    /*added by wangl for hybrid copp*/
    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_OPENFLOW_PACKET_IN = 0x00005003,

    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_PTP_TIMESTAMP = 0x00005004,

    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_TPOAM = 0x00005005,

    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_L2PRO_PROTOCOL_MAC = 0x00005006,

    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_L2PRO_GROUP_MAC = 0x00005007,
    
//    SAI_HOSTIF_TRAP_ID_CUSTOM_EXCEPTION_CPU_MIRROR = 0x00005008,
    SAI_HOSTIF_TRAP_ID_L2_COPY_TO_CPU  = 0x00005009,
...
typedef enum _sai_hostif_trap_channel_t
{
    ...
    SAI_HOSTIF_TRAP_CHANNEL_NOPROCESS,
...
typedef enum _sai_hostif_packet_attr
{
    ...
    /** VLAN ID [sai_uint16_t] (for receive-only) */
    SAI_HOSTIF_PACKET_VLAN_ID,

    SAI_HOSTIF_PACKET_EGRESS_PORT_VLAN,

    SAI_HOSTIF_PACKET_PORT_VLAN_ID,

    SAI_HOSTIF_PACKET_TUNNEL_PAYLOAD_OFFSET,

    SAI_HOSTIF_PACKET_PDU_TYPE, /* added by kcao for debug 2017-06-13 */
    
    SAI_HOSTIF_PACKET_METADATA,

    SAI_HOSTIF_PACKET_PTP_METADATA,

    SAI_HOSTIF_PACKET_PTP_PACKET_RCV,

    SAI_HOSTIF_PACKET_TPOAM_PACKET_RCV,
    
    SAI_HOSTIF_PACKET_SUB_TYPE,
 ...
 *		None
 */
typedef void(*sai_cpu_packet_debug_fn)(
    _In_ const void *buffer,
    _In_ sai_size_t buffer_size,
    _In_ uint32_t attr_count,
    _In_ const sai_attribute_t *attr_list
