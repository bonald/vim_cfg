...
ctc_sai_copp_internal_port_init()
{
    ...
    sal_memset(&acl_prop, 0x0, sizeof(acl_prop));
    acl_prop.flag = CTC_ACL_PROP_FLAG_USE_PORT_BITMAP;
    acl_prop.acl_en = 1;
    acl_prop.acl_priority = 7;
    ...
    for (index = SAI_COPP_REASON_PORT_ID; index < SAI_COPP_PORT_ID_MAX; index++)
    {
        ...
        sys_usw_internal_port_set_routed_port(0, inter_port, 1);
        ...
        if (index == SAI_COPP_L2PRO_PROTOCOL_MAC_PORT_ID)
        {
            ...
            sys_usw_packet_speical_set_cpu_port0(0, inter_port);
            sys_usw_add_port_to_channel(0, inter_port, 77);
        }
        else if (index == SAI_COPP_L2PRO_GROUP_MAC_PORT_ID)
        {
            alloc_iloop_nh.lpbk_lport = inter_port | 0x8000;
            sys_usw_packet_speical_set_cpu_port1(0, inter_port);
            sys_usw_add_port_to_channel(0, inter_port, 77);
        }
        else if (index == SAI_COPP_IGMP_PORT_ID)
        {
            alloc_iloop_nh.lpbk_lport = inter_port | 0x8000;
            sys_usw_packet_speical_set_cpu_port2(0, inter_port);
            sys_usw_add_port_to_channel(0, inter_port, 77);
        }
        else if (index == SAI_COPP_TTL_EXPIRE_PORT_ID)
        {
            alloc_iloop_nh.lpbk_lport = inter_port | 0x8000;
            sys_usw_packet_speical_set_cpu_port3(0, inter_port);
            sys_usw_add_port_to_channel(0, inter_port, 77);
        }
        else
        {
        ...
        ctc_port_set_direction_property(inter_port, CTC_PORT_DIR_PROP_ACL_TCAM_LKUP_TYPE_0, CTC_INGRESS, CTC_ACL_TCAM_LKUP_TYPE_L2_L3);
        #ifdef OFPRODUCT
        ctc_port_set_acl_property(inter_port, &acl_prop);
        #else
        ctc_port_set_direction_property(inter_port, CTC_PORT_DIR_PROP_ACL_TCAM_LKUP_TYPE_3, CTC_INGRESS, CTC_ACL_TCAM_LKUP_TYPE_L2_L3);
        #endif
        ctc_l2pdu_set_port_action(inter_port, CTC_L2PDU_ACTION_INDEX_BPDU, CTC_PDU_L2PDU_ACTION_TYPE_FWD);
    }
...
ctc_sai_copp_tbl_init()
{
    ...
    #ifdef OFPRODUCT
    attr_list[1].value.u32 = SAI_ACL_COPP_GROUP_PRIORITY_DUET2_HYBRID;
    #else
        #ifdef TAPPRODUCT
        attr_list[1].value.u32 = SAI_ACL_COPP_GROUP_PRIORITY_DUET2_TAP;
        #else
        attr_list[1].value.u32 = SAI_ACL_COPP_GROUP_PRIORITY_DUET2;   /* tcam2, low priority than tcam0 */
        #endif 
    #endif
...
{    
    ...
    for (index = SAI_ACL_COPP_REASON_GROUP; index < SAI_ACL_COPP_REASON_GROUP_MAX; index++)
    {
        ...
        #ifdef OFPRODUCT
        acl_group.priority = SAI_ACL_COPP_GROUP_PRIORITY_DUET2_HYBRID;   /* tcam2, low priority than tcam0 for hybrid 550*/
        #elif TAPPRODUCT
        acl_group.priority = SAI_ACL_COPP_GROUP_PRIORITY_DUET2_TAP;
        #else
        acl_group.priority = SAI_ACL_COPP_GROUP_PRIORITY_DUET2;   /* tcam2, low priority than tcam0 */
        #endif
    ...
    port_bitmap = (1 << SAI_COPP_REASON_PORT_ID) | (1 << SAI_COPP_FWD_PORT_ID) |
                  ...
                  (1 << SAI_COPP_MTU_CHECK_FAIL_PORT_ID);
    acl_group.un.port_bitmap[0] = port_bitmap;
    ctc_acl_create_group(group_id, &acl_group);
    ctc_acl_install_group(group_id, &acl_group);

    /* add by hansf for d2 copp */
    ctc_acl_entry_t  acl_entry;  /*duet2 default entry used to sperated with normal ace*/
    sal_memset(&acl_entry, 0x0, sizeof(acl_entry));
    acl_entry.key.type = CTC_ACL_KEY_MAC_IPV4;
