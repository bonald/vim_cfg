...
#ifdef GREATBELT
extern int32
sys_greatbelt_internal_port_set_routed_port(uint8 lchip, uint16 gport, uint32 is_routed);
#endif
...
ctc_sai_copp_internal_port_init()
{
    ...
    for (index = SAI_COPP_REASON_PORT_ID; index < SAI_COPP_PORT_ID_MAX; index++)
    {
        ...
        sys_greatbelt_internal_port_set_routed_port(0, inter_port, 1);
        ...
        ctc_port_set_direction_property(inter_port, CTC_PORT_DIR_PROP_ACL_EN, CTC_INGRESS, 0x3); /*tcam0 & tcam1*/
...
ctc_sai_copp_tbl_init()
{
    ...
    attr_list[1].value.u32 = SAI_ACL_COPP_GROUP_PRIORITY_GB;
...
{    
    ...
    for (index = SAI_ACL_COPP_REASON_GROUP; index < SAI_ACL_COPP_REASON_GROUP_MAX; index++)
    {
        ...
        acl_group.priority = SAI_ACL_COPP_GROUP_PRIORITY_GB;   // tcam2, low priority than tcam0 
    ...
    /* add by hansf for gb copp */
    ctc_acl_entry_t  acl_entry;  /*gb default entry used to sperated with normal ace*/
    ...
    /*added by yejl for ipv6 profile and bug 49927: default entry used to sperated with normal ace*/
    if (ftm_info_profile_type_gb == CTC_FTM_PROFILE_5 || GLB_STM_MODE_OF_IPV6 == hybrid_profile_type)
    {
        sal_memset(&acl_entry, 0x0, sizeof(ctc_acl_entry_t));
        acl_entry.key.type = CTC_ACL_KEY_IPV6;
        acl_entry.priority_valid = 1;
        acl_entry.priority = SAI_ACL_COPP_DEFAULT_IPV6_ACL_PRIORITY;
        acl_entry.entry_id = SAI_ACL_COPP_DEFAULT_IPV6_ACL_ENTRY_ID;
        ctc_acl_add_entry(group_id, &acl_entry);
        ctc_acl_install_entry(SAI_ACL_COPP_DEFAULT_IPV6_ACL_ENTRY_ID);
    }
    ...
    )
{
    ...
    /*added by huwx for bug 47348, GB CTC_PKT_CPU_REASON_FWD_CPU reason special, 2018-07-24 */
    if (0 == ctc_queue_id)
    {
        sal_memset(&queue_stats, 0x0, sizeof(queue_stats));
        queue_stats.queue.queue_type = CTC_QUEUE_TYPE_EXCP_CPU;
        queue_stats.queue.cpu_reason = CTC_PKT_CPU_REASON_FWD_CPU;
        ret = ctc_qos_query_queue_stats(&queue_stats);
        if(ret)
        {
            return ret;
