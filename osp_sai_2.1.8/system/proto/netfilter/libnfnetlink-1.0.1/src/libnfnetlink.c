...
struct nfnl_handle {
	...
	u_int32_t		flags;
    void*           pRead;              /* read handler taocy added for CTC NAT 2019-06-03*/
...
extern void ctc_task_delete_read(ctc_task_t *p_task);
extern ctc_task_t* ctc_task_add_read(int32 prio, CTC_TASK_HANDLER handler, void *p_arg, int32 fd);

void ctc_nftnl_read_cb(void *pArg)
{
    struct nfnl_handle *h = (struct nfnl_handle *)pArg;
    unsigned char buf[h->rcv_buffer_size] __attribute__ ((aligned));
    int ret;

    ret = nfnl_recv((const struct nfnl_handle *)h, buf, sizeof(buf));
    if (ret == -1) 
    {
        return;
    }
...
int ctc_nfnl_catch(struct nfnl_handle *h)
{
    assert(h);
    ...
    h->pRead = ctc_task_add_read(TASK_PRI_NORMAL, ctc_nftnl_read_cb, h, h->fd); 
    return 0;
}
