...
struct ofp_if_tx_stats
{
    ...
    uint64_t queue_drop_pkts;              /**< total number of packets dropped due to queue overflow */
};
...
/**
 @brief meter statistics data structure, Added by weizj for meter
*/
struct ofp_meter_stats_s
{
    uint64_ofp confirmPackets;
    uint64_ofp confirmBytes;
    uint64_ofp exceedPackets;
    uint64_ofp exceedBytes;
    uint64_ofp violatePackets;
    uint64_ofp violateBytes;
};
typedef struct ofp_meter_stats_s ofp_meter_stats_t;
