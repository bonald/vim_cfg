...
static int parse_rtattr(struct rtattr *tb[], int max, struct rtattr *rta, int len)
{
    ...
    if (len)
	log(LOG_WARNING, 0, "NETLINK: Deficit in rtattr %d", len);
...
int init_routesock(void)
{
    ...
    if (routing_socket < 0) {
	log(LOG_ERR, errno, "Failed creating netlink socket");
	return -1;
    }
	...
	log(LOG_ERR, errno, "Failed binding to netlink socket");
	return -1;
    }

    addr_len = sizeof(local);
    if (getsockname(routing_socket, (struct sockaddr *)&local, &addr_len) < 0) {
	log(LOG_ERR, errno, "Failed netlink getsockname");
	return -1;
    }

    if (addr_len != sizeof(local)) {
	log(LOG_ERR, 0, "Invalid netlink addr len.");
	return -1;
    }

    if (local.nl_family != AF_NETLINK) {
	log(LOG_ERR, 0, "Invalid netlink addr family.");
	return -1;
    }
...
int k_req_incoming(uint32_t source, struct rpfctl *rpf)
{
	...
	//IF_DEBUG(DEBUG_RPF)
	    log(LOG_DEBUG, 0, "NETLINK: ask path to %s", inet_fmt(rpf->source.s_addr, s1));
    }
	...
	if (!IN_LINK_LOCAL_RANGE(rpf->source.s_addr)) {
		...
		log(LOG_WARNING, 0, "netlink: wrong answer type %d", n->nlmsg_type);
	    else
...
static int getmsg(struct rtmsg *rtm, int msglen, struct rpfctl *rpf)
{
    ...
    if (!rpf) {
	log(LOG_WARNING, 0, "Missing rpf pointer to netlink.c:getmsg()!");
	return FALSE;
    }
    ...
    if (!rta[RTA_OIF]) {
	log(LOG_WARNING, 0, "NETLINK: no outbound interface");
	return FALSE;
    }
